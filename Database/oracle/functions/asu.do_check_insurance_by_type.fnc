DROP FUNCTION ASU.DO_CHECK_INSURANCE_BY_TYPE
/

--
-- DO_CHECK_INSURANCE_BY_TYPE  (Function) 
--
--  Dependencies: 
--   STANDARD (Package)
...
--
CREATE OR REPLACE FUNCTION ASU.DO_CHECK_INSURANCE_BY_TYPE (pFC_SER IN VARCHAR2,pFC_NUM IN VARCHAR2,pFK_COMPANYID IN NUMBER,pFK_PEPLID IN NUMBER, pFK_TYPEDOCID in number)
  ...
  CURSOR cCount1 is
    SELECT FK_PEPLID
     ...
     WHERE TRIM(FC_SER||FC_NUM)=TRIM(pFC_SER||pFC_NUM)
       ...
       and FK_TYPEDOCID = pFK_TYPEDOCID;
/*  CURSOR cCount2 is
    SELECT FK_PEPLID
      FROM TINSURANCE
