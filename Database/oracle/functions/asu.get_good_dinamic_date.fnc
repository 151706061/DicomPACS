DROP FUNCTION ASU.GET_GOOD_DINAMIC_DATE
/

--
-- GET_GOOD_DINAMIC_DATE  (Function) 
--
--  Dependencies: 
--   STANDARD (Package)
...
--   GET_SEX (Function)
--
CREATE OR REPLACE FUNCTION ASU."GET_GOOD_DINAMIC_DATE" (pFK_PACID IN NUMBER, pFK_SMID IN NUMBER,pFK_COLID IN NUMBER,pFD_INS IN DATE)
  RETURN NUMBER IS

  CURSOR cCheckCount (pPACID NUMBER,pSMID NUMBER,pCOLID NUMBER,pINS DATE) IS
    SELECT COUNT(FK_ID) FROM VRESAN WHERE FK_PACID=pPACID AND FK_SMID=pSMID AND FK_COLID=pCOLID AND FD_INS<=pINS;
  CURSOR cCheckCount1 (pPACID NUMBER,pSMID NUMBER,pINS DATE) IS
    SELECT COUNT(FK_ID) FROM VRESAN WHERE FK_PACID=pPACID AND FK_SMID=pSMID AND FD_INS<=pINS;

  CURSOR cCVal (pPACID NUMBER,pSMID NUMBER,pCOLID NUMBER,pINS DATE) IS
    SELECT FP_PRIOR FL_PAT, FN_RES, FN_RES2
     ...
     WHERE VRESAN.FK_ID = FK_MINID AND TPATNAME.FK_ID=FK_PATID AND FD_INS<=pINS;
  CURSOR cCVal1 (pPACID NUMBER,pSMID NUMBER,pINS DATE) IS
    SELECT FP_PRIOR FL_PAT, FN_RES, FN_RES2
      FROM VRESAN,(SELECT MIN(FK_ID) FK_MINID FROM VRESAN WHERE FK_PACID=pPACID AND FK_SMID = pSMID ),TPATNAME
     WHERE VRESAN.FK_ID = FK_MINID AND TPATNAME.FK_ID=FK_PATID AND FD_INS<=pINS;
  CURSOR c (pPACID NUMBER,pSMID NUMBER,pCOLID NUMBER,pINS DATE) IS
    SELECT FP_PRIOR FL_PAT, FN_RES, FN_RES2
      FROM VRESAN,TPATNAME,(SELECT MAX(FK_ID) FK_MAXID FROM VRESAN WHERE FK_PACID=pPACID AND FK_SMID = pSMID AND FK_COLID=pCOLID)
     WHERE VRESAN.FK_ID=FK_MAXID AND FK_PATID=TPATNAME.FK_ID AND FD_INS<=pINS;
  CURSOR c1 (pPACID NUMBER,pSMID NUMBER,pINS DATE) IS
    SELECT FP_PRIOR FL_PAT, FN_RES, FN_RES2
...
*/
  IF pFK_COLID IS NULL THEN
    OPEN cCheckCount1(pFK_PACID,pFK_SMID,pFD_INS);
    FETCH cCheckCount1 INTO nCOUNT;
    CLOSE cCheckCount1;
  ELSE
    OPEN cCheckCount(pFK_PACID,pFK_SMID,pFK_COLID,pFD_INS);
    FETCH cCheckCount INTO nCOUNT;
    CLOSE cCheckCount;
  END IF;
  IF nCOUNT<2 THEN
    RETURN 0;
  END IF;
  IF pFK_COLID IS NULL THEN
    OPEN cCVal1(pFK_PACID,pFK_SMID,pFD_INS);
    FETCH cCVal1 INTO nID, nCV1, nCV2;
    CLOSE cCVal1;
  ELSE
    OPEN cCVal(pFK_PACID,pFK_SMID,pFK_COLID,pFD_INS);
  ...
  IF pFK_COLID IS NOT NULL THEN
    FOR p IN c (pFK_PACID,pFK_SMID,pFK_COLID,pFD_INS) LOOP
  --      raise_application_error(-20002,p.FL_PAT);
    ...
    END LOOP;
  ELSE
    FOR p IN c1 (pFK_PACID,pFK_SMID,pFD_INS) LOOP
  --      raise_application_error(-20002,p.FL_PAT);
      IF p.FL_PAT = 0 THEN
        RETURN 1;
      ELSIF strType = 'NUMBER' THEN
