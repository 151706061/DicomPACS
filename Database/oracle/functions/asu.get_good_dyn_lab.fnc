DROP FUNCTION ASU.GET_GOOD_DYN_LAB
/

--
-- GET_GOOD_DYN_LAB  (Function) 
--
--  Dependencies: 
--   STANDARD (Package)
...
--   GET_SEX (Function)
--
CREATE OR REPLACE FUNCTION ASU."GET_GOOD_DYN_LAB" (pFK_NAZID IN NUMBER, pFK_PACID IN NUMBER, pFD_INS IN DATE, pFK_SMID IN NUMBER,pFK_COLID IN NUMBER)
  RETURN NUMBER IS

  CURSOR c (pID NUMBER,pPACID NUMBER,pSMID NUMBER,pCOLID NUMBER) IS
  ...
  CURSOR c1 (pID NUMBER,pPACID NUMBER,pSMID NUMBER) IS
    SELECT FP_PRIOR FL_PAT, FN_RES, FN_RES2
      FROM VRESAN,(SELECT MIN(FK_ID) FK_MINID FROM VRESAN WHERE FK_PACID=pPACID AND FK_SMID = pSMID ),TPATNAME
     WHERE VRESAN.FK_ID = FK_MINID AND TPATNAME.FK_ID=FK_PATID;
  CURSOR cCheckMax(pPACID NUMBER,pSMID NUMBER) IS
     SELECT MAX(FK_ID) FROM VRESAN WHERE FK_PACID=pPacID AND FK_SMID=pSMID;
  CURSOR cCID(pNAZID NUMBER,pSMID NUMBER,pCOLID NUMBER) IS SELECT FK_ID FROM VRESAN WHERE FK_NAZID=pNAZID AND FK_SMID=pSMID AND FK_COLID=pCOLID;
  CURSOR cCID1(pNAZID NUMBER,pSMID NUMBER) IS SELECT FK_ID FROM VRESAN WHERE FK_NAZID=pNAZID AND FK_SMID=pSMID;
/*  CURSOR cCVal (pNAZID NUMBER, pSMID NUMBER) IS
    SELECT FK_ID, FN_RES, FN_RES2
      FROM VRESAN
...
*/
  ...
  nID NUMBER;
BEGIN
--  GET_TYPE_FROM_SMID(pFK_SMID);
/*  OPEN cCheckMax(pFK_PACID,pFK_SMID);
  FETCH cCheckMax INTO nMAX;
  CLOSE cCheckMax;

  IF pFK_COLID IS NULL THEN
    OPEN cCID1(pFK_PACID,pFK_SMID);
    FETCH cCID1 INTO nID;
    CLOSE cCID1;
  ELSE
    OPEN cCID(pFK_PACID,pFK_SMID,pFK_COLID);
    FETCH cCID INTO nID;
    CLOSE cCID;
  END IF;
  IF nID <>nMax THEN
...
*/
  ...
  END IF;
  IF pFK_COLID IS NOT NULL THEN
    FOR p IN c (nID,pFK_PACID,pFK_SMID,pFK_COLID) LOOP
  --      raise_application_error(-20002,p.FL_PAT);
    ...
    END LOOP;
  ELSE
    FOR p IN c1 (nID,pFK_PACID,pFK_SMID) LOOP
  --      raise_application_error(-20002,p.FL_PAT);
      IF p.FL_PAT = 0 THEN
        RETURN 1;
      ELSIF strType = 'NUMBER' THEN
