DROP FUNCTION ASU.GET_LASTMONTHDAYS
/
...
--   DO_VYBDATE (Function)
--
CREATE OR REPLACE FUNCTION ASU."GET_LASTMONTHDAYS" 
  ( pacid in number)
  RETURN  number IS
 cursor c1  is select TRUNC(fd_data1), TRUNC(fd_data3)
                 from tsroky
                where fk_pacid=pacid and fk_pryb in (2, 4, 5, 6)
                  and trunc(fd_data1) < trunc(do_vybdate(fk_pacid), 'mon');
