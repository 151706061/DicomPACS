DROP FUNCTION ASU.GET_MAPPING_IEMK_SPR
/

--
-- GET_MAPPING_IEMK_SPR  (Function) 
--
--  Dependencies: 
--   STANDARD (Package)
...
--
CREATE OR REPLACE FUNCTION ASU.GET_MAPPING_IEMK_SPR(pTHEIR_SPR_NAME VARCHAR2, pOWN_SPR_NAME VARCHAR2, pOWN_CODE NUMBER, pCOMP_IDV NUMBER DEFAULT ASU.GET_IEMK_COMPANY_ID)
 RETURN NUMBER
IS
--Возвращает значение (ключ) из справочника соответсвий
...
-- pCOMP_IDV - ID строки для "комапнии" в справочнике соответсвий
  retVal  NUMBER;
BEGIN
   select MAX(td_t.fn_code) INTO retVal
