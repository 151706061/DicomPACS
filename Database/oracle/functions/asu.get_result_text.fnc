DROP FUNCTION ASU.GET_RESULT_TEXT
/
...
--
CREATE OR REPLACE FUNCTION ASU."GET_RESULT_TEXT" (pFK_NAZID IN NUMBER)
  RETURN  NUMBER IS
--
...
--Просто значение?
  IF c not IN ('TABLE','MTABLE') THEN
    nSMID:=GET_RESULT_SIMPLE(pFK_NAZID);
  ELSIF c='TABLE' THEN
    nSMID:=GET_RESULT_TABLE(pFK_NAZID);
  ELSIF c='MTABLE' THEN
    nSMID:=GET_RESULT_MTABLE(pFK_NAZID);
 END IF;
  RETURN nSMID;
