DROP FUNCTION ASU.GET_RESULT_TEXT3
/
...
--
CREATE OR REPLACE FUNCTION ASU."GET_RESULT_TEXT3" (pFK_NAZID IN NUMBER)
  RETURN  NUMBER IS
--
...
CURSOR cGetHeader(pFK_SMID NUMBER) IS SELECT /*+rule*/FC_NAME,FC_ED_IZM,GET_NORM(pFK_SMID,GET_PACID_FROM_NAZID(pFK_NAZID)),FC_TYPE FROM TSMID WHERE FK_ID=pFK_SMID;
...
--Просто значение?
  IF c not IN ('TABLE','MTABLE') THEN
    nSMID:=GET_RESULT_SIMPLE3(pFK_NAZID);
  ELSIF c='TABLE' THEN
    nSMID:=GET_RESULT_TABLE3(pFK_NAZID);
  ELSIF c='MTABLE' THEN
    nSMID:=GET_RESULT_MTABLE3(pFK_NAZID);
 END IF;
  RETURN nSMID;
