DROP FUNCTION ASU.GET_XRAY_KOLVO_KONTRAST
/

--
-- GET_XRAY_KOLVO_KONTRAST  (Function) 
--
--  Dependencies: 
--   STANDARD (Package)
...
--   GET_VIPNAZ (Function)
--
CREATE OR REPLACE FUNCTION ASU."GET_XRAY_KOLVO_KONTRAST" (PSMID IN NUMBER, PDATE1 IN DATE, PDATE2 IN DATE, nKey IN NUMBER DEFAULT 0)
  RETURN NUMBER IS
  ...
  CURSOR cResVsego IS SELECT SUM(T.SUMNUM) 
                        FROM (SELECT SUM(TIB.FN_NUM) AS SUMNUM 
                               ...
                               WHERE TIB.FK_PACID = TNAZIS.FK_ID
                                 AND TNAZIS.FD_RUN >= PDATE1
                                 AND TNAZIS.FD_RUN <= PDATE2
                                 AND TIB.FK_SMID = (SELECT FK_ID FROM TSMID WHERE FC_SYNONIM = 'XRAY_KOLVOKONTRAST')
                                 AND TNAZIS.FK_NAZSOSID IN (ASU.GET_LABVIP, ASU.GET_VIPNAZ)
                                 --AND EXISTS (SELECT NULL FROM TPODPISNAZ WHERE FK_NAZID = TNAZIS.FK_ID AND FK_SOS = ASU.GET_LABVIP)
                              GROUP BY TNAZIS.FC_TYPE) T;                       
  nRes NUMBER;
