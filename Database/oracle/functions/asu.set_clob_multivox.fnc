DROP FUNCTION ASU.SET_CLOB_MULTIVOX
/
...
--  idPac NUMBER;
  ...
  vpeplid varchar2(1000);
  vIB     varchar2(1000);
  verr    number(1) := 0;

  CURSOR cPac2 IS SELECT ' ' as FC_FAM,
                        ' ' as FC_IM,
                        ' ' as FC_OTCH,
                        TO_CHAR(sysdate, 'YYYYMMDD') AS DATE_BIRTH,
                        'U' AS PACIENT_SEX,
                        ' ' AS IsAmbul,
                        -1 as TypeKart                        
                   FROM dual;
...
BEGIN
  ...
  WHEN OTHERS THEN 
    verr :=1;
    OPEN cPac2;
    FETCH cPac2 INTO Fam, Im, Otch, DateBirth, PatientSex, IsAmbul, TypeKart;
    CLOSE cPac2;
  end;
  ...
  if verr=0 then
    vpeplid := TO_CHAR(asu.get_peplid(PFK_PACID));
    vIB     := ASU.GET_IB(PFK_PACID);
  else
    vpeplid := ' ';
    vIB     := ' ';
  end if;
  ...
  sTmp := UTL_I18N.string_to_raw('<?xml version="1.0" encoding="windows-1251"?>'||chr(13)||
          ...
          '<PID.2><CX.1>'||vIB||'</CX.1><!--Карта пациента-->'||chr(13)||
--          '<CX.5>'||IsAmbul||'<!--Тип карты (идентификатор типа): I для стационарной, O для амбулаторной--></CX.5>'||chr(13)||
          ...
          '</PID.2>'||chr(13)||
          '<PID.3><CX.1>'||vpeplid||'<!--Идентификатор пациента--></CX.1>'||chr(13)||
          '<CX.5>PI</CX.5>'||chr(13)||
          '</PID.3>'||chr(13)||
