DROP PACKAGE BODY ASU.PKG_R_GRAFOSV
/

--
-- PKG_R_GRAFOSV  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY ASU."PKG_R_GRAFOSV" -- Modified by TimurLan
 IS
  CURSOR cKorp IS SELECT * FROM tkorp order by 1;
...
|
\*-------------------------------------------------------------------------------------------*/
  Function COUNT_OUT(pFD_DATAC IN DATE)
  RETURN NUMBER
   IS
    CURSOR C IS -- вместе с запланированными почему-то
...
|
\*-------------------------------------------------------------------------------------------*/
  FUNCTION COUNT_OUT_KORP_FLOOR (pFD_DATAC IN DATE, pfk_korpid IN NUMBER, pfn_floor IN NUMBER)
  RETURN NUMBER
   IS
    CURSOR c IS-- вместе с запланированными почему-то
     SELECT /* + rule*/COUNT (tperesel.fk_id)
