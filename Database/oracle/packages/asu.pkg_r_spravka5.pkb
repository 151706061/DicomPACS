DROP PACKAGE BODY ASU.PKG_R_SPRAVKA5
/

--
-- PKG_R_SPRAVKA5  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY ASU."PKG_R_SPRAVKA5" 
 IS-- Created by TimurLan
  ...
  BEGIN
    ...
    str := PKG_HTML.GET_HEAD('word','Справка о движении отдыхающих по санаторию "' ||PKG_SMINI.READSTRING('CONFIG','S_NAME','Не известен')||'"',
                             null,'10.0',null,
                             null,null,null,
                             null,null,null,
                             '297','210','landscape',
    ...
    str:='<BODY><DIV class=user>'||
         ...
         '<P ALIGN="CENTER"><B>Справка о движении отдыхающих по санаторию "'||PKG_SMINI.READSTRING('CONFIG','S_NAME','Не известен')||'"<BR>'||
...
/*    OPEN cBegin(pFD_DATA1);
    ...
    str:='<TR><TD><P class=user>1.</P></TD>'||
         ...
         '<TD><P class=user>'||/*nTotal*/PKG_R_ALLFUNC.Get_Count_Morning(pFD_DATA1)||'</P></TD>'||
    ...
    str:='<TR><TD><P class=user>2.</P></TD>'||
         ...
         '<TD><P class=user>'||PKG_R_ALLFUNC.GET_COUNT_PRYB(pFD_DATA1)||'</P></TD>'||
    ...
    str:='<TR><TD><P class=user>3.</P></TD>'||
         ...
         '<TD><P class=user>'||PKG_R_ALLFUNC.GET_COUNT_VYB(pFD_DATA1)||'</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
         '<TD><P class=user>'||nTotal||'</P></TD>'||
         '<TD><P class=user>'||nShortTotal||'</P></TD>'||
...
/*    OPEN cBegin(pFD_DATA1+1);
    ...
    str:='<TR><TD><P class=user>4.</P></TD>'||
         ...
         '<TD><P class=user>'||PKG_R_ALLFUNC.Get_Count_Morning(pFD_DATA1+1)/*nTotal*/||'</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
         '<TD><P class=user>&nbsp</P></TD>'||
    ...
    for p IN cTypePut loop
      ...
      FOR n IN cPryb(pFD_DATA1,p.FK_ID) LOOP
        str:=str||'<TR><TD><P class=user>'||n1||'.</P></TD>'||
                  ...
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FC_PUT)||'</P></TD>'||
                  '<TD><P class=user>'||TO_CHAR(n.FD_DATA1,'dd.mm.yyyy')||'</P></TD>'||
                  '<TD><P class=user>'||TO_CHAR(n.FD_DATA2,'dd.mm.yyyy')||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_RANEE)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_OPOZD)||'</P></TD></TR>';
        n2:=n2+n.FN_RANEE;
        n3:=n3+n.FN_OPOZD;
        n1:=n1+1;
        DBMS_LOB.WRITEAPPEND(cC,LENGTH(str),str);
...
/*    OPEN cCountPut(pFD_DATA1);
    ...
    str:='<TR><TD><P class=user>&nbsp;</P></TD>'||
         ...
         '</TABLE>'||
         '<BR><P>Путевки на прибывших в количестве '||PKG_R_ALLFUNC.GET_COUNT_INC_PUT(pFD_DATA1)||' штук прилагаются.</P></DIV>';
    DBMS_LOB.WRITEAPPEND(cC,LENGTH(str),str);
    ...
    for p IN cTypePut loop
      ...
      FOR n IN cVyb(pFD_DATA1,p.FK_ID) LOOP
        str:=str||'<TR><TD><P class=user>'||n1||'.</P></TD>'||
                  ...
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_RANEE)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_OPOZD)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_VYBRANEE)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_PRODLOP)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_PRODLNAL)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_PROSHL)||'</P></TD>'||
                  '<TD><P class=user>'||PKG_R_ALLFUNC.RETURN_VALUE(n.FN_TEKUSH)||'</P></TD></TR>';
        n2:=n2+n.FN_RANEE;
        n3:=n3+n.FN_OPOZD;
        n4:=n4+n.FN_VYBRANEE ;
        n5:=n5+n.FN_PRODLOP;
