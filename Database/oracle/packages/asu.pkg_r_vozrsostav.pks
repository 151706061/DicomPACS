DROP PACKAGE ASU.PKG_R_VOZRSOSTAV
/

--
-- PKG_R_VOZRSOSTAV  (Package) 
--
--  Dependencies: 
--   STANDARD (Package)
--   TSROKY (Table)
...
--   TPRIZN (Table)
--
CREATE OR REPLACE PACKAGE ASU."PKG_R_VOZRSOSTAV" 
 IS
  ...
  CURSOR cShortTotal(pFD_DATA1 DATE,pFD_DATA2 DATE,n1 NUMBER,n2 NUMBER) IS
    SELECT COUNT(FK_ID)
      FROM TKARTA,
           (SELECT FK_PACID,FD_DATA1,GET_PACDAYSPLAN(FK_PACID) FN_DAYS
              FROM TSROKY
             WHERE FD_DATA1 BETWEEN pFD_DATA1 and pFD_DATA2+1-1/(24*60*60)
               AND FK_PRYB=3)
     WHERE FK_ID = FK_PACID
  ...
  CURSOR cShortKontAll(pFD_DATA1 DATE,pFD_DATA2 DATE,nPrizn NUMBER) IS
    SELECT COUNT(FK_ID)
      FROM TKARTA,
           (SELECT FK_PACID,GET_PACDAYSPLAN(FK_PACID) FN_DAYS
              FROM TSROKY
             WHERE FD_DATA1 BETWEEN pFD_DATA1 and pFD_DATA2+1-1/(24*60*60)
               AND FK_PRYB=3)
     WHERE FK_ID = FK_PACID
