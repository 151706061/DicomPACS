DROP PACKAGE BODY ASU.PKG_REGIST_GORKANEW_OLD
/

--
-- PKG_REGIST_GORKANEW_OLD  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY ASU."PKG_REGIST_GORKANEW_OLD" IS-- Created by TimurLan
  FUNCTION cGET_FC_KOD(PAC_ID NUMBER)
  RETURN VARCHAR2
   IS
  cTemp VARCHAR2(100);
  CURSOR c IS SELECT FC_NAME FROM TSUBVID WHERE FK_ID = (SELECT FK_KOD FROM TKARTA WHERE FK_ID = PAC_ID);
  BEGIN
    OPEN c;
    FETCH c INTO cTemp;
    CLOSE c;
    RETURN cTemp;
  END;
  FUNCTION DO_MONTHIB_BY_ALL (pFD_DATA1 IN DATE,pFD_DATA2 IN DATE,pFK_VRACHID IN NUMBER)
  RETURN NUMBER
  ...
  END;
/*############################################################################################################*/
  FUNCTION DO_MONTHIB_BY_KORP (pFD_DATA1 IN DATE, pFD_DATA2 IN DATE, pFK_VRACHID IN NUMBER, pFK_KORPID IN NUMBER)
  RETURN NUMBER
   IS-- Used in rMonthIB.dpr
    ...
    CURSOR cMain is
           select GET_PACFIO(fk_pacid) as FC_FIO,
            ...
            where fd_data1 between pFD_DATA1 and pFD_DATA2+1-1/(24*60)
              and fk_pryb = 2
              AND GET_PACKORPID(fk_pacid,pFD_DATA2) = pFK_KORPID
         ORDER BY 3,1;
    nC NUMBER;
    cC CLOB;
    str VARCHAR2 (32767);
