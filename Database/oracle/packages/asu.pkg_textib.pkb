DROP PACKAGE BODY ASU.PKG_TEXTIB
/

--
-- PKG_TEXTIB  (Package Body) 
--
CREATE OR REPLACE PACKAGE BODY ASU."PKG_TEXTIB" 
 IS
  sTmp       VARCHAR2(32767);
...
/*commented by sill 11.02.06*/  
  ...
  \*------------------------------------------------------------------------------------------------------------------*/
    ...
    IS
      ...
      IS
        SELECT /*+rule*/fk_id, fl_first, fd_date
          FROM tdiag
  ...
  BEGIN
    ...
    if pFN_MODE = 0 then
      cTemp := asu.do_diagbytype(pFK_PACID, 1, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Диагноз с места отбора:</b><br><u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := asu.do_diagbytype(pFK_PACID, 1, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
        end if;
      end if;

      cTemp := asu.do_diagbytype(pFK_PACID, 6, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Диагноз при поступлении:</b><br><u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := asu.do_diagbytype(pFK_PACID, 6, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      cTemp := asu.do_diagbytype(pFK_PACID, 2, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Клинический диагноз:</b><br>' || '<u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := asu.do_diagbytype(pFK_PACID, 2, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      cTemp := asu.do_diagbytype(pFK_PACID, 5, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Клинический заключительный диагноз:</b><br>' || '<u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := asu.do_diagbytype(pFK_PACID, 5, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      /*cTemp := DO_INTERRDIAG;
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Интеркуррентные заболевания:</b>' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
      end if;*/

    else
      cTemp := do_diagbytype(/*pFK_PACID, */1, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Диагноз с места отбора:</b><br><u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := do_diagbytype(/*pFK_PACID, */1, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      cTemp := do_diagbytype(/*pFK_PACID, */6, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Диагноз при поступлении:</b><br><u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := do_diagbytype(/*pFK_PACID, */6, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      cTemp := do_diagbytype(/*pFK_PACID, */2, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Клинический диагноз:</b><br>' || '<u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := do_diagbytype(/*pFK_PACID, */2, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
      ...
      cTemp := do_diagbytype(/*pFK_PACID, */5, 1, -1, 1);
      if cTemp is not null or TRIM(cTemp) <> '' then
        sTmp := '<p><b>Клинический заключительный диагноз:</b><br>' || '<u>Основной:</u> ' || cTemp;
        DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        cTemp := null;
        cTemp := do_diagbytype(/*pFK_PACID, */5, 0, -1, 1);
        if cTemp is not null or TRIM(cTemp) <> '' then
          sTmp := '<br><u>Сопутствующий:</u> ' || '' || cTemp;
          DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          cTemp := null;
 ...
 /* sTmp := '<br><b>Интеркуррентные заболевания:</b><br>';
    ...
    if pFK_PACID IS NULL THEN
      ...
      FOR i IN c LOOP
        IF i.name IS NOT NULL THEN
          ...
          END IF;
        ELSE
          sTxt := TRIM(get_ibformvalue(pFK_PACID, i.fk_id, ''));
          IF sTxt IS NOT NULL THEN
            if not bNull then
              sTmp := '<p><h3 align = "center">' || sCaption || '</h3></p>';
              DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
        ...
        FOR i IN c(arPacList(n)) LOOP
          if not bExists then
            sTmp := '<p><h3 align = "center">Лист диагностических назначений</h3></p>'||
                    '<table align="center" width="100%" bordercolorlight="#000000" bordercolordark="#FFFFFF" bordercolor="#FFFFFF" bgcolor="#FFFFFF" border="1" cellpadding="0" cellspacing="0">' ||
  ...
  FUNCTION getProcListIbText(pFK_PACID IN NUMBER) RETURN CLOB IS
    CURSOR c(pFK_PACID IN NUMBER) IS
      SELECT /*+rule*/
       TO_CHAR(fd_naz, 'dd.mm.yyyy') AS fd_naz,
       get_fullpath(fk_smid) || ' ' || PKG_NAZ.GET_NAZPARAM(FK_ID) AS naim,
  ...
  END;
  /*------------------------------------------------------------------------------------------------------------------*\
  |               getResAnIbTextРЕЗУЛЬТАТЫ ЛАБОРАТОРНЫХ ИССЛЕДОВАНИЙ
  \*------------------------------------------------------------------------------------------------------------------*/
...
--Mikrom
  ...
  FUNCTION getPodpisiIbText(pFK_PACID IN NUMBER) RETURN VARCHAR2 IS
    ...
    bLechVrach NUMBER;
  BEGIN
    IF do_vrachfio(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID)) <>
       get_zavotdfio(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID)) THEN
      sTmp := '<P><HR><table width="100%" border="0" cellspacing="0" cellpadding="0"> ' ||
              '  <tr> ' ||
              '    <td width="41%"><b>##VALUE19</b></td> ' ||
              '    <td width="59%">/' || do_vrachfio(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID)) || '/</td>' ||
              '  </tr> ' || '  <tr> ' ||
              '    <td width="41%"><b>Зав. отделением ##VALUE20</b></td> ' ||
              '    <td width="59%">/' || get_zavotdfio(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID)) || '/</td> ' ||
              ' </tr> ' ||
      ...
      begin
        ...
        sTmp := REPLACE(sTmp,'##VALUE20','"'||NVL(GET_VRACHFCOTDEL(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID))||'"',' '));
    ...
    ELSE
      sTmp := '<P><HR><table width="100%" border="0" cellspacing="0" cellpadding="0"> ' ||
              ...
              '    <td width="59%">/' ||
              do_vrachfio(PKG_REGIST_PACFUNC.GET_PAC_VRACHID(pFK_PACID)) || '/</td>' ||
              '  </tr> ' ||
    ...
    ELSE
      FOR n in 1..arPacList.COUNT LOOP
        ...
        FOR i IN cAnamBol(arPacList(n)) LOOP
          if NOT a then
            cTemp := '<p><h3 align = "center">Анамнез болезни</h3></P>';
 ...
 FUNCTION getStatNazIbClob(pFK_PACID IN NUMBER, bEp IN NUMBER) RETURN CLOB IS
  ...
  BEGIN
    ...
    FOR p in cIB LOOP
      begin
        DBMS_LOB.CREATETEMPORARY(clTemp, TRUE, DBMS_LOB.call);
        EXECUTE IMMEDIATE 'DECLARE ' || 'BEGIN ' || ' :clTemp:=PKG_TEXTIB.' || p.FC_FUNC || '(:pFK_PACID); ' || 'END;'
  ...
  FUNCTION getFullIbClobID_by_SET(pFC_PACID IN VARCHAR2,pFN_MODE IN NUMBER DEFAULT 0,pFN_TYPE IN NUMBER DEFAULT 1,pFN_CALLTYPE IN NUMBER DEFAULT 1) RETURN NUMBER
   IS
    --TYPE TPacList IS TABLE OF BINARY_INTEGER INDEX BY BINARY_INTEGER;
    --arPacList TPacList;
    CURSOR cIB is
  ...
  BEGIN
    if pFN_CALLTYPE = 1 then --если обыкновенная Амбулаторная ИБ
      ...
      RETURN PKG_TEXTIB.getFullIbClobID_by_SET2(pFC_PACID,pFN_MODE,pFN_TYPE,pFN_CALLTYPE);
    end if;
  END;
  /*------------------------------------------------------------------------------------------------------------------*\
  |               getFullIbClobID_by_SET2
  \*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    if pFN_CALLTYPE = 2 then
      ...
      --RETURN -1;
      RETURN PKG_TEXTIB.getFullIbClobID_by_SET3(pFC_PACID,pFN_MODE,pFN_TYPE,pFN_CALLTYPE);
    end if;
  END;
  /*------------------------------------------------------------------------------------------------------------------*\
  |               getFullIbClobID_by_SET3
  \*------------------------------------------------------------------------------------------------------------------*/
...
/*      SELECT PKG_REGIST_PACFUNC.GET_PAC_IB(pFK_PACID) FC_IB,
        ...
        FROM TPEOPLES, TGROUP, TPRIZN,
             (SELECT FK_GROUPID, FK_PRIZN, FC_PUT, FK_PEPLID FROM TKARTA WHERE FK_ID = pFK_PACID
              ...
              UNION
              SELECT TO_NUMBER(NULL) FK_GROUPID, TO_NUMBER(NULL) FK_PRIZN, TO_CHAR(NULL) FC_PUT, FK_PACID FK_PEPLID FROM TDOC WHERE FK_ID = pFK_PACID
              ) T
       WHERE TPEOPLES.FK_ID(+) = T.FK_PEPLID
      ...
      SELECT PKG_REGIST_PACFUNC.GET_PAC_IB(pFK_PACID) FC_IB,
             ...
             --TGROUP.FC_NAME||'. Пол: '||LOWER(PKG_REGIST_PEPLFUNC.GET_PEPL_SEX(TPEOPLES.FK_ID)) FC_GROUP_SEX,
             PKG_REGIST_PACFUNC.GET_PAC_STATUS(pFK_PACID)
        FROM TPEOPLES, /*TGROUP, */TPRIZN,
             (SELECT /*FK_GROUPID, */FK_PRIZN, FC_PUT, FK_PEPLID FROM TKARTA WHERE FK_ID = pFK_PACID
              UNION
              SELECT /*FK_GROUPID, */FK_PRIZN, FC_PUT, FK_PEPLID FROM TAMBULANCE WHERE FK_ID = pFK_PACID) T
       WHERE TPEOPLES.FK_ID(+) = T.FK_PEPLID
         --AND TGROUP.FK_ID(+) = T.FK_GROUPID
         AND TPRIZN.FK_ID(+) = T.FK_PRIZN
      UNION
      SELECT TO_CHAR(NULL) FC_IB,
             GET_OTDNAME(GET_VRACHOTD(TDOC.FK_VRACHID)) FC_OTDEL,
             INITCAP(GET_SOTRNAME(TDOC.FK_VRACHID)) FC_VRACH,
             TDOC.FK_VRACHID FK_VRACHID,
             TO_CHAR(NULL) FC_PALATA,
             TPEOPLES.FC_FAM, TPEOPLES.FC_IM, TPEOPLES.FC_OTCH, TO_CHAR(NULL) FC_PUT,
             PKG_REGIST_PEPLFUNC.GET_PEPL_ADRFULL(TPEOPLES.FK_ID) FC_ADR,
             PKG_REGIST_PEPLFUNC.GET_PEPL_WORKPLACE(TPEOPLES.FK_ID) FC_RABOTA,
             TDOC.FD_DATE FD_IN,
             TDOC.FD_ENDDATE FD_OUT,
             TDOC.FD_ENDDATE-TDOC.FD_DATE+1 FN_KDN,
             PKG_REGIST_PEPLFUNC.GET_PEPL_AGE_MONTH(TPEOPLES.FK_ID) FC_AGE,
             TO_CHAR(NULL) FC_PRIZN,
             'Пол: '||LOWER(PKG_REGIST_PEPLFUNC.GET_PEPL_SEX(TPEOPLES.FK_ID)) FC_GROUP_SEX,
             --TGROUP.FC_NAME||'. Пол: '||LOWER(PKG_REGIST_PEPLFUNC.GET_PEPL_SEX(TPEOPLES.FK_ID)) FC_GROUP_SEX,
             GET_NAZSOS(TDOC.FN_STATUS) FC_STATUS
        FROM TPEOPLES, /*TGROUP, */TDOC
       WHERE TDOC.FK_ID = pFK_PACID
         AND TPEOPLES.FK_ID(+) = TDOC.FK_PACID/*
  ...
  FUNCTION getIbHeaderMulti(pFC_PACID IN VARCHAR2) RETURN VARCHAR2 IS
    s VARCHAR2(32767) := '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">'||
         ...
         ' <td width=21%><B>##VALUE07</B></td>' ||
         ' <td width=4%>&nbsp;</td>' ||
         ' <td width=47%>Обращений стационарно: <B>##VALUE08</B> (Проведено: <B>##VALUE12 к.дн.</B>) </td>' ||
         '</tr><tr>' ||
         ' <td width=28%>Отчество:</td>' ||
         ' <td width=21%><B>##VALUE09</B></td>' ||
         ' <td width=4%>&nbsp;</td>' ||
         ' <td width=47%>амбулаторно: <B>##VALUE10</B></td>' ||
         '</tr><tr>' ||
         ' <td width=28%>Возраст:</td>' ||
         ' <td width=21%><B>##VALUE11</B></td>' ||
         ' <td width=4%>&nbsp;</td>' ||
         ' <td width=47%>мероприятий проведено : <B>##VALUE17</B></td>' ||
         '</tr><tr>' ||
         ' <td width=28%>Место работы:</td>' ||
    ...
    CURSOR cCount(pFK_PEPLID IN NUMBER) IS
      SELECT FN_SKOL+FN_AKOL+FN_DOCS KOL_SUM, FN_SKDN+FN_AKDN KDN_SUM, T1.*,T2.*,T3.* FROM
      ...
      (SELECT COUNT(FK_ID) FN_DOCS, 0 FN_DKDN
        FROM TDOC
       WHERE FK_PACID = pFK_PEPLID) T3;
    CURSOR cIb(pFK_PACID IN NUMBER) IS
      ...
      SELECT PKG_REGIST_PACFUNC.GET_PAC_IB(pFK_PACID) FC_IB,
       ...
       UNION
      SELECT '&nbsp;' FC_IB,
             GET_SOTRNAME(TDOC.FK_VRACHID) FC_VRACH,
             TDOC.FD_DATECREATE FD_IN,
             NVL(GET_SMIDNAME(TDOC.FK_DOCID),'Обращение') FC_VID
        FROM TDOC
       WHERE FK_ID = pFK_PACID;
    --TYPE TPacList IS TABLE OF BINARY_INTEGER INDEX BY BINARY_INTEGER;
    --arPacList TPacList;
  ...
  BEGIN
    ...
    cTemp := --'<div align=right><U>Обращения в медучреждения:</U></div>'||
             ...
             '  <td style=''text-align:center;border-bottom:solid windowtext .5pt;border-right:solid windowtext .5pt''>Дата:</td>'||
             '  <td style=''text-align:center;border-bottom:solid windowtext .5pt;border-right:solid windowtext .5pt''>Обращение:</td>'||
    ...
    FOR i in 1..arPacList.COUNT LOOP
      ...
      cTemp := cTemp||'<tr>'||
             ...
             '  <td style=''text-align:right;border-right:solid windowtext .5pt''>'||NVL(TO_CHAR(p.FD_IN,'dd.mm.yyyy'),'&nbsp;')||'</td>'||
             '  <td style=''text-align:right;border-right:solid windowtext .5pt''>'||p.FC_VID||'</td>'||
  ...
  BEGIN
    if TO_NUMBER(pFC_PACID) > 0 then
      DBMS_LOB.CREATETEMPORARY(clTemp, TRUE, DBMS_LOB.call);
      SELECT VNAZ.FK_PACID into cTemp FROM VNAZ WHERE VNAZ.FK_ID = TO_NUMBER(pFC_PACID);
  ...
  FUNCTION getIbHeader3(pFK_PEPLID IN NUMBER) RETURN VARCHAR2 IS
    s VARCHAR2(32767) := '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">'||CHR(13)||
         ...
         '<head><meta http-equiv=Content-Type content="text/html; charset=windows-1251">'||CHR(13)||
         '<meta name=ProgId content=Word.Document>'||CHR(13)||
         '<title>МИС ПАЦИЕНТ - МЕДИЦИНСКАЯ КАРТА АМБУЛАТОРНОГО БОЛЬНОГО ##VALUE04 ##VALUE07 ##VALUE09</title></head>'||CHR(13)||
         '<body><table width=100% border=0>' ||CHR(13)||
         '<tr>' ||CHR(13)||
         ' <td colspan=4><div align=right><font style=''font-size:8.0pt''>форма № 025/у-87<BR>Утв. МЗ СССР 31.12.87г. №1338</font></div></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td colspan=4><table width=100% border=0>' ||CHR(13)||
         ' <tr>' ||CHR(13)||
         --'  <td width=90><img src=c://asu/000.jpg></img></td>' ||
         '  <td><div align=center><font style=''color:navy''><b>Министерство здравоохранения и социального развития Российской Федерации</b></font><BR><BR>' ||CHR(13)||
         '  <font style=''font-size:14.0pt;color:red''><b>НАИМЕНОВАНИЕ ЛПУ</b></font></div></td>' ||CHR(13)||
         ' </tr></table>' ||CHR(13)||
         ' </td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td colspan=4><p align=center><font style=''font-size:18.0pt''><BR><b>МЕДИЦИНСКАЯ КАРТА АМБУЛАТОРНОГО БОЛЬНОГО</b></font></p></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>&nbsp;</td>' ||CHR(13)||
         ' <td width=21%>&nbsp;</td>' ||CHR(13)||
         ' <td width=4%>&nbsp;</td>' ||CHR(13)||
         ' <td width=47%><b><i>##VALUE03</i></b></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Фамилия:</td>' ||CHR(13)||
         ' <td width=21%><B>##VALUE04</B></td>' ||CHR(13)||
         ' <td width=4%>&nbsp;</td>' ||CHR(13)||
         ' <td width=47%>&nbsp;</B></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Имя:</td>' ||CHR(13)||
         ' <td width=21%><B>##VALUE07</B></td>' ||CHR(13)||
         ' <td width=4%>&nbsp;</td>' ||CHR(13)||
         ' <td width=47%>&nbsp;</td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Отчество:</td>' ||CHR(13)||
         ' <td width=21%><B>##VALUE09</B></td>' ||CHR(13)||
         ' <td width=4%>&nbsp;</td>' ||CHR(13)||
         ' <td width=47%>&nbsp;</B></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Дата рождения:</td>' ||CHR(13)||
         ' <td width=21%><B>##VALUE11</B></td>' ||CHR(13)||
         ' <td width=4%>&nbsp;</td>' ||CHR(13)||
         ' <td width=47%>&nbsp;</td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Место работы (учебы) и должность:</td>' ||CHR(13)||
         ' <td colspan=3><B>##VALUE13</B></td>' ||CHR(13)||
         '</tr><tr>' ||CHR(13)||
         ' <td width=28%>Домашний адрес, телефон:</td>' ||CHR(13)||
         ' <td colspan=3><B>##VALUE16</B></td>' ||CHR(13)||
         '</tr></table>'||CHR(13);
    CURSOR cMain IS
      SELECT TPEOPLES.FC_FAM, TPEOPLES.FC_IM, TPEOPLES.FC_OTCH,
...
/*------------------------------------------------------------------------------------------------------------------*/
  FUNCTION SET_getIbEnder(pFC_PACID IN VARCHAR2,pFN_MODE IN NUMBER DEFAULT 0,pFN_TYPE IN NUMBER DEFAULT 1) RETURN CLOB IS
    clTemp CLOB;
    cTemp  VARCHAR2(32767):= '</BODY></HTML>'||CHR(13);
  BEGIN
    DBMS_LOB.CREATETEMPORARY(clTemp, TRUE, DBMS_LOB.call);
    DBMS_LOB.WRITEAPPEND(clTemp, LENGTH(cTemp), cTemp);
    RETURN clTemp;
...
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    IF pFN_MODE = 1 THEN
      --DBMS_LOB.APPEND(clTemp, SET_GET_RAZD_BREAK(pFK_PACID));
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(NULL, get_galobid, 'Жалобы', ''));
    ELSE
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(pFK_PACID, get_galobid, 'Жалобы', ''));
    END IF;
    RETURN clTemp;
  END;
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    IF pFN_MODE = 1 THEN
      --DBMS_LOB.APPEND(clTemp, SET_GET_RAZD_BREAK(pFK_PACID));
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(NULL, get_anamgiznid, 'Анамнез жизни', ''));
    ELSE
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(pFK_PACID, get_anamgiznid, 'Анамнез жизни', ''));
    END IF;
    RETURN clTemp;
  END;
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    IF pFN_MODE = 1 THEN
      --DBMS_LOB.APPEND(clTemp, SET_GET_RAZD_BREAK(pFK_PACID));
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(NULL, get_epidid, 'Эпиданамнез', ''));
    ELSE
      DBMS_LOB.APPEND(clTemp, getRazdelIbText(pFK_PACID, get_epidid, 'Эпиданамнез', ''));
    END IF;
    RETURN clTemp;
  END;
/*------------------------------------------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    IF pFN_MODE = 1 THEN
      --DBMS_LOB.APPEND(clTemp, SET_GET_RAZD_BREAK(pFK_PACID));
      cTmp := TRIM(do_diagbytype(/*pFK_PACID, */6, 1, -1, 1));
      if cTmp is not null then
        cTemp := '<p><b>Предварительный диагноз:</b><br><u>Основной:</u> ' || cTmp;
      end if;
    ELSE
      cTmp := TRIM(asu.do_diagbytype(pFK_PACID, 6, 1, -1, 1));
      if cTmp is not null then
        cTemp := '<p><b>Предварительный диагноз:</b><br><u>Основной:</u> ' || cTmp;
      end if;
    END IF;
...
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    IF pFN_MODE = 1 THEN
      --DBMS_LOB.APPEND(clTemp, SET_GET_RAZD_BREAK(pFK_PACID));
      cTmp := TRIM(do_diagbytype(/*pFK_PACID, */6, 0, -1, 1));
      if cTmp is not null then
        cTemp := '<br><u>Сопутствующий:</u> ' || cTmp;
      end if;
    ELSE
      cTmp := TRIM(asu.do_diagbytype(pFK_PACID, 6, 0, -1, 1));
      if cTmp is not null then
        cTemp := '<br><u>Сопутствующий:</u> ' || cTmp;
      end if;
    END IF;
...
--    IF pFN_MODE = 1 THEN
   ...
   DBMS_LOB.CREATETEMPORARY(clTemp, TRUE, DBMS_LOB.call);
   DBMS_LOB.CREATETEMPORARY(ep, TRUE, DBMS_LOB.call);
      FOR cEP IN (SELECT * FROM tepikriz WHERE fp_type IN (1,2,3) AND fk_pacid=pfk_pacid ) LOOP
       SELECT fc_clob INTO ep FROM tclobs WHERE fk_id=cEP.fk_epclobid;
       DBMS_LOB.writeappend(clTEmp, LENGTH('<br>'), '<br>');
       ep:=CLOB_REPLACE (ep,'<HTML>',' ');
       ep:=CLOB_REPLACE (ep,'</HTML>',' ');
...
/*------------------------------------------------------------------------------------------------------------------*/
  ...
  BEGIN
    ...
    SELECT get_rootid(VNAZ.FK_SMID) into nRootID from VNAZ WHERE FK_ID = pFK_NAZID;
      ...
      FOR i IN cKonsIssl(pFK_NAZID) LOOP
        if i.sos = get_neyav then
          ...
          FOR j IN cFiles(i.fk_nazid) LOOP
            ...
            DBMS_LOB.writeappend(cl, LENGTH(sTmp), sTmp);
          END LOOP;
          /*if nRootID = get_konsid then
            sTmp:='<br>Консультация проведена: '||INITCAP(do_vrachfio(i.ispol))||' _____________________';
          elsif nRootID =  get_issledid then
            sTmp:='<br>Исследование проведено: '||INITCAP(do_vrachfio(i.ispol))||' _____________________';
          else*/
            sTmp:='<br>Специалист: '||INITCAP(do_vrachfio(i.ispol))/*||' _____________________'*/;
          --end if;
