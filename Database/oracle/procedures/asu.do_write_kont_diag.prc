DROP PROCEDURE ASU.DO_WRITE_KONT_DIAG
/

--
-- DO_WRITE_KONT_DIAG  (Procedure) 
--
--  Dependencies: 
--   STANDARD (Package)
--   DUAL (Synonym)
...
--
CREATE OR REPLACE PROCEDURE ASU."DO_WRITE_KONT_DIAG" -- Modified by Marriage
 (
  pFK_PACID in NUMBER,
  ...
  pFC_DIAG in varchar2
   ...
   CURSOR DoesKontingentEx IS
                              SELECT /*+first_row*/COUNT(FK_ID)
                               FROM TIB
                              WHERE
                               FK_PACID = pFK_PACID--( Select FK_PEPLID from TKARTA where FK_ID = pFK_PACID)
                                and
