DROP VIEW ASU.WEB_SCHEDULE_SCHEMA
/

/* Formatted on 06.04.2016 15:19:30 (QP5 v5.115.810.9015) */
--
-- WEB_SCHEDULE_SCHEMA  (View)
--
--  Dependencies:
...
CREATE OR REPLACE FORCE VIEW ASU.WEB_SCHEDULE_SCHEMA
(
   ENDON,
   LPU,
   STARTON,
   ...
   SELECT   DECODE (
     ...
     FROM   ( /*SELECT TRUNC(G.FD_DATE) + NUMTODSINTERVAL(G.FT_HOUR, 'HOUR') +
             ...
             SELECT     TRUNC (G.FD_DATE)
                      ...
                      FK_NAZVRACHID NAZID,
                      CASE
                         WHEN (R.FL_SHOW_FLAG = 1) AND (G.FL_SHOW = 1) THEN 1
                         ELSE 0
                      END
                         AS SHOWONSITE,
                      R.FL_SHOW_FLAG AS NEED_TRANSFER
               FROM   ASU.TRASPIS_SCHEMA R,
                      ASU.TNAZVRACH N,
                      ASU.TGRAPH G,
                      --I?eoeinu aiaaaeou oaeia ia?aie?aiea, o.e. caiaiee R.FK_NAZVRACHID = N.FK_ID ia R.FK_NAZVRACHID = N.FK_SOTRKABID
