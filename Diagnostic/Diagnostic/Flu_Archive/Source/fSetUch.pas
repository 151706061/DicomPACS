...
begin
  ...
  try
    ...
    Reg.OpenKey('\Software\Softmaster\Flu_Archive\ID_SOTR'+IntToStr(frmMain.pSOTRID), True);
    lcbType.EditValue := Reg.ReadInteger('\Software\Softmaster\Flu_Archive\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', 0);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey:=HKEY_CURRENT_USER;
    if not Reg.OpenKey('\Software\Softmaster\Flu_Archive\', FALSE) then
      Reg.CreateKey('\Software\Softmaster\Flu_Archive');
    try
      Reg.WriteInteger('\Software\Softmaster\Flu_Archive\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', lcbType.EditValue);
    except
    end;
  finally
    Reg.Free;
