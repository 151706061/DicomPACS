...
type
  TfrmMain = class(TForm)
    ...
    dxMemData1FN_ORDER: TIntegerField;
...
//    if idTag <> 0 then
//    begin
//      frmSetParam.cxName.Tag := idTag;
//      frmSetParam.cxName.Text := lbName.Caption;
//      frmSetParam.cxYear.Value := nYear;
//    end else
//    begin
//      frmSetParam.cxYear.Value := StrToInt(sSysDate);
//    end;
...
//      dxMemData1.Close;
//      dxMemData1.Open;
//      idTag := frmSetParam.cxName.Tag;
//      nYear := frmSetParam.cxYear.Value;
//      lbName.Caption := frmSetParam.cxName.Text;
//      lbYear.Caption := IntToStr(frmSetParam.cxYear.Value);
//      paParam.Visible := True;
//      paSubParam.Visible := True;
//      cxSplitter1.Visible := True;
//
...
//      oqThread.Scrollable := True;
//      Animator1.Animate := True;
//      oqThread.SetVariable('PFK_OWNER', idTag);
//      oqThread.SetVariable('PDATE1', frmSetParam.datebeg);
//      oqThread.SetVariable('PDATE2', frmSetParam.dateend);
//      oqThread.Execute;
//    end;
//  frmSetParam.Free;
end;
...
begin
  ...
  try
    ...
    frmMain.Caption := 'ПЛАН ПО ПРЕДПРИЯТИЯМ - ОБЩИЙ('+Ver.FileVersion+'). Пользователь: '+sVrachFIO;
  ...
  TV.StoreToRegistry('\SoftWare\SoftMaster\Fluoro_company\fMain\TV'+'Vrach'+ '_' + IntToStr(pSOTRID)+'Default', TRUE, [], 'TV');
  TV.RestoreFromRegistry('\SoftWare\SoftMaster\Fluoro_company\fMain\TV'+'Vrach'+ '_' + IntToStr(pSOTRID), FALSE, FALSE, [], 'TV');
end;
...
begin
  TV.StoreToRegistry('\SoftWare\SoftMaster\Fluoro_company\fMain\TVVrach_' + IntToStr(pSOTRID), TRUE, [], 'TV');
end;
...
begin
  TV.RestoreFromRegistry('\SoftWare\SoftMaster\Fluoro_company\fMain\TV'+'Vrach'+ '_' + IntToStr(pSOTRID)+'Default', FALSE, FALSE, [], 'TV');
end;
...
begin
  ...
  begin
    try
      ...
      dxMemData1.FieldByName('FN_ORDER').AsString := Sender.Field('FN_ORDER');
      dxMemData1.Post;
      dxMemData1.Next;
    finally
    end;
