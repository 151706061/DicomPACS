...
uses
  ...
  cxGridDBTableView, cxGrid, ActnList, JvDBUtils, Oracle, frxClass, frxDBSet, dxSkinBlack, dxSkinBlue, dxSkinCaramel,
...
implementation
uses fMain, fChooseNaz, fChooseAnal;
{$R *.dfm}

procedure TfrmMetodics.aAddExecute(Sender: TObject);
var oq : TOracleQuery;
begin
  if pSMIDID = frmMain.get_analid then
    begin
      Application.CreateForm(TfrmChooseAnal, frmChooseAnal);
//      frmChooseAnal.DoShowForm(pSMIDID);
      frmChooseAnal.ShowModal;
      if frmChooseAnal.ModalResult = MrOk then
        begin
          ...
          try
            ...
            frmChooseAnal.odsNazCheck.First;
            while not frmChooseAnal.odsNazCheck.Eof do
              begin
                if frmChooseAnal.odsNazCheck.FieldByName('FL_CHECK').AsInteger = 1 then
                  begin
                    oq.DeleteVariables;
                    oq.SQL.Text := ' INSERT INTO asu.TS_COLUMNNAZ(FC_NAME, FK_OWNER, FK_SMID) '+
                                   ' VALUES(:pFC_NAME, (SELECT FK_ID FROM asu.TS_COLUMNNAZ WHERE FC_SYNONIM = :PFC_SYNONIM), :pFK_SMID)';
                    oq.DeclareAndSet('pFC_NAME', otString, frmChooseAnal.odsNazCheck.FieldByName('FC_NAME').AsString);
                    oq.DeclareAndSet('PFC_SYNONIM', otString, sSYNONIM);
                    oq.DeclareAndSet('pFK_SMID', otInteger, frmChooseAnal.odsNazCheck.FieldByName('FK_ID').AsInteger);
                    oq.Execute;
                    frmMain.os.Commit;
                  end;
                frmChooseAnal.odsNazCheck.Next;
              end;
...
begin
  ...
  if pColumn = '' then
    ...
    frmMetodics.Caption := 'Методики. Таблица: '+sTableName+'; Строка: '+pRow+', Столбец: '+pColumn;
  if (sTableName = '5300') or (sTableName = '5302') or (sTableName = '5303') then
    pSMIDID := frmMain.get_analid
  else
    pSMIDID := frmMain.get_issledid;
end;

procedure TfrmMetodics.frxRepMetodicsGetValue(const VarName: string; var Value: Variant);
begin
