...
type
  TfrmChooseNaz = class(TForm)
    ...
    aSet: TAction;
    aUnSet: TAction;
    odsNazCheckFC_UPPER: TStringField;
...
begin
  cxTreeNaz.BeginUpdate;
  try
     Cursor := crSQLWait;
     odsNazCheck.First;
     while not odsNazCheck.Eof do begin
        if odsNazCheck.FieldByName('FL_CHECK').AsInteger <> 1 then begin
           odsNazCheck.Edit;
...
begin
  ...
  try
     ...
     odsNazCheck.First;
     while not odsNazCheck.Eof do begin
        if odsNazCheck.FieldByName('FL_CHECK').AsInteger <> 0 then begin
           odsNazCheck.Edit;
...
begin
  ...
  odsNazCheck.Filter := 'FC_UPPER = ''*' + AnsiUpperCase(cxFilter.Text) + '*'' '; //!!! сделать так чтобы фильтр работал в независимости от регистра
end;
...
begin
  ...
  if (frmMetodics.pSMIDID = frmMain.get_analid) then begin
     Self.odsNazCheck.SQL.Text := msh.StringsByName['table_anal'].Text;
  end else begin
     Self.odsNazCheck.SQL.Text := msh.StringsByName['table_issled'].Text;
  end;
  ...
  while not frmMetodics.odsMetodics.EOF do begin
     arrId[i]:=frmMetodics.odsMetodics.FieldByName('FK_SMID').AsInteger;
     frmMetodics.odsMetodics.Next;
     i:=i+1;
  end;

  for i:=0 to odsNazCheck.RecordCount-1 do begin
     k:=odsNazCheck.FieldByName('FK_ID').AsInteger;

     for j:=0 to odsNazCheck.RecordCount-1 do begin
        if k = arrID[j] then begin
           odsNazCheck.Edit;
           odsNazCheck.FieldByName('FL_CHECK').AsInteger:=1;
        end;
     end;
  ...
  cxTreeNaz.GotoBOF;
  if cxTreeNaz.FocusedNode.HasChildren then
     cxTreeNaz.FocusedNode.Expand(FALSE);
...
procedure TfrmChooseNaz.FormCreate(Sender: TObject);
begin
  russian:=LoadKeyboardLayout('00000419', 0);

//  if (not odsNazCheck.Active) then
//     odsNazCheck.Active := True;
end;
