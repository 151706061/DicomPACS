  ...
  if frmChooseNaz.ModalResult = MrOk then
    begin
      ...
      try
        ...
        oq.Cursor := crSQLWait;
        oq.SQL.Text := ' DELETE FROM ASU.TS_COLUMNNAZ WHERE FK_OWNER = (SELECT FK_ID FROM TS_COLUMNNAZ WHERE FC_SYNONIM = :PFC_SYNONIM) ';
            ...
            if frmChooseNaz.odsNazCheck.FieldByName('FL_CHECK').AsInteger = 1 then
              begin
                ...
                oq.SQL.Text := ' INSERT INTO ASU.TS_COLUMNNAZ(FC_NAME, FK_OWNER, FK_SMID) '+
                               ' VALUES(:pFC_NAME, (SELECT FK_ID FROM TS_COLUMNNAZ WHERE FC_SYNONIM = :PFC_SYNONIM), :pFK_SMID)';
