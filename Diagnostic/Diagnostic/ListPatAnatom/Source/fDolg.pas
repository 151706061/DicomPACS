...
uses
  ...
  dxSkinscxPCPainter, dxSkinsdxBarPainter, jvDBUtils, dxSkinOffice2007Green;
...
//------------------------------------------------------------------------------
  ...
  Reg.OpenKey('\Software\Softmaster\List_diagnost\fDolg\ID_SOTR'+IntToStr(frmMain.pSOTRID), True);
//  id := Reg.ReadInteger('\Software\Softmaster\List_diagnost\fDolg\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_KABINET_DOLG', '');
  cxlcRoom.EditValue := Reg.ReadInteger('\Software\Softmaster\List_diagnost\fDolg\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_KABINET_DOLG', 0);
  Reg.Free;
//------------------------------------------------------------------------------
...
begin
  ...
  ods.SQL.Text := ' SELECT GET_NAZCANCEL, TRUNC(SYSDATE) AS SYSTEMDATE FROM DUAL ';
  ods.Close;
  ods.Open;
...
begin
  Reg := TRegIniFile.Create;
  Reg.RootKey:=HKEY_CURRENT_USER;
  if not Reg.OpenKey('\Software\Softmaster\List_diagnost\fDolg\', FALSE) then
   begin
     Reg.CreateKey('\Software\Softmaster\List_diagnost\fDolg');
   end;
  try
    Reg.WriteInteger('\Software\Softmaster\List_diagnost\fDolg'+'\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_KABINET_DOLG', cxlcRoom.EditValue);
  except
  end;
  Reg.Free;
