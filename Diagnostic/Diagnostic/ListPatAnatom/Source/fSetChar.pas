...
uses
  ...
  dxSkinOffice2007Green, ExtCtrls, cxTextEdit, cxContainer, cxLabel;
...
procedure TfrmSetChar.aClearExecute(Sender: TObject);
begin
  teFilter.Text := '';
  ods.Filtered := False;
  aOK.Enabled := True;
...
begin
  ...
  if length(teFilter.Text) > 0 then
    begin
      ods.Filter := 'lowername = ''*' + teFilter.Text + '*'' ';
      ods.Filtered := True;
      aClear.Enabled := True;
    end else
    begin
...
procedure TfrmSetChar.TVCHARKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
