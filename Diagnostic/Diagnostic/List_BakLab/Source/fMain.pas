...
begin
  try
    if RestoreApplication(dsList.DataSet.FieldByName('FIOPAC_SHORT').AsString) then Exit;
...
//------------------------------------------------------------------------------
  ...
  frmMain.Caption := '';
  frmMain.Caption := 'ЖУРНАЛ ВРАЧА-ПАТАЛОГОАНАТОМА('+Ver.FileVersion+'). Пользователь: '+sSotrFIO+'; КАБИНЕТ: '+odsKab.FieldByName('FC_NAME').AsString;
  cxGr.SetFocus;
