...
uses
  ...
  Dialogs, ExtCtrls, OracleData, ImgList, dxBar, ActnList, Jpeg;
...
begin
  ...
  if not DirectoryExists(HTMLImagesPath) then
    CreateDir(HTMLImagesPath);
  DataSetImg.First;
  aPrior.Enabled := FALSE;
  try
    BmpToJPEG(DataSetImg.FieldByName('SFILENAME').AsString, DataSetImg.FieldByName('SFILENAME').AsString);
    Image1.Picture.LoadFromFile(HTMLImagesPath+DataSetImg.FieldByName('SFILENAME').AsString);
  except
...
var SR:  TSearchRec;
begin
  FindFirst(HTMLImagesPath+'*.JPG', faAnyFile, SR);
  repeat
    DeleteFile(HTMLImagesPath+SR.Name);
  until FindNext(SR) <> 0;
end;
...
var
  ...
  lStream: TFileStream;
begin
  lStream:=TFileStream.Create(HTMLImagesPath+pFNAME2,fmOpenRead);
  try
    if PhysicalResolveFileType(lStream)=1 then
      begin
        ...
        try
          Bitmap.LoadFromFile(HTMLImagesPath+pFNAME2);
          JPEG.Assign(Bitmap);
          Image1.Picture.Assign(JPEG);
          Image1.Picture.SaveToFile(HTMLImagesPath+pFNAME1);
        finally
          Image1.free;
          JPEG.Free;
          Bitmap.Free;
