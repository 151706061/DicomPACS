...
uses
  ...
  cxVGrid, cxInplaceContainer, JvComponentBase, JvFormPlacement, cxButtonEdit,
  cxImageComboBox; 
...
type
  ...
  TInitDLLWizzard = procedure(Panel_HND, App_HND, Form_HND: THandle; pPacID, pSotrID, pAppSotrID, pOsmotrID, pRootID:integer); stdcall;
  ...
  TfrmSearch = class(TForm)
    ...
    bbSnimok: TdxBarButton;
    aSnimok: TAction;
    odsNazFK_RESULTID: TFloatField;
    VISSLCNTIMAGE: TcxGridDBColumn;
...
uses fMain, fPacInfoDiagnost, fWait, fEditNaz, fOtmena, fSysInfo, uCommonXray;
...
begin
  ...
  rowNapr.Visible := frmMain.get_dbname <> 'SK'; // для Сургут Кардио не показываем графу "Направлен"   
end;
...
procedure TfrmSearch.frxRepProsmotr_SOKBGetValue(const VarName: string; var Value: Variant);
begin
  if VarName = 'PACAGE' then Value := sPacAge_4rep;
  if VarName = 'PACFIO' then Value := odsPac.FieldByName('FIO').AsString;
  if VarName = 'RUNDATE' then Value := odsNaz.FieldByName('FD_RUN').AsString;
...
procedure TfrmSearch.frxRepProsmotr_TravmaGetValue(const VarName: string; var Value: Variant);
begin
  if VarName = 'COMPANYNAME' then Value := frmMain.sCompanyName_4Rep;
  if VarName = 'OTDELNAME' then Value := frmMain.sOtdelName_4Rep;
  if VarName = 'PACAGE' then Value := sPacAge_4rep;
...
procedure TfrmSearch.MenuItem7Click(Sender: TObject);
begin
  TVISSL.RestoreFromRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacISSL_' + IntToStr(frmMain.pSOTRID)+'Default', FALSE, FALSE, [], 'tvIssl');
end;
          ...
          if trim(StrPas(buff)) = trim(frmMain.arFIO[i-iDel]) then
            begin
              ...
              begin
                  ...
                  begin
                    frmMain.arFIO[j]:=trim(frmMain.arFio[j+1]);
                  end;
       ...
       begin
         odsPac.Close;
			   odsPac.SQL.Text := ' select tkarta.fk_peplid as fk_id, '+ #13 +
                            ...
                            '        GREATEST(TKARTA.FL_VYB, TKARTA.FL_VYB2) ARCHIVED '+ #13+
...
procedure TfrmSearch.aSnimokExecute(Sender: TObject);
var Reg : TRegistry;
    ods : TOracleDataSet;
begin
      ...
      begin
        ...
        try
          ...
          DoShowSnimki(odsPac.FieldByName('nib').AsString,
                       '',
                       ods.FieldByName('fc_guid').AsString,
...
begin
  ...
  try
    ...
    oq.SQL.Text := ' DECLARE '+#13+
                   ...
                   '      WHERE FK_ID = ID23; '+#13+
                   '   ELSE '+#13+
                   '     INSERT INTO TIB(FK_PACID, FK_SMID, FK_SMEDITID, FC_CHAR, FK_VRACHID) '+#13+
...
procedure TfrmSearch.rProt_TOGetValue(const VarName: string; var Value: Variant);
begin
  if VarName = 'COMPANYNAME' then Value := frmMain.sCompanyName_4Rep;
  if VarName = 'OTDELNAME' then Value := frmMain.sOtdelName_4Rep;
  if VarName = 'PACAGE' then Value := sPacAge_4rep;
...
//        begin
//          ShowPacInfo(Application.Handle, -1, True, frmMain.pSOTRID); // -1 - ???; 2й параметр код пациента, 3й - разрешение редактирования, 4й - код сотрудника
//        end;
//        FreeLibrary(hHandle);
//    end;
end;
...
//---
  ...
  try
    Reg.RootKey:=HKEY_CURRENT_USER;
    Reg.OpenKey('\Software\Softmaster\List_diagnost\fSearch\ID_SOTR'+IntToStr(frmMain.pSOTRID), True);
    ...
    TVPAC.StoreToRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacVrach_' + IntToStr(frmMain.pSOTRID)+'Default', TRUE, [], 'tvPac');
    TVPAC.RestoreFromRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacVrach_' + IntToStr(frmMain.pSOTRID), FALSE, FALSE, [], 'tvPac');
    TVISSL.StoreToRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacISSL_' + IntToStr(frmMain.pSOTRID)+'Default', TRUE, [], 'tvIssl');
    TVISSL.RestoreFromRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacISSL_' + IntToStr(frmMain.pSOTRID), FALSE, FALSE, [], 'tvIssl');
  finally
    Reg.Free;
  end;
end;
...
begin
  if odsPac.Active then
    begin
      ...
      actNaz.Enabled := (odsPac.RecordCount > 0) and (odsPac.FieldByName('ARCHIVED').AsInteger = 0);
    end;
...
//------------------------------------------------------------------------------
  ...
  TVPAC.StoreToRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacVrach_' + IntToStr(frmMain.pSOTRID), TRUE, [], 'tvPac');
  TVISSL.storetoRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacISSL_' + IntToStr(frmMain.pSOTRID), true, [], 'tvIssl');
end;

procedure TfrmSearch.N2Click(Sender: TObject);
begin
  TVPAC.RestoreFromRegistry('\SoftWare\SoftMaster\List_diagnost\tvPacVrach_' + IntToStr(frmMain.pSOTRID)+'Default', FALSE, FALSE, [], 'tvPac');
end;
...
begin
  ...
  if frmOtmena.ModalResult = MrOK then
    begin
      ...
      oq.Session := frmMain.os;
      oq.Cursor := crSQLWait;
      // ??? нужно ли здесь инсертить запись в TPRIEMNAZ
      oq.SQL.Text := ' UPDATE TNAZIS SET FK_ISPOLID = :pFK_ISPOLID, FK_NAZSOSID = :pFK_NAZSOSID WHERE FK_ID = :pFK_ID ';
...
var ods : TOracleDataSet;
begin
  if (frmMain.get_dbname = 'SO') or (frmMain.get_dbname = 'SK') then
    begin
      frmMain.DoResultHTML(odsNaz.FieldByName('FK_ID').AsInteger, odsNaz.FieldByName('fk_pacid').AsInteger);
    end else
    begin
      ods := TOracleDataSet.Create(nil);
      try
        ...
        ods.Close;
        ods.DeleteVariables;
        ods.SQL.Text := ' SELECT ASU.DO_VRACHFIO(TNAZIS.FK_ISPOLID) AS ISPOLFIO, '+#13+               // !!! зачем здесь делать запрос из TNAZIS
                        '        ASU.PKG_REGIST_PACFUNC.GET_PAC_AGE_NOW(TNAZIS.FK_PACID) AGE '+#13+   // если в гриде и так TNAZIS
                        '   FROM asu.TNAZIS '+#13+
                        '  WHERE FK_ID = :PFK_ID ';
        ods.DeclareAndSet('PFK_ID', otInteger, odsNaz.FieldByName('FK_ID').AsInteger);
        ods.Open;
...
// -- показывать или не показывать кнопку "снимки"
  if (frmMain.get_dbname = 'H') or (frmMain.get_dbname = 'SO') then
    begin
      if StrToInt(frmMain.get_dicom_show) = -1 then
        begin
          aSnimok.Enabled := odsNaz.FieldByName('fk_resultid').AsInteger = 1;
        end else
        begin
...
var InitDLLWizzard : TInitDLLWizzard;
    InitDLLWizzardDefIspolKab : TInitDLLWizzardDefIspolKab;
...
begin
  if bbNaz.Down then
    begin
      ...
      if ANazHandle <> 0 then
        begin
          if frmMain.get_dbname = 'M' then
            begin
              @InitDLLWizzard := GetProcAddress(ANazHandle, 'InitPlugin');
              if @InitDLLWizzard <> nil then
                InitDLLWizzard(paLoad.Handle,
                               Application.Handle,
                               Handle,
