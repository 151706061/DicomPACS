...
type
  ...
  TfrmMain = class(TForm)
    ...
    aAgreeSurgery: TAction;
    aAgreeTomoContrast: TAction;
    aAgreeMRT: TAction;
    frxAgreeReportSurgery: TfrxReport;
    frxDBDatasetAgree: TfrxDBDataset;
    frxAgreeReportTomography: TfrxReport;
    frxAgreeReportMRT: TfrxReport;
    ...
    procedure aAgreeSurgeryExecute(Sender: TObject);
    procedure aAgreeTomoContrastExecute(Sender: TObject);
    procedure aAgreeMRTExecute(Sender: TObject);
  private
    FileVersion : string;
    sSortType : String;
    DateB, DateE : TDate;
...
procedure SenfDataToWorklist (p_AccessionNumber:string;
                              p_sex : string;
                              p_pid : string;
...
uses fOtmena, fPacInfo, fSetNaz, fLabNaprav, fSearchLab, fEditNaz, uDBTableUtils,
     ...
     fCompare, fCorrectPac, fAddTalonInfo, fNaprRad, fDolgTalon, fPreview;
{$R *.dfm}
...
begin
  ...
  // ShowMessage('xray_vrach_shownumber='+inttostr(xray_vrach_shownumber));

    VNUMBERPROTOCOL.Visible:= xray_vrach_shownumber = 0; //1;
    vFC_accessionnumber.visible := xray_vrach_shownumber = 1; // 0;// GET_DB_NAME <> 'H';
  end else 
  if (xray_vrach_shownumber = 1) then
  begin
...
//showmessage('v_db_lpu = '+v_db_lpu);

  aAgreeSurgery.Visible      := (v_db_lpu = 'SO');
  aAgreeTomoContrast.Visible := (v_db_lpu = 'SO');
  aAgreeMRT.Visible          := (v_db_lpu = 'SO');
  //
end;
...
procedure TfrmMain.aAgreeMRTExecute(Sender: TObject);
begin  // Согласие на магнитно-резонансную томографию с контрастным усилением
  if odsList.Eof then Exit;
  frxAgreeReportMRT.Variables['FIO']:='''' +odsList.fieldbyname('FIOPAC').AsString+' дата рождения : '+odsList.fieldbyname('FD_ROJD').AsString + '''';
  frxAgreeReportMRT.ShowReport;
end;

procedure TfrmMain.aAgreeSurgeryExecute(Sender: TObject);
begin  // Согласие на хирургическое лечение под рентген.контролем
  if odsList.Eof then Exit;
  frxAgreeReportSurgery.Variables['FIO']:='''' +odsList.fieldbyname('FIOPAC').AsString+' дата рождения : '+odsList.fieldbyname('FD_ROJD').AsString + '''';
  frxAgreeReportSurgery.ShowReport;
end;

procedure TfrmMain.aAgreeTomoContrastExecute(Sender: TObject);
begin  // Согласие на компьютерную томографию с контрастным усилением
  if odsList.Eof then Exit;
  frxAgreeReportTomography.Variables['FIO'] := '''' +odsList.fieldbyname('FIOPAC').AsString+' дата рождения : '+DateToStr(odsList.fieldbyname('FD_ROJD').AsDateTime)+ '''';
  frxAgreeReportTomography.ShowReport;
end;
      ...
      if odsKab.FieldByName('is_flu').AsInteger = 1 then
        ...
        begin
          ...
          try
            ...
            frmLabNaprav.GET_DB_NAME := GET_DB_NAME;
...
{
INSERT INTO ASU.TSMINI (FC_SECTION, FC_KEY, FC_VALUE) VALUES('DLL_WORKLIST_NAZN','PG_DATABASE', 'test' );
...
INSERT INTO ASU.TSMINI (FC_SECTION, FC_KEY, FC_VALUE) VALUES('DLL_WORKLIST_NAZN','PG_PASSWORD', 'postgres' );
}
procedure SenfDataToWorklist (p_AccessionNumber:string;
                              p_sex : string;
                              p_pid : string;
...
// Отправка назначения на наш worklist
 p_SndToVCLWorklist : procedure ( p_AppHandl : THandle;
...
begin
  ...
  begin
    ...
    Try
      ...
      begin
        @p_SndToVCLWorklist := GetProcAddress(DLL_HANDLE, 'p_SndToVCLWorklist');
        if @p_SndToVCLWorklist <> nil then
        begin
          p_SndToVCLWorklist( Application.Handle,
                              p_AccessionNumber,
                              p_sex ,
                              p_pid ,
                              p_dateborn ,
                              ...
                              stepdescription ,
                              //
                              f_gtTSMINI('DLL_WORKLIST_NAZN','PG_DATABASE'), // p_PG_Database  : string;
                              StrToInt(f_gtTSMINI('DLL_WORKLIST_NAZN','PG_PORT')), // p_PG_Port : integer;
                              f_gtTSMINI('DLL_WORKLIST_NAZN','PG_SERVER'), // p_PG_Server  : string;
                              f_gtTSMINI('DLL_WORKLIST_NAZN','PG_SCHEMA'), // p_PGSchema : string;
                              f_gtTSMINI('DLL_WORKLIST_NAZN','PG_USERNAME'), // p_PG_Username  : string;
                              f_gtTSMINI('DLL_WORKLIST_NAZN','PG_PASSWORD') // p_PG_Password  : string
                              //
                            );
        end;
      end;
