...
type
  ...
  TfrmNaprRad = class(TForm)
    ...
    dsTypeR: TDataSource;
    odsVidIssled: TOracleDataSet;
    dsVidIssled: TDataSource;
    ...
    if (pNazID > 0) or (rowUsl.Tag > 0) then
      begin
        if (rowRFP.Properties.Value = null) or (rowRFP.Properties.Value = '') or (rowRFP.Properties.Value = '0') then
          begin
            Application.MessageBox('Вам необходимо заполнить поле "РФП"!', 'Предупреждение !', MB_OK + MB_ICONWARNING);
    ...
    if (pNazID > 0) or (rowUsl.Tag > 0) then
      begin
        if (merVve.Properties.Editors[0].Value = 0) or (merVve.Properties.Editors[0].Value = null) then
          begin
            Application.MessageBox('Вам необходимо заполнить поле "Введено (МБк)"!', 'Предупреждение !', MB_OK + MB_ICONWARNING);
...
//          end;
//-->>----------------------------------------------------------------------------
      if (trim(rowNum.Properties.Value) <> '') and (rowNum.Properties.Value <> null) and (rowNum.Properties.Value <> '0')  then //номер исследования
        begin
          oqNaz.DeleteVariables;
...
//-->>----------------------------------------------------------------------------
      if (rowTypeKontrast.Properties.Value <> '') and (rowTypeKontrast.Properties.Value <> null) and (rowTypeKontrast.Properties.Value <> '0') then //контраст(тип)
        begin
          oqNaz.DeleteVariables;
					oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt3 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (rowKolvoKontrast.Properties.Value <> 0) and (rowKolvoKontrast.Properties.Value <> null) then //кол-во контраста
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            ' nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowTypeVvod.Properties.Value <> '') and (rowTypeVvod.Properties.Value <> null) and (rowTypeVvod.Properties.Value <> '0') then //способ ввода
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt5 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowKto.Properties.Value <> '') and (rowKto.Properties.Value <> null) and (rowKto.Properties.Value <> '0') then //Кто ввел контраст
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID15 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowCntSnimki.Properties.Value <> 0) and (rowCntSnimki.Properties.Value <> null) then // кол-во снимков
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID NUMBER; '+#13+         
...
//-->>----------------------------------------------------------------------------
      if (rowDoza.Properties.Value <> 0) and (rowDoza.Properties.Value <> null) then //доза -1
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt9 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowTypePlenka.Properties.Value <> '') and (rowTypePlenka.Properties.Value <> null) and (rowTypePlenka.Properties.Value <> '0') then // Тип пленки
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt6 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowLists.Properties.Value <> '') and (rowLists.Properties.Value <> null) and (rowLists.Properties.Value <> '0') then // листы
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt6 NUMBER; '+#13+
        ...
        begin
          if (rowPhone.Properties.Value <> '') and (rowPhone.Properties.Value <> null) then //Контактный телефон
            begin
              ...
              oqNaz.DeclareAndSet('pFK_VRACHID', otInteger, frmMain.pSOTRID);
              oqNaz.Execute;
              // -- отметка о том, что записан по телефону   = -1
              oqNaz.DeleteVariables;
              oqNaz.SQL.Text := ' DECLARE '+#13+
                                '   nSMID12 NUMBER; '+#13+
                                '   Cnt12 NUMBER; '+#13+
            ...
            begin
              ...
              oqNaz.DeclareAndSet('pfk_pacid', otInteger, pNazID);
              oqNaz.Execute;
              // -- отметка о том, что записан по телефону   = 0
              oqNaz.DeleteVariables;
              oqNaz.SQL.Text := ' DECLARE '+#13+
                                '   nSMID12 NUMBER; '+#13+
                                '   Cnt12 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowSotr.Properties.Value <> '') and (rowSotr.Properties.Value <> null) and (rowSotr.Properties.Value <> '0') then //Рентген-лаборант
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID15 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (merWH.Properties.Editors[1].Value <> 0) and (merWH.Properties.Editors[1].Value <> null) then // вес
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (rowVol.Properties.Value <> 0) and (rowVol.Properties.Value <> null) then // объем РФП
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (merVve.Properties.Editors[0].Value <> 0) and (merVve.Properties.Editors[0].Value <> null) then // введено (МБк) на
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (merVve.Properties.Editors[1].Value <> 0) and (merVve.Properties.Editors[1].Value <> null) then // часов
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if (merVve.Properties.Editors[2].Value <> 0) and (merVve.Properties.Editors[2].Value <> null) then // минут
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
			if rowAct.Properties.Value <> null then // остаточная активность
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowRFP.Properties.Value <> null) and (rowRFP.Properties.Value <> '') and (rowRFP.Properties.Value <> '0') then // РФП
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowType.Properties.Value <> null) and (rowType.Properties.Value <> '') and (rowType.Properties.Value <> '0') then // тип
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   nSMID4 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowScan.Properties.Value <> '') and (rowScan.Properties.Value <> null) and (rowScan.Properties.Value <> '0') then // режим
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt2 NUMBER; '+#13+
...
//-->>----------------------------------------------------------------------------
      if (rowVvodRFP.Properties.Value <> '') and (rowVvodRFP.Properties.Value <> null) and (rowVvodRFP.Properties.Value <> '0') then // Спосо ввода РФП
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt2 NUMBER; '+#13+
  ...
  if EditMode = True then
    begin
      if frmMain.bIsAdmin <> 1 then
        ...
        end; // -- обработать данные если сотрудник входит в группу админы
    end else
    begin
      rowEdit.Visible := False;
      rowEdit.Properties.Value := '';
...
begin
  try
    ...
    pKABID := nKABID;
    pOwnerID := nOwner;
// -- в зависимости от исследования показываем или не показываем
    if pOwnerID <> 0 then
      begin
...
// -- Основные данные
    ...
    rowStatusPac.Properties.Value := 'В реанимации!';
...
// -- подгружаем значения из TIB
    ...
    while not ods.Eof do
      begin
        if ods.FieldByName('synname').AsString = 'RADIO_TYPE' then // Тип
          begin
            rowType.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'RADIO_VVOD_RFP' then // Способ ввода РФП
          begin
            rowVvodRFP.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'RADIO_TYPESCAN' then // Режим
          begin
            rowScan.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'XRAY_VVOD' then // Способ ввода
          begin
            rowTypeVvod.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_LAST_ACTIVE' then // Остаточная активность
          begin
            rowAct.Properties.Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_MINUTES' then // минут
          begin
            merVve.Properties.Editors[2].Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_HOURS' then // часов
          begin
            merVve.Properties.Editors[1].Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_VVEDENO' then // введено
          begin
            merVve.Properties.Editors[0].Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_VOLUME_RFP' then // объем РФП
          begin
            rowVol.Properties.Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_VES' then // Вес
          begin
            merWH.Properties.Editors[1].Value := ods.FieldByName('fn_num').AsFloat;
          end;
        if ods.FieldByName('synsmid').AsString = 'RADIO_ROST' then // Рост
          begin
            merWH.Properties.Editors[0].Value := ods.FieldByName('fn_num').AsFloat;
          end;
        if ods.FieldByName('synsmid').AsString = 'NUMBER_ISSL' then // Номер исследования
          begin
            rowNum.Properties.Value := ods.FieldByName('fc_char').AsString;
          end;
        if ods.FieldByName('synsmid').AsString = 'KTO_KONTRAST' then // Кто ввел контраст
          begin
            rowKto.Properties.Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'XRAY_KOLVOKONTRAST' then // Кол-во контраста
          begin
            rowKolvoKontrast.Properties.Value := ods.FieldByName('fn_num').AsFloat;
          end;
        if ods.FieldByName('synsmid').AsString = 'XRAY_KOLVOKOMY' then // Кол-во кому выдано
          begin
            rowKolvo.Properties.Value := ods.FieldByName('fn_num').AsInteger;
          end;
        if ods.FieldByName('synsmid').AsString = 'XRAY_FIO_REMARK' then // ФИО, примечания
          begin
            rowPrim.Properties.Value := ods.FieldByName('fc_char').AsString;
          end;
        if ods.FieldByName('synname').AsString = 'XRAY_KRATNOST' then // Кратность
          begin
            rowKrat.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'OTDEL_VYDANO' then // Куда выдано
          begin
            rowKuda.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'TIME_NORMA_ISSL' then // Надбавка
          begin
            rowNad.Properties.Value := ods.FieldByName('fk_smid').AsInteger;
          end;
        if ods.FieldByName('synname').AsString = 'RAD_VIDPLENKI' then // Тип пленки
          begin
...
procedure TfrmNaprRad.DoVidOplat; // если нет сохраненного значения, то берется значение по умолчанию
var ods : TOracleDataSet;
begin
  ods := TOracleDataSet.Create(nil);
  try
...
// -- "заряжаем" датасеты
  ...
  if odsVidIssled.Active = False then
    odsVidIssled.Active := True;
...
begin
  ...
  then begin
    try
      ...
      if ods.RecordCount > 0
      then begin
        Application.MessageBox(PAnsiChar('Назначение "' + oldName +
          '" невозможно изменить, т.к. для данного назначения имеются оплаченные платные услуги!'),
