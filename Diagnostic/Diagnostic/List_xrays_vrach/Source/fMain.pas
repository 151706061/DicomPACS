...
uses
  ...
  uPrintType, frxExportHTML, RVStyle, uPrintDocument, frxExportRTF, cxSpinEdit;
...
type
  ...
  TfrmMain = class(TForm)
    ...
    bbDicomCD: TdxBarButton;
    aDicomCD: TAction;
    aPrintZaklTalons: TAction;
    bbPrintZaklTalon: TdxBarButton;
    aPrintZaklTalonsPac: TAction;
    bbPrintZakltalonPac: TdxBarButton;
    ...
    procedure aDicomCDExecute(Sender: TObject);
    procedure aPrintZaklTalonsExecute(Sender: TObject);
  private
    FileVersion : string;
    HandleDLL: THandle;
  ...
  procedure pWLSend ( pAccessionNumber : string );
...
uses fOtmena, fPacInfo, fEditNaz, fSetNaz, fOtmetki, fArchive, uDBTableUtils,
     fProtocol, fSearchLab, fDolg, fOtmetkiFlu, fWait, fAllHistory, uGlobals,
     fProgressNorma, fCorrect, fProtocolN, fProtocolMore, fDolgTalon, fSysInfo,
     fPreview;
{$R *.dfm}

procedure TfrmMain.pSQLSetSQLText;
const cInSQLForSOKB = ' NVL (( select max(td.fc_name) from '+
                             ' ASU.tambtalon t, ASU.TINSURDOCS i, ASU.tambtalon_naz an, asu.ttypedoc td '+
                             ' WHERE  t.FK_INSURANCEID = i.FK_ID '+
                             ' and an.FK_TALONID = t.FK_ID '+
                             ' and td.fk_id=i.fk_typedocid '+
                             ' and an.fk_nazid=###.fk_id ) '+
                          ' , asu.GET_VIDOPLATY(###.fk_pacid)) as fc_vid_oplaty ';
begin
  if frmMain.get_db_name = 'SO' then
  begin
    odsList.SQL.Text                := StringReplace( odsList.SQL.Text,
                                                      'asu.GET_VIDOPLATY(isl.fk_pacid) as fc_vid_oplaty',
                                                      StringReplace( cInSQLForSOKB , '###' , 'isl',[rfReplaceAll, rfIgnoreCase]),
                                                      [rfReplaceAll, rfIgnoreCase]);
    odsListCito.SQL.Text            := StringReplace( odsListCito.SQL.Text,
                                                      'asu.GET_VIDOPLATY(isl.fk_pacid) as fc_vid_oplaty',
                                                      StringReplace( cInSQLForSOKB , '###' , 'isl',[rfReplaceAll, rfIgnoreCase]),
                                                      [rfReplaceAll, rfIgnoreCase]);
    odsListUnCito.SQL.Text          := StringReplace( odsListUnCito.SQL.Text,
                                                      'asu.GET_VIDOPLATY(isl.fk_pacid) as fc_vid_oplaty',
                                                      StringReplace( cInSQLForSOKB , '###' , 'isl',[rfReplaceAll, rfIgnoreCase]),
                                                      [rfReplaceAll, rfIgnoreCase]);
    odsListKardio.SQL.Text          := StringReplace( odsListKardio.SQL.Text,
                                                      'asu.GET_VIDOPLATY(t1.fk_pacid) as fc_vid_oplaty',
                                                      StringReplace( cInSQLForSOKB , '###' , 't1',[rfReplaceAll, rfIgnoreCase]),
                                                      [rfReplaceAll, rfIgnoreCase]);
    odsListKardioUnKabinet.SQL.Text := StringReplace( odsListKardioUnKabinet.SQL.Text,
                                                      'asu.GET_VIDOPLATY(t1.fk_pacid) as fc_vid_oplaty',
                                                      StringReplace( cInSQLForSOKB , '###' , 't1',[rfReplaceAll, rfIgnoreCase]),
                                                      [rfReplaceAll, rfIgnoreCase]);
  end;
...
//                    '        asu.rgreg_hide_last_fluoro'+#13+
 ...
 //   listxrayvrach_caneditcancel := ods.fieldbyname('listxrayvrach_caneditcancel').asinteger;
  ...
  //  rgreg_hide_last_fluoro := ods.fieldbyname('rgreg_hide_last_fluoro').asinteger;
    ...
    USE_AMBTALON_DLL := uDBTableUtils.TDBUtils.GetFunctionDef('USE_AMBTALON_DLL', os, -1979) = 1;
...
//-- tsmini
  ...
  if ( GET_DB_NAME = 'SK' ) then
  begin
    bbDicomCD.Visible := ivAlways;
  end
  else
  begin
    bbDicomCD.Visible := ivNever ;
  end;
  //
  pSQLSetSQLText;
end;
...
begin
  ...
  aPrintZaklTalonsPac.enabled := aPrintZaklTalonsPac.Visible and (odslist.recordcount > 0) and (odsList.FieldByName('fk_nazsosid').AsInteger = get_vipnaz);
end;
...
{$ENDIF}
  ...
  try
    aPrintZaklTalons.Visible := fileexists(extractfilepath(application.exename)+'print_zakl_talons_in_day.fr3');
    aPrintZaklTalonsPac.Visible := aPrintZaklTalons.Visible;
  except
...
var
  ods: TOracleDataSet;
  AmbTalon: THandle;
  InitPlugin_A: procedure (AppHandle: THandle; nPacID: Integer; nSotrID: Integer; nDef: Integer; nTalonID: Integer); stdcall;
  str: string;
  talonID: Integer;
begin
  ods := TOracleDataSet.Create(nil);
  ...
  try
    ods.Session := os;
    ods.Cursor := crSQLWait;
    ods.SQL.Text := ' select fk_nazid, fk_talonid from asu.tambtalon_naz where fk_nazid = :pfk_nazid ';
    ods.DeclareAndSet('pfk_nazid', otInteger, odsList.FieldByName('fk_id').AsInteger);
    ods.Open;
    ...
    else talonID := ods.FieldByName('fk_talonid').AsInteger;

    if get_db_name = 'H'
    then begin // только для ОКБ г.Ханты-Мансийск
      ods.DeleteVariables;
      ods.Close;
      ...
      if ods.FieldByName('fc_synonim').AsString <> 'TAL_OPLOMS'
      then begin
  ...
  then begin
    ...
    try
      if AmbTalon <> 0
      then begin
        @InitPlugin_A := GetProcAddress(AmbTalon, 'InitPluginModalDefTalon');

        if @InitPlugin_A = nil
        then Application.MessageBox('Функция InitPluginModalDefTalon не найдена в AMBTalonDLL.dll', 'Внимание', MB_OK + MB_ICONEXCLAMATION);

        try
          InitPlugin_A(Application.Handle,
...
procedure TfrmMain.aDicomCDExecute(Sender: TObject);
var
  ...
  vDir, v_filename, v_CallingAET : string;
  Ini : TIniFile;
  v_FlEx : Boolean; vDICOMport, vDICOMLocalPort : integer;
...
begin // Dicom CD
  //
  if odsList.Eof then Exit;
  //
  vDir := GetCurrentDir;
  try
    SetCurrentDir( ExtractFilePath( ParamStr(0) ) );
    if FileExists( ExtractFilePath(paramstr(0))+c_fileDLL ) then
    begin
      DLL_HANDLE:= LoadLibrary( PChar( ExtractFilePath(paramstr(0))+c_fileDLL ) );
      Try
        ...
        {  @p_DicomTest := GetProcAddress(DLL_HANDLE, 'p_DicomTest');
          ...
          if @pMakeCD <> nil then
           try
             ...
             try
               //
               vStudyList.Append( odsList.FieldByName('fc_type').AsString );

              { Showmessage( 'accessionnumber = ' +
                            odsList.FieldByName('FC_accessionnumber').AsString + ' | ' +
                            odsList.FieldByName('fc_type').AsString + ' | ' +
                            odsList.FieldByName('NUMBERPROTOCOL').AsString + ' | ' +
               ...
               if not TryStrToInt( f_gtTSMINI('XRAY','DICOM_PORT'), vDICOMport) then vDICOMport:=0;
               if not TryStrToInt( f_gtTSMINI('XRAY','DICOM_LOCAL_PORT'), vDICOMLocalPort) then vDICOMLocalPort :=0;
               pMakeCD (  4,              // 0 - STUDY  1 - PATIENTS, 2 - IMAGES, 4 - accessionnumber
                          vStudyList,
                          f_gtTSMINI('XRAY','DICOM_IP'), // '192.168.1.47', // pDICOMhost : string;
                          vDICOMport, // 11114,          // pDICOMport : Integer;
                          f_gtTSMINI('XRAY','DICOM_CALLED_AE'), // 'PACSSERVER',   // pDICOMCALLEDAE : string;
                          v_CallingAET, // 'TEST1',        // pDICOMCALLINGAE : string;
                          vDICOMLocalPort,  // 1131,           // pDICOMLocalPort : Integer;
                          0               // pIsDebug : integer
                        ) ;

  //             vStudyList.Append('1.3.46.670589.11.21162.5.0.7224.2014122309184792293');
  //             vStudyList.Append('1.2.840.113704.1.111.4588.1421305681.10');
               //
             {  pMakeCD (  0,              // 0 - STUDY  1 - PATIENTS, 2 - IMAGES
                          vStudyList,
...
procedure TfrmMain.aPrintZaklTalonsExecute(Sender: TObject);
var
  path, sPacID: String;
...
begin
  ...
  Parameters := PChar(' /file="' + path + 'print_zakl_talons_in_day.fr3" /param:pdate=' + dateToStr(dxdedate.date) +
