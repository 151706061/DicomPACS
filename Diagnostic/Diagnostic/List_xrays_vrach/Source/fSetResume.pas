...
uses
  ...
  dxSkinValentine, dxSkinXmas2008Blue, cxContainer, cxTextEdit, cxMemo;
...
type
  ...
  public
    procedure DoShowForm(AUseText : integer = 0);
...
procedure TfrmSetResume.DoShowForm(AUseText : integer = 0);
begin
  FUseText := AUseText;
  m.visible := AUseText = 1;
  cxgrid1.Visible := AUseText = 0;
  aRefresh.Visible := AUseText = 0;
  aAdd.Visible := AUseText = 0;
  aEdit.Visible := AUseText = 0;
  aDel.Visible := AUseText = 0;
  aUp.Visible := AUseText = 0;
  aDown.Visible := AUseText = 0;
  aAddChild.Visible := AUseText = 0;

  frmSetResume.Caption := 'Заключения для '+frmMain.odsKab.FieldByName('FC_NAME').AsString;
  DoSetEditRights;
  aSet.Enabled := FUseText = 1;
end;
...
begin
  ...
  aSet.Enabled := (FUseText = 1) or ((odsTree.FieldByName('FK_ID').AsInteger <> -2) and (odsTree.FieldByName('COUNTCHILD').AsInteger = 0));
