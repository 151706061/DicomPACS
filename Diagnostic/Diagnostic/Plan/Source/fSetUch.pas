...
begin
  ...
  try
    ...
    Reg.OpenKey('\Software\Softmaster\Plan\ID_SOTR'+IntToStr(frmMain.pSOTRID), True);
    lcbType.EditValue := Reg.ReadInteger('\Software\Softmaster\Plan\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', 0);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey:=HKEY_CURRENT_USER;
    if not Reg.OpenKey('\Software\Softmaster\Plan\', FALSE) then
      Reg.CreateKey('\Software\Softmaster\Plan');
    try
      Reg.WriteInteger('\Software\Softmaster\Plan\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', lcbType.EditValue);
    except
    end;
  finally
    Reg.Free;
