...
type
  ...
  TfrmRegistrAmbPac = class(TForm)
    ...
    pueVidOplat: TcxPopupEdit;
    grdbtvVidOplat: TcxGridDBTableView;
    grlVidOplat: TcxGridLevel;
    grVidOplat: TcxGrid;
    odsVidOpl: TOracleDataSet;
    dsVidOpl: TDataSource;
    grdbtvVidOplatFK_ID: TcxGridDBColumn;
    grdbtvVidOplatFK_PEPLID: TcxGridDBColumn;
    grdbtvVidOplatFC_DOGOVOR: TcxGridDBColumn;
    grdbtvVidOplatFL_OLD: TcxGridDBColumn;
    grdbtvVidOplatFD_DATE: TcxGridDBColumn;
    grdbtvVidOplatFD_END: TcxGridDBColumn;
    grdbtvVidOplatFC_NAME: TcxGridDBColumn;
    grdbtvVidOplatFC_TYPEDOC: TcxGridDBColumn;
    grdbtvVidOplatFN_ORDER: TcxGridDBColumn;
    grdbtvVidOplatFL_NOTDOC: TcxGridDBColumn;
    grdbtvVidOplatFC_SYNONIM: TcxGridDBColumn;
    grdbtvVidOplatTYPEID: TcxGridDBColumn;
    grdbtvVidOplatTYPEOPL: TcxGridDBColumn;
    grdbtvVidOplatFC_FULL_STRAH: TcxGridDBColumn;
    grdbtvVidOplatFK_COMPANYID: TcxGridDBColumn;
    grdbtvVidOplatISSERTIFICAT: TcxGridDBColumn;
    grdbtvVidOplatSER_NUM: TcxGridDBColumn;
    teNumNapravl: TcxTextEdit;
    lNumNapravl: TcxLabel;
    ...
    procedure pueVidOplatPropertiesChange(Sender: TObject);
    procedure grdbtvVidOplatDblClick(Sender: TObject);
  private
//    pSumDoza : Double;
...
//    isAgreePacient : Boolean; //--> включена ли настройка "Согласие пациента" ('-1' - включена, 'другое значение' - выключена)
    ...
    rgregist_use_typeopl : integer; //1-использовать вид оплаты из tinsuredocs, 0-по старому только tsmid
    rgregist_numaktusl_nullable : integer;//=1 НЕ обязательно для заполненния номер акта об оказании услуг
    rgregist_numnapravl_nullable : integer;//=1 НЕ обязательно для заполненния номер направления
    HandleDLL: THandle;
...
uses fNazRgOne1, fMain, fAllHistory, fProsmotrZakl, fKem, uDBTableUtils;
      ...
      try
        ...
        if KolvoDays >= kolvoParam then
          begin
            ...
            pueVidOplat.Enabled := (cxOnPhone.Checked) and (pAmbul = 1);
...
begin
  try
    rgregist_use_typeopl := TDBUtils.GetConstantDef('rgregist_use_typeopl',frmmain.os,0);
    rgregist_numnapravl_nullable := TDBUtils.GetConstantDef('rgregist_numnapravl_nullable',frmmain.os,1);
    rgregist_numaktusl_nullable := TDBUtils.GetConstantDef('rgregist_numaktusl_nullable',frmmain.os,1);
    Screen.Cursor := crSQLWait;
    bVozrast.Visible := cxdeRojd.Enabled;
    pAMBUL := nAMBUL;
    pPACID := nPACID;
    pPEPLID := nPEPLID;
    if rgregist_use_typeopl = 1 then
     begin
       odsVidOpl.close;
       odsVidOpl.setvariable('fk_peplid',pPeplid);
       odsVidOpl.open;
     end;
    pOTDELID := nOTDELID;
    pSMIDID := nSMIDID;
    ...
    if idtalon > 0 then
     begin
       try
         ods := toracledataset.create(nil);
         ods.session := frmmain.os;
         ods.sql.text := 'select fc_naprnum from asu.tambtalon where fk_id = :ptalonid';
         ods.declareandset('ptalonid',otinteger,idtalon);
         ods.open;
         teNumNapravl.text := ods.fieldbyname('fc_naprnum').asstring;
         ods.close;
       finally
         ods.free;
       end;
...
begin
  ...
  if (teNumNapravl.visible) and (lNumNapravl.tag = 0 {vidopl synonim in (DOGOVOR,UDOST)}) and (rgregist_numnapravl_nullable = 0) and (teNumNapravl.text = '') then
    begin
      Application.MessageBox('Не заполнен номер направления!',
                             'Предупреждение !', MB_OK + MB_ICONWARNING);
      teNumNapravl.SetFocus;
      Result := 0;
      exit;
    end;

  if (teNumNapravl.visible) and (lNumNapravl.tag = 1 {vidopl synonim in (KOMERC)}) and (rgregist_numaktusl_nullable = 0) and (teNumNapravl.text = '') then
    begin
      Application.MessageBox('Не заполнен номер акта об оказании услуг!',
                             'Предупреждение !', MB_OK + MB_ICONWARNING);
      teNumNapravl.SetFocus;
      Result := 0;
      exit;
...
procedure TfrmRegistrAmbPac.pueVidOplatPropertiesChange(Sender: TObject);
var
  syn  : string;
begin
  syn := odsVidOpl.FieldByName('FC_SYNONIM').AsString;
  lbCompany.Visible := syn = 'DOGOVOR';
  cxCompDogovor.Visible := syn = 'DOGOVOR';
  lNumNapravl.Visible := (syn = 'DOGOVOR') or (syn = 'UDOST') or (syn = 'KOMERC');
  teNumNapravl.Visible := (syn = 'DOGOVOR') or (syn = 'UDOST') or (syn = 'KOMERC');
  if (syn = 'DOGOVOR') or (syn = 'UDOST') then
   begin
    lNumNapravl.caption := '№ направления';
    lNumNapravl.tag := 0;
   end
  else if (syn = 'KOMERC') then
   begin
    lNumNapravl.caption := '№ акта об оказании услуг';
    lNumNapravl.tag := 1;
   end;
  (*if odsVidOpl.FieldByName('FC_SYNONIM').AsString = 'DOGOVOR'{'TAL_OTHER'} then
    begin
      lbCompany.Visible := True;
      cxCompDogovor.Visible := True;
    end else
...
//-->>----------------------------------------------------------------------------
      if (cxVidOplat.Text <> '') or (pueVidOplat.tag > 0) then //способ оплаты
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' DECLARE '+#13+
                            '   Cnt24 NUMBER; '+#13+
                            ...
                            ' IF Cnt24 > 0 THEN '+#13+
                            '   UPDATE TIB SET FN_NUM=:PNUM, FK_SMID=:pFK_SMID24, FK_SMEDITID=:pFK_SMEDITID24, FK_VRACHID=:pFK_VRACHID, '+
                            'fd_date=sysdate WHERE FK_PACID=:pFK_ID AND FK_SMID IN (SELECT FK_ID FROM TSMID WHERE FK_OWNER=(SELECT MAX(FK_ID) FROM TSMID WHERE FC_SYNONIM=''AMBTALON_OPL'')); '+#13+
                            ' ELSE '+#13+
                            '   INSERT INTO TIB(FK_PACID, FN_NUM, FK_SMID, FK_SMEDITID, FK_VRACHID, fd_date) VALUES(:pFK_ID, :PNUM, :pFK_SMID24, :pFK_SMEDITID24, :pFK_VRACHID, sysdate); '+#13+
                            ' END IF; '+#13+
                            ' if :pupdatalon = 1 then' + #13 +
                            '  update asu.tambtalon set fk_oplata = :pFK_SMID24 where fk_id = :ptalon;' + #13 +
                            ' end if;' + #13 +
                            ' END; ';
          oqNaz.DeclareAndSet('PFK_ID', otInteger, odsListNaz.FieldByName('FK_ID').AsInteger);
          oqNaz.declareandset('ptalon',otinteger, idtalon);
          if rgregist_use_typeopl = 1 then
           begin
            oqNaz.declareandset('pupdatalon',otinteger, 1);
            oqNaz.DeclareAndSet('PNUM', otInteger, pueVidOplat.tag);
            oqNaz.DeclareAndSet('pFK_SMID24', otInteger, grVidOplat.tag);
            oqNaz.DeclareAndSet('pFK_SMEDITID24', otInteger, grVidOplat.tag);
           end
          else
           begin
            oqNaz.declareandset('pupdatalon',otinteger, 0);
            oqNaz.DeclareAndSet('PNUM', otInteger, null);
...
//-->>----------------------------------------------------------------------------
      if teNumNapravl.visible and (teNumNapravl.text <> '') then  //added by Melnikov 2015.12.05  рабочая суббота это плохо
        begin
          oqNaz.DeleteVariables;
          oqNaz.SQL.Text := ' update asu.tambtalon set fc_naprnum = :pnaprnum where fk_id = :ptalon';
          oqNaz.declareandset('ptalon',otinteger, idtalon);
          oqNaz.DeclareAndSet('pnaprnum', otstring, teNumNapravl.text);
          oqNaz.Execute;
          frmMain.os.Commit;
        end;
//-->>----------------------------------------------------------------------------
...
begin
  ...
  if rgregist_use_typeopl = 1 then
   begin
    if VarName = 'VID_OPLATA' then Value := pueVidOplat.Text;
   end
  else
   begin
...
var
  ...
  vType, vSerNum : variant;
begin
  ods := TOracleDataSet.Create(nil);
  try
    ods.Session := frmMain.os;
    ods.SQL.Text := 'DECLARE' + #13#10 +
                    '  CURSOR C IS' + #13#10 +
                    '    SELECT FN_NUM, FK_SMID' + #13#10 +
                    '      FROM ASU.TIB' + #13#10 +
                    '     WHERE FK_PACID = :PFK_PEPLID' + #13#10 +
                    '       AND FK_SMID IN' + #13#10 +
                    '           (SELECT FK_ID' + #13#10 +
                    '              FROM ASU.TSMID' + #13#10 +
                    '             WHERE FK_OWNER =' + #13#10 +
                    '                   (SELECT MAX(FK_ID)' + #13#10 +
                    '                      FROM ASU.TSMID' + #13#10 +
                    '                     WHERE FC_SYNONIM = ''AMBTALON_OPL''))' + #13#10 +
                    '     ORDER BY FK_ID DESC;' + #13#10 +
                    'BEGIN' + #13#10 +
                    '  OPEN C;' + #13#10 +
                    '  FETCH C' + #13#10 +
                    '    INTO :PNUM, :VAL;' + #13#10 +
                    '  CLOSE C;' + #13#10 +
                    '  IF :VAL IS NULL THEN' + #13#10 +
                    '    SELECT MAX(FK_ID)' + #13#10 +
                    '      INTO :VAL' + #13#10 +
                    '      FROM TSMID' + #13#10 +
                    '     WHERE FK_OWNER =' + #13#10 +
                    '           (SELECT FK_ID FROM ASU.TSMID WHERE FC_SYNONIM = ''AMBTALON_OPL'')' + #13#10 +
                    '       AND FK_DEFAULT = 1;' + #13#10 +
                    '     :pnum := -1;' + #13#10 +
                    '  END IF;' + #13#10 +
                    'END;';
    ods.DeclareAndSet('PFK_PEPLID', otInteger, pNAZID);
    ods.DeclareVariable('VAL', otInteger);
    ods.DeclareVariable('PNUM', otInteger);
    ods.Open;
    if rgregist_use_typeopl = 1 then
     begin
      cxVidOplat.Visible := false;
      pueVidOplat.Visible := true;
      if ods.GetVariable('pnum') <> null then
       pueVidOplat.tag := ods.GetVariable('pnum');
      if ods.GetVariable('VAL') <> null then
       grVidOplat.tag := ods.GetVariable('VAL');
      if pueVidOplat.tag > 0 then
       if odsVidOpl.locate('fk_id',pueVidOplat.tag,[]) then
        begin
         dc := grdbtvVidOplat.datacontroller;
         if dc.GetRecordCount > 0 then
          begin
           vType := dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatFC_TYPEDOC.index];
           vSerNum := dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatSER_NUM.index];
           pueVidOplat.Text := vartostr(vType) + ' ' + vartostr(vSerNum);
          end;
        end;
     end
    else
     begin
      cxVidOplat.Visible := true;              
      pueVidOplat.Visible := false;
    ...
    if ods.FieldByName('cnt').AsInteger = 0 then
      begin
        ...
        try
          oq.Session := frmMain.os;
          oq.Cursor := crSQLWait;
          oq.SQL.Text := ' insert into asu.tambtalon(fk_sotrid, fd_opened, fk_ambid, fn_sos, fk_otdelid, fc_naprnum, fk_oplata) '+#13+
                         ' values(:pfk_sotrid, sysdate, :pfk_ambid, 0, :pfk_otdelid, :pnaprnum, :poplata) '+#13+
                         ' returning fk_id into :pfk_id ';
          oq.DeclareAndSet('pfk_sotrid', otInteger, frmMain.pSOTRID);
          oq.DeclareAndSet('pfk_ambid', otInteger, pPACID);
          oq.DeclareAndSet('pfk_otdelid', otInteger, pOTDELID);
          if teNumNapravl.visible then
           oq.DeclareAndSet('pnaprnum', otString, teNumNapravl.text)
          else
           oq.DeclareAndSet('pnaprnum', otString, '');
          if rgregist_use_typeopl = 1 then
           begin
            oq.declareandset('poplata', otinteger, grVidOplat.tag);
           end
          else
           begin
            oq.declareandset('poplata', otinteger, -1);
           end;
...
procedure TfrmRegistrAmbPac.grdbtvVidOplatDblClick(Sender: TObject);
var
  dc : tcxdbdatacontroller;
begin
  dc := grdbtvVidOplat.DataController;
  pueVidOplat.Text := trim(vartostr(dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatFC_TYPEDOC.index]) + ' '
                    + vartostr(dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatSER_NUM.index]));
  pueVidOplat.Tag := strtointdef(vartostr(dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatFK_ID.index]),0);
  grVidOplat.Tag := strtointdef(vartostr(dc.values[dc.GetFocusedRecordIndex,grdbtvVidOplatTypeOpl.index]),0);
  pueVidOplat.PopupWindow.CloseUp;
  pueVidOplat.PostEditValue;
end;
