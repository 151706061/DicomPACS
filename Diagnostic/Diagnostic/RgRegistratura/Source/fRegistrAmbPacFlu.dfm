object frmRegistrAmbPacFlu: TfrmRegistrAmbPacFlu
  ...
  object oqAddTalon: TOracleQuery
    SQL.Strings = (
      'DECLARE'
      '  MKB      NUMBER;'
      '  MKB_NAME ASU.TSMID.FC_NAME%TYPE;'
      '  TALONID  NUMBER;'
      '  NAZSOS   NUMBER;'
      '  ISPOLID  NUMBER;'
      '  CURSOR C IS'
      ...
      '      FROM ASU.TSMID S'
      '     WHERE S.FK_MKB10 = '#39'Z01.9'#39
      '    CONNECT BY PRIOR FK_ID = FK_OWNER'
      '     START WITH FC_SYNONIM = '#39'DIAGS_MKB10'#39';'
      '  CURSOR CNAZ IS'
      
        '    SELECT FK_NAZSOSID, FK_ISPOLID FROM ASU.VNAZ WHERE FK_ID = :' +
        'PNAZID;'
      '  CURSOR CTALONNAZ IS'
      
        '    SELECT FK_TALONID FROM ASU.TAMBTALON_NAZ WHERE FK_NAZID = :P' +
        'NAZID;'
      'BEGIN'
      '  OPEN CNAZ;'
      '  FETCH CNAZ'
      ...
      '      (:PAMBID,'
      '       MKB,'
      '       ASU.GET_SYNID('#39'PRI_POSTYPLENII'#39'),'
      '       MKB_NAME,'
      '       SYSDATE,'
      '       :PSOTRID,'
      '       ASU.GET_SYNID('#39'MAIN'#39'),'
      '       MKB,'
      '       asu.get_synid('#39'ZDOROV'#39'))'
      '    RETURNING FK_ID INTO MKB;'
      '  '
      '    IF NAZSOS <> ASU.GET_VIPNAZ THEN'
      '      ISPOLID := :PSOTRID;'
      '    END IF;'
      '  '
      '    INSERT INTO ASU.TAMBTALON'
      '      (FK_SOTRID,'
      '       FK_SOTRCLOSED,'
      '       FD_OPENED,'
      '       FD_DOCDATE,'
      ...
      '       SYSDATE,'
      '       :PAMBID,'
      '       ASU.GET_SYNID('#39'TAL_POLYKL'#39'),'
      '       ASU.GET_SYNID('#39'AR_SINGLE'#39'),'
      '       MKB,'
      '       SYSDATE,'
      '       ASU.RESULT_BEZ_PER,'
      '       ASU.GET_SYNID('#39'LECHENIE_ZAVERSCHENO'#39'),'
      '       '#39'SET_SOTR_AS_ISPOL_FROM_NAZ'#39')'
      '    RETURNING FK_ID INTO TALONID;'
      '  '
      '    INSERT INTO ASU.TAMBTALON_DIAGS'
      '      (FK_DIAGID, FK_TALONID)'
      '    VALUES'
      '      (MKB, TALONID);'
      '  '
      '    INSERT INTO ASU.TIB'
      ...
      '    VALUES'
      '      (TALONID,'
      '       ASU.GET_SYNID('#39'TALON_TYPE_CREATE_PRIKREP'#39'),'
      '       ASU.GET_TALON_TYPE_CREATE,'
      '       :PSOTRID,'
      '       SYSDATE);'
      '  '
      '    INSERT INTO ASU.TAMBTALON_NAZ'
      '      (FK_TALONID, FK_AMBID, FK_NAZID)'
      '    VALUES'
      '      (TALONID, :PAMBID, :PNAZID);'
      '  ELSE'
      '    IF NAZSOS <> ASU.GET_VIPNAZ THEN'
      '      ISPOLID := :PSOTRID;'
      '    END IF;'
      ...
      '       SET A.FK_SOTRID = ISPOLID, A.FK_SOTRCLOSED = ISPOLID'
      '     WHERE FK_ID = TALONID;'
      '  END IF;'
      ''
      
        '--  UPDATE ASU.VNAZ SET FL_MORE_PUT = 1024 WHERE FK_ID = :PNAZID' +
        ';'
      'END;')
    Session = frmMain.os
    Optimize = False
    Variables.Data = {
      0300000003000000070000003A50414D42494403000000000000000000000008
      0000003A50534F54524944030000000000000000000000070000003A504E415A
