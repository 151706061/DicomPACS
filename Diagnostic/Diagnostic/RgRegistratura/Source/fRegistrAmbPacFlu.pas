...
uses fNazRgOne1, fMain, fAllHistory, fCompanyList, fProsmotrZaklFlu, uDBTableUtils;
...
begin
  try
    ...
    pNAZID := nNAZID;
    isAdd := nnazid = 0;
    iTARGET_PROFILAKTIKA := TDBUtils.GetSmidIDBySyn('TARGET_PROFILAKTIKA',frmMain.os,-1);
    if pPACID = 0 then
      begin
...
function GetTalonID(App_HND: THandle; OS: TORacleSession; pPacID, pSotrID:Integer):integer;
const
  ADD_TALON_IMMIDIATLY = -15478;
var
  AAmbTalonHandle: THandle;
  str: string;
...
begin
  ...
  if AAmbTalonHandle > HINSTANCE_ERROR then
    begin
      ...
      begin
        try
          RESULT := GetAmbtalonId(App_HND,OS,pPacID, pSotrID, ADD_TALON_IMMIDIATLY);
        except
          on E:Exception do
          begin
  ...
  //ShowMEssage('EndDoCreateAmbTalonSave');
end;
...
//            oqIns.Cursor := crSQLWait;
    ...
    //Melnikov 2015.07.27 #31769
  if (cxTarget.EditValue = iTARGET_PROFILAKTIKA) and isAdd then
   begin
     oqAddTalon.SetVariable('pambid',idpac);
     oqAddTalon.SetVariable('psotrid',frmmain.pSOTRID);
     oqAddTalon.SetVariable('pnazid',cxNaz.Tag);
     oqAddTalon.execute;
     {idtalon := GetTalonID(application.handle, frmMain.os,idpac,frmmain.pSOTRID);
     with toraclequery.create(nil) do
      begin
        session := frmMain.os;
        sql.text := 'insert into asu.tambtalon_naz(fk_ambid,fk_talonid,fk_nazid) values(:pambid,:ptalonid,:pnazid)';
        declareandset('pambid',otinteger, IDPac);
        declareandset('ptalonid',otinteger, idtalon);
        declareandset('pnazid',otinteger, cxNaz.Tag);
        execute;
      end;   }
   end;
