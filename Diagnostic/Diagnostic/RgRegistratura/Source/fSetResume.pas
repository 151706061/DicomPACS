  ...
  if frmAddEditElement.ModalResult = MrOK then
    begin
      ...
      try
        ...
        oq.Cursor := crSQLWait;
        oq.SQL.Text := ' INSERT INTO TSHABLON_XRAY_ZAKL(FC_NAME, FK_OWNER, FK_KABINETID) VALUES(:PFC_NAME, :PFK_OWNER, :PFK_KABINETID) ';
  ...
  if frmAddEditElement.ModalResult = MrOK then
    begin
      ...
      try
        oq.Session := frmMain.os;
        oq.Cursor := crSQLWait;
        oq.SQL.Text := ' UPDATE TSHABLON_XRAY_ZAKL SET FC_NAME = :PFC_NAME WHERE FK_ID = :PFK_ID ';
        oq.DeclareAndSet('PFC_NAME', otString, frmAddEditElement.cxName.Text);
...
//  aDel.Enabled := odsTree.FieldByName('FK_ID').AsInteger <> -2;
...
begin
  odsTree.SetVariable('PFK_KABINETID', frmMain.odsKab.FieldByName('FK_ID').AsInteger); // !!! косяк при вызове формы из поиска - пусто
end;
