...
uses
  ...
  dxSkinOffice2007Green, Menus, cxLookAndFeelPainters, cxButtons;
...
begin
  ...
  try
    ...
    Reg.OpenKey('\Software\Softmaster\Fluoro_vyverka\ID_SOTR'+IntToStr(frmMain.pSOTRID), True);
    lcbType.EditValue := Reg.ReadInteger('\Software\Softmaster\Fluoro_vyverka\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', 0);
  finally
    Reg.Free;
  end;
end;
...
begin
  ...
  try
    Reg.RootKey:=HKEY_CURRENT_USER;
    if not Reg.OpenKey('\Software\Softmaster\Fluoro_vyverka\', FALSE) then
      Reg.CreateKey('\Software\Softmaster\Fluoro_vyverka');
    try
      Reg.WriteInteger('\Software\Softmaster\Fluoro_vyverka\ID_SOTR'+IntToStr(frmMain.pSOTRID), 'ID_UCH', lcbType.EditValue);
    except
    end;
  finally
    Reg.Free;
