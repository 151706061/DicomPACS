...
type
  TfrmMain = class(TForm)
    ...
    cxLabel5: TcxLabel;
    cxLabel4: TcxLabel;
    TVKONTRAST: TcxGridTableView;
    VKONTRASTKABINET: TcxGridColumn;
    VKONTRASTFD_NAZ: TcxGridColumn;
    VKONTRASTFD_RUN: TcxGridColumn;
    VKONTRASTFIOPAC: TcxGridColumn;
    VKONTRASTISSLNAME: TcxGridColumn;
    acEnable: TAction;
    VKONTRASTFK_NAZSOSID: TcxGridColumn;
    VKONTRASTFD_RUN_TIME: TcxGridColumn;
    VKONTRASTFK_ID: TcxGridColumn;
    ...
    procedure odsListLabAfterOpen(DataSet: TDataSet);
    ...
    procedure TVKONTRASTCellDblClick(Sender: TcxCustomGridTableView;
      ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
      AShift: TShiftState; var AHandled: Boolean);
    procedure TVKONTRASTFocusedRecordChanged(Sender: TcxCustomGridTableView;
...
var i : Integer;
begin
  for i := 0 to TVKONTRAST.DataController.LockCount - 1 do
    TVKONTRAST.EndUpdate;
end;
...
begin
  ...
  if frmSetParam.ModalResult = mrOK then
    begin
      ...
      if TVKONTRAST.DataController.LockCount = 0 then
        TVKONTRAST.BeginUpdate;
      TVKONTRAST.DataController.RecordCount := 0;
      while TVKONTRAST.DataController.LockCount > 0 do
        TVKONTRAST.EndUpdate;
      cxGr.Refresh;
      dxMemData1.Close;
      dxMemData1.Open;
...
var ods : TOracleDataSet;
begin
  if TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFK_NAZSOSID.Index] = GET_VIPNAZ then
    begin
      ods := TOracleDataSet.Create(nil);
      try
        ...
        ods.SQL.Text := ' SELECT FK_ID, '+
                        ...
                        '        (SELECT FD_ROJD FROM TPEOPLES WHERE FK_ID = (SELECT FK_PEPLID FROM TKARTA WHERE FK_ID = TNAZIS.FK_PACID '+
                        '         UNION '+
                        '         SELECT FK_PEPLID FROM TAMBULANCE WHERE FK_ID = TNAZIS.FK_PACID)) FD_ROJD '+
                        '   FROM TNAZIS '+
                        '  WHERE FK_ID = :PFK_ID ';
        ods.DeclareAndSet('PFK_ID', otInteger, TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFK_ID.Index]);
        ods.Open;
        ...
        dDateRojd_4Rep := ods.FieldByName('FD_ROJD').AsDateTime;
      finally
        ods.Free;
      end;
...
begin
  aResult.Enabled := (oqThread.RowCount <> 0) and (TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFK_NAZSOSID.Index] <> GET_LABVIP);
...
begin
  ...
  try
    ...
    frmMain.Caption := 'Статистика по сотруднику - Контраст('+Ver.FileVersion+'). Пользователь: '+sVrachFIO;
  ...
  TVKONTRAST.StoreToRegistry('\SoftWare\SoftMaster\Xray_sotr_kontrast\fMain\TVLIST'+'Vrach'+ '_' + IntToStr(pSOTRID)+'Default', TRUE, [], 'TVLIST');
  TVKONTRAST.RestoreFromRegistry('\SoftWare\SoftMaster\Xray_sotr_kontrast\fMain\TVLIST'+'Vrach'+ '_' + IntToStr(pSOTRID), FALSE, FALSE, [], 'TVLIST');
end;
...
begin
  TVKONTRAST.StoreToRegistry('\SoftWare\SoftMaster\Xray_sotr_kontrast\fMain\TVLIST'+'Vrach'+ '_' + IntToStr(pSOTRID), TRUE, [], 'TVLIST');
end;
...
begin
  if VarName = 'COMPANYNAME' then Value := sCompanyName;
  if VarName = 'OTDELNAME' then Value := sOtdelName;
  if VarName = 'PACFIO' then Value := TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFIOPAC.Index];
  if VarName = 'DATE_ROJD' then Value := dDateRojd_4Rep;
  if VarName = 'NUMIB' then Value := sNumIB_4rep;
  if VarName = 'NAMEISSL' then Value := TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTISSLNAME.Index];
  if VarName = 'DOZA' then Value := nDoza_4Rep;
  if VarName = 'DATEISSL' then Value := TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFD_RUN_TIME.Index];
...
begin
  TVKONTRAST.RestoreFromRegistry('\SoftWare\SoftMaster\Xray_sotr_kontrast\fMain\TVLIST'+'Vrach'+ '_' + IntToStr(pSOTRID)+'Default', FALSE, FALSE, [], 'TVLIST');
end;

procedure TfrmMain.odsListLabAfterOpen(DataSet: TDataSet);
begin
  CheckEnabledButtons;
end;
...
begin
  TVKONTRAST.BeginUpdate;
  TVKONTRAST.DataController.RecordCount := TVKONTRAST.DataController.RecordCount + 1;
  with TVKONTRAST.DataController do
  begin
    try
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFK_ID.Index] := Sender.Field('FK_ID');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTKABINET.Index] := Sender.Field('KABNAME');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFD_NAZ.Index] := Sender.Field('FD_NAZ');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFD_RUN.Index] := Sender.Field('FD_RUN');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFIOPAC.Index] := Sender.Field('FIOPAC');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTISSLNAME.Index] := Sender.Field('ISSLNAME');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFK_NAZSOSID.Index] := Sender.Field('FK_NAZSOSID');
      Values[TVKONTRAST.DataController.RecordCount - 1, VKONTRASTFD_RUN_TIME.Index] := Sender.Field('FD_RUN_TIME');
      dxMemData1.Insert;
  ...
  cxLabel6.Caption := FormatDateTime('hh:mm:ss', Now() - StartTime);
  cxLabel6.Refresh;
  cxLabel5.Caption := 'Выбрано ' + IntToStr(TVKONTRAST.DataController.RecordCount) + ' записей.';
  cxLabel5.Refresh;
end;
...
procedure TfrmMain.TVKONTRASTCellDblClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmMain.TVKONTRASTFocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
  AFocusedRecord: TcxCustomGridRecord; ANewItemRecordFocusingChanged: Boolean);
begin
  aResult.Enabled := TVKONTRAST.DataController.Values[TVKONTRAST.DataController.FocusedRecordIndex, VKONTRASTFK_NAZSOSID.Index] <> GET_LABVIP;
end;
