...
uses
  ...
  cxButtonEdit, cxVGrid, Makhaon_TLB, ActiveX, ComObj, OleServer, uPrintDocument,
  dxSkinOffice2007Green, dxSpellChecker, frxExportHTML, frxExportRTF{,
  dxSkinsdxBarPainter DCM_Client, DCM_Attributes, DCM_Dict, KXString, KXServerCore, DCM_Server,  }
  {, KXString, KXServerCore, DCM_Server};
...
type
  TfrmProtocol = class(TForm)
    ...
    bbFR3Print: TdxBarButton;
    bbOfficePrint: TdxBarButton;
...
uses fMain, fAllHistory, fOtmetki, fSetResume, fMedEditorTemplates,
     fOtmetkiFlu, fPass, fControlProtocol, uReportUtils, //fSnimki,
     fShowControlProtokol, fSetQ, uGlobal, CmnUnit, {DICOM_CMN,} UnFrmProgressBar,
     {uCommonXray,} fSnimok, RVFuncs, fProfFactor, fSaveRSh, fAddEditSh, IniFiles;
...
// -- показывать или не показывать кнопку "Зачесть"
  if (frmMain.odsKab.FieldByName('is_flu').AsInteger = 1) or (pOWNERID = frmMain.GET_ISSL_FLU) then
    begin
...
//Showmessage('GET_DB_NAME = '+frmMain.GET_DB_NAME);

      if frmMain.get_dicom_show = '-1' then
        begin

//Showmessage('get_dicom_show = -1 ') ;
    ...
    begin
      ...
      begin
        //  Showmessage( 'get_use_cmove='+frmMain.get_use_cmove );
...
procedure TfrmProtocol.aPrintOfficeExecute(Sender: TObject);
begin // print into Office
  if pOwnerID = get_rad_gamma then
    begin
      DoShowProtocol_Gamma(True);
      exit;
    end;
  if PownerID = get_rad_petct then
    begin
      DoShowProtocol_Petct(True);
      exit;
    end;
  ...
  if frmMain.GET_DB_NAME = 'SO' then // печать протокола для Сургут ОКБ
    begin
      DoShowProtocol_4SOKB(True);
      exit;
    end;
  if frmMain.GET_DB_NAME = 'NV' then // печать протокола для Нижневартовска
    begin
      DoShowProtocol_4NV(True);
      exit;
    end;
  DoShowProtocol_4OKBHMAO(True); // печать протокола для всех остальных
end;
...
procedure TfrmProtocol.frxRepSOKBGetValue(const VarName: string; var Value: Variant);
var
  TEXTISSL : string;
begin
  if VarName = 'PACFIO' then Value := merFIOMK.Properties.Editors[0].Value;
  ...
  if VarName = 'DATEISSL' then Value := DateToStr(cxdeDateProtocol.Date)+ ' '+ cxteTimeProtocol.Text;
  if VarName = 'SOTR' then Value := cxSotr.Text;
  TEXTISSL := rveText.GetSelText;
  if cxTextResume.Text <> '' then
  begin
    textissl := Textissl+#13+'Заключение: '+cxTextResume.Text;
  end;
  if memrecom.text <> '' then
  begin
    textissl := Textissl+#13+'Рекомендовано: '+memrecom.Text;
  end;
  if VarName = 'TEXTISSL' then Value := TEXTISSL;
  if VarName = 'KABINET' then Value := frmMain.odsKab.FieldByName('FC_NAME').AsString;
  if VarName = 'PHONEKAB' then Value := frmMain.odsKab.FieldByName('FC_PHONE').AsString;
//  if VarName = 'COMPANYNAME' then Value := frmMain.sCompanyName_4Rep;
//  if VarName = 'OTDELNAME' then Value := frmMain.sOtdelName_4Rep;
...
//      Showmessage('our viewer');

      {  DoShowSnimki(merFIOMK.Properties.Editors[1].Value,
...
//Showmessage('v_pid='+v_pid+'   v_an='+v_an);
