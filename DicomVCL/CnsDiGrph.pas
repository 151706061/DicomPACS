...
type
  ...
  protected
    ...
    FOnReadWriteProgress: TDicomEasyProgressEvent;
  ...
  public
    ...
    property OnReadWriteProgress: TDicomEasyProgressEvent
...
procedure SetPixelBc16(const ScanLine: PByteArray;
  const X: LongInt;
  const Value: LongInt); register;

function GetPixelBc16(const ScanLine: PByteArray;
  const X: LongInt): LongInt; register;
...
begin
  ...
  try
    ...
    if (ImageFormat <> ifTrueColor) and (ImageFormat <> ifGray4096) and
      (pPalette = nil) then
    begin
      case ImageFormat of
...
begin
  raise EDicomEasyError.Create(msgUseNewImageToChangeSize);
end;
...
begin
  case BitCount of
    ...
    raise EDicomEasyError.Create('Unsupported bit count');
  end;
end;
...
begin
  ...
  if BitmapBits = nil then
    raise EDicomEasyError.Create('GlobalAlloc failed');
  try
    if not GetDIB(InBitmapHandle, BitmapHeaderPtr^, BitmapBits) then
      raise EDicomEasyError.Create('GetDIB failed');

    with BitmapHeaderPtr^.bmiHeader do
    begin
      if biCompression <> Windows.BI_RGB then
        raise EDicomEasyError.Create('Unsupported compression');
...
begin
  ...
  try
    ...
    begin
      ...
      raise EDicomEasyError.Create('Invalid HDIB');
...
begin
  ...
  try
    ...
    if AData = 0 then
      raise EDicomEasyError.Create('CopyBitmap: CreateBitmap failed');
...
begin
  if IsEmpty then
    raise EDicomEasyError.Create(msgGraphicIsEmpty);
...
begin
  if ImageFormat = ifTrueColor then
    raise EDicomEasyError.Create(msgImageDoesNotHavePalette);
