...
{                                                       }
{*******************************************************}
unit DCM32_Interface;

{$I DicomPack.inc}
interface

uses DCM32, Graphics, Windows, Classes, DCM32_Transforms, DCM32_Polygons;
...
function SignD(const AValue: Double): TValueSign; overload;
{$ENDIF}
...
function WinFixToFixed(x: _FIXED): DCM32.TFixed;
function FixedToWinFix(x: DCM32.TFixed): _Fixed;
...
function PointsAreEqual(const p1, p2: DCM32.tFixedPoint): boolean;
function Distance(const p1, p2: DCM32.tFixedPoint): DCM32.tFixed; overload;
function Distance(const p1x, p1y, p2x, p2y: double): double; overload;
function SqrDistance(const p1, p2: DCM32.tFixedPoint): DCM32.tFixed;
function Norm1(const x1, y1, x2, y2: integer): integer; overload;
function Norm1(const p1, p2: tFixedPoint): DCM32.tFixed; overload;
...
function SegmentConditionalLengthQ3N2(const p1, p2, p3, p4: tFixedPoint): DCM32.tFixed;
{ qadric bezier segment conditional length in norm1 (supremum valuation) }
function SegmentConditionalLengthQ2N1Sup(const x0, x1, x2: tFixedPoint): DCM32.tFixed;
{ qadric bezier segment conditional length in norm2 (supremum valuation) }
function SegmentConditionalLengthQ2N2Sup(const x0, x1, x2: tFixedPoint): DCM32.tFixed;
{ qubic bezier segment conditional length in norm1 (supremum valuation) }
function SegmentConditionalLengthQ3N1Sup(const x0, x1, x2, x3: tFixedPoint): DCM32.tFixed;
{ qubic bezier segment conditional length in norm2 (supremum valuation) }
function SegmentConditionalLengthQ3N2Sup(const x0, x1, x2, x3: tFixedPoint): DCM32.tFixed;
{ qubic bezier segment conditional curvatre in norm1 }
function SegmentConditionalCurvatureQ2(const x0, x1, x2: tFixedPoint): DCM32.tFixed;
{ return position and tangent valuations of point, sited on xLength among segment - just interpolation of line}
procedure gGetPointPositionValuationAtSegment(const p1, p2: DCM32.tFixedPoint;
  const xLength: DCM32.tFixed;
  out xPathX, xPathY: DCM32.tFixed;
  out xAngle: double);

{ return qubic bezier length with curve approximation }
function gGetCurveLength(const xCurve: tArrayOfFixedPoint): DCM32.tFixed;
...
procedure gGetPointAtCurve(const xCurve: tArrayOfFixedPoint;
  const xLength: DCM32.tFixed;
  out xPathX, xPathY: DCM32.tFixed; out xAngle: double
  );
...
function gGetPointAtCurveEx(const xStartSegmentInd: integer;
  const xStartLength: DCM32.tFixed;
...
procedure gDrawSymbol(xBitmap: TCnsBitmap32;
  const xP: DCM32.tFixedPoint;
  const xSymbol: tSymbolKind;
  const xSize: DCM32.tFixed;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);

procedure gDrawSymbols(xBitmap: TCnsBitmap32;
  const xPoints: tArrayOfFixedPoint;
  const xSymbol: tSymbolKind;
  const xSize: DCM32.tFixed;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);
{ just call one of DCM32.Poligon function according options }
procedure gPolygon(Bitmap: TCnsBitmap32;
...
{ just call one of DCM32.Poligon function according options }
procedure gPolyPolygon(Bitmap: TCnsBitmap32;
...
procedure gPolyBezier(Bitmap: TCnsBitmap32;
  const Points: TArrayOfFixedPoint;
  const Color: TColor32;
  const Options: tPolygonDrawOptions;
  const Closed: Boolean;
...
procedure gPolyPolyBezier(Bitmap: TCnsBitmap32;
  const Points: TArrayOfArrayOfFixedPoint;
  const Color: TColor32;
  const Options: tPolygonDrawOptions;
  const Closed: Boolean;
...
procedure gPolygonRounded(xBitmap: TCnsBitmap32;
  const xPoints: TArrayOfFixedPoint;
  const xRadius: DCM32.tFixed;
  const xColor: TColor32;
  const xOptions: tPolygonDrawOptions;
  const xClosed: Boolean;
  const xFillMode: TPolyFillMode = pfAlternate);

procedure gPolyPolygonRounded(xBitmap: TCnsBitmap32;
...
procedure gRectangleHole(xBitmap: TCnsBitmap32;
  const xRect: tFixedRect;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);

{ draw simple ellipse }
procedure gEllipse(xBitmap: TCnsBitmap32;
  const xRect: TFixedRect;
  const xColor: TColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);

{ draw rotated ellipse }
procedure gEllipseRotated(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed; { if xAngle = 0 then A <-> width and xB <-> Height }
  const xAngle: double; { value in radians }
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);

{ draw transformed ellipse with xAT affine tranformation matrix }
procedure gEllipseT(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed; { if xAngle = 0 then A <-> width and xB <-> Height }
...
procedure gArc(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);

{ draw elliptic arc }
procedure gArcElliptic(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);

{ draw elliptic and rotated arc }
procedure gArcER(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
...
procedure gArcET(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xAT: TFloatMatrix; { affine transformation matrix }
...
procedure gSegment(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
...
procedure gSegmentElliptic(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xColor: tColor32;
...
procedure gSegmentER(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xRotAngle: double;
...
procedure gSegmentET(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xAT: TFloatMatrix; { affine transformation matrix }
...
procedure gPie(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
...
procedure gPieElliptic(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
...
procedure gPieER(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xRotAngle: double;
...
procedure gPieET(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double; { values in radians }
  const xAT: TFloatMatrix; { affine transformation matrix }
...
procedure gRectangleRounded(xBitmap: TCnsBitmap32;
  const xRect: TFixedRect;
  const xR: DCM32.TFixed;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure gRectangleRR(xBitmap: TCnsBitmap32;
  const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xR: DCM32.tFixed;
  const xAngle: double;
...
procedure gCardinalSpline(xBitmap: TCnsBitmap32;
  const xPoints: tArrayOfFixedPoint;
...
procedure gPolyCardinalSpline(xBitmap: TCnsBitmap32;
  const xPoints: tArrayOfArrayOfFixedPoint;
  const xTension: double;
  const xColor: tColor32;
  const xClosed: boolean;
...
procedure gTCBSpline(xBitmap: TCnsBitmap32;
  const xPoints: tArrayOfFixedPoint;
  const xTension: double;
...
procedure gPolyTCBSpline(xBitmap: TCnsBitmap32;
  const xPoints: tArrayOfArrayOfFixedPoint;
  const xTension: double;
  const xContinuity: double;
  const xBias: double;
...
{ end: wrapper around TCnsBitmap32, to don't depend on future versions g32 lib. }

type

  TCnsBitmap32Ex = class(TCnsBitmap32)
  private
    fFont_e31: Single;
    fFont_e32: Single;
    fLastSumLength: DCM32.tFixed;
  ...
  public
    ...
    procedure DrawGlyph(const xCharCode: longword;
      const xLeft, yTop: DCM32.tFixed;
      const xColor: tColor32;
      const xOptions: tPolygonDrawOptions);

    { eng: render text with current font and symbols transform}
    procedure RenderTextEx(const xLeft, yBottom: DCM32.tFixed;
      const xText: string;
      const xColor: tColor32;
      const xOptions: tPolygonDrawOptions);
...
uses Sysutils, Math, DCM32_LowLevel;
...
function FloatToWinFix(x: single): _FIXED;
begin
  result := _Fixed(DCM32.tFixed(trunc(x * 65536)));
end;

function WinFixToFloat(x: _Fixed): double;
begin
  { result := DCM32.tFixed(x)/65536; }
  result := div65536 * DCM32.tFixed(x);
end;
{$ENDIF}

function WinFixToFixed(x: _FIXED): DCM32.TFixed;
begin
  result := DCM32.TFixed(x);
end;
...
begin
  ...
  result.eM11 := _Fixed(DCM32.tFixed(trunc(xMat[0, 0] * 65536)));
  result.eM21 := _Fixed(DCM32.tFixed(trunc(xMat[1, 0] * 65536)));
  result.eM12 := _Fixed(DCM32.tFixed(trunc(xMat[0, 1] * 65536)));
  result.eM22 := _Fixed(DCM32.tFixed(trunc(xMat[1, 1] * 65536)));
  {$ELSE}
...
begin
  {$IFDEF OPTIMIZE_CALLFUNCTIONS}
  result[0, 0] := div65536 * DCM32.tFixed(xMat.eM11);
  result[1, 0] := div65536 * DCM32.tFixed(xMat.eM21);
  result[0, 1] := div65536 * DCM32.tFixed(xMat.eM12);
  result[1, 1] := div65536 * DCM32.tFixed(xMat.eM22);
  {$ELSE}
...
begin
  ...
  C := Cos(xAngle);
  {$IFDEF OPTIMIZE_CALLFUNCTIONS}
  result.eM11 := _Fixed(DCM32.tFixed(trunc(C * 65536)));
  result.eM21 := _Fixed(DCM32.tFixed(trunc(S * 65536)));
  result.eM12 := _Fixed(DCM32.tFixed(trunc(-S * 65536)));
  result.eM22 := _Fixed(DCM32.tFixed(trunc(C * 65536)));
  {$ELSE}
...
begin
  {$IFDEF OPTIMIZE_CALLFUNCTIONS}
  m1_00 := div65536 * DCM32.tFixed(m1.eM11);
  m1_01 := div65536 * DCM32.tFixed(m1.eM12);
  m1_10 := div65536 * DCM32.tFixed(m1.eM21);
  m1_11 := div65536 * DCM32.tFixed(m1.eM22);

  m2_00 := div65536 * DCM32.tFixed(m2.eM11);
  m2_01 := div65536 * DCM32.tFixed(m2.eM12);
  m2_10 := div65536 * DCM32.tFixed(m2.eM21);
  m2_11 := div65536 * DCM32.tFixed(m2.eM22);

  Result.eM11 := _Fixed(DCM32.tFixed(trunc((m1_00 * m2_00 + m1_10 * m2_01) * 65536)));
  Result.eM12 := _Fixed(DCM32.tFixed(trunc((m1_01 * m2_00 + m1_11 * m2_01) * 65536)));
  Result.eM21 := _Fixed(DCM32.tFixed(trunc((m1_00 * m2_10 + m1_10 * m2_11) * 65536)));
  Result.eM22 := _Fixed(DCM32.tFixed(trunc((m1_01 * m2_10 + m1_11 * m2_11) * 65536)));
  {$ELSE}
...
begin
  {$IFDEF OPTIMIZE_CALLFUNCTIONS}
  Result := div65536 * DCM32.tFixed(xMat.eM11) * div65536 * DCM32.tFixed(xMat.eM22)
    - div65536 * DCM32.tFixed(xMat.eM12) * div65536 * DCM32.tFixed(xMat.eM21);
  {$ELSE}
...
function Norm1(const p1, p2: tFixedPoint): DCM32.tFixed;
begin
...
function Distance(const p1, p2: DCM32.tFixedPoint): DCM32.tFixed;
begin
...
var
  ...
  function point(const x, y: DCM32.tFixed): tFixedPoint;
  begin
    result.x := x;
    result.y := y;
  end;
begin
  ...
  PP[0, 0] := point(DCM32.Fixed(-1), DCM32.Fixed(-1));
  PP[0, 1] := point(DCM32.Fixed(xBitmap.Width + 1), DCM32.Fixed(-1));
  PP[0, 2] := point(DCM32.Fixed(xBitmap.Width + 1), DCM32.Fixed(xBitmap.Height + 1));
  PP[0, 3] := point(DCM32.Fixed(-1), DCM32.Fixed(xBitmap.Height + 1));
...
begin
  ...
  yPP[0].y := xCenter.Y;
  yPP[1].x := yPP[0].x;
  yPP[1].y := xCenter.Y - dy;
  yPP[2].x := xCenter.x - dx;
  yPP[2].y := xCenter.Y - B;
  yPP[3].x := xCenter.x;
  yPP[3].y := yPP[2].y;
  yPP[4].x := xCenter.x + dx;
  yPP[4].y := yPP[2].y;
  yPP[5].x := xCenter.x + A;
  yPP[5].y := xCenter.Y - dy;
  yPP[6].x := yPP[5].x;
  yPP[6].y := xCenter.Y;
  yPP[7].x := yPP[5].x;
  yPP[7].y := xCenter.Y + dy;
  yPP[8].x := xCenter.x + dx;
  yPP[8].y := xCenter.Y + B;
  yPP[9].x := xCenter.x;
  yPP[9].y := yPP[8].y;
  yPP[10].x := xCenter.x - dx;
  yPP[10].y := yPP[8].y;
  yPP[11].x := yPP[0].x;
  yPP[11].y := xCenter.Y + dy;
  yPP[12].x := yPP[0].x;
  yPP[12].y := xCenter.Y;
...
begin
  ...
  PP[0].x := xRect.Right - xR;
  PP[0].y := xRect.Top;
  PP[1].x := xRect.Right - dR;
  PP[1].y := xRect.Top;
  PP[2].x := xRect.Right;
  PP[2].y := xRect.Top + dR;
  PP[3].x := xRect.Right;
  ...
  PP[5].x := xRect.Right;
  PP[5].y := xRect.Bottom - xR;
  PP[6].x := xRect.Right;
  PP[6].y := PP[5].y;
  PP[7].x := xRect.Right;
  PP[7].y := xRect.Bottom - dR;
  PP[8].x := PP[1].x;
  PP[8].y := xRect.Bottom;
  PP[9].x := PP[0].x;
  PP[9].y := xRect.Bottom;
  ...
  PP[11].x := xRect.Left + xR;
  PP[11].y := xRect.Bottom;
  PP[12].x := PP[11].x;
  PP[12].y := xRect.Bottom;
  PP[13].x := xRect.Left + dR;
  PP[13].y := xRect.Bottom;
  PP[14].x := xRect.Left;
  PP[14].y := PP[7].y;
  PP[15].x := xRect.Left;
  PP[15].y := PP[5].y;
...
begin
  ...
  PP[0].x := vRect.Right - xR;
  PP[0].y := vRect.Top;
  PP[1].x := vRect.Right - dR;
  PP[1].y := vRect.Top;
  PP[2].x := vRect.Right;
  PP[2].y := vRect.Top + dR;
  PP[3].x := vRect.Right;
  ...
  PP[5].x := vRect.Right;
  PP[5].y := vRect.Bottom - xR;
  PP[6].x := vRect.Right;
  PP[6].y := PP[5].y;
  PP[7].x := vRect.Right;
  PP[7].y := vRect.Bottom - dR;
  PP[8].x := PP[1].x;
  PP[8].y := vRect.Bottom;
  PP[9].x := PP[0].x;
  PP[9].y := vRect.Bottom;
  PP[10].x := PP[0].x;
  PP[10].y := vRect.Bottom;

  PP[11].x := vRect.Left + xR;
  PP[11].y := vRect.Bottom;
  PP[12].x := PP[11].x;
  PP[12].y := vRect.Bottom;
  PP[13].x := vRect.Left + dR;
  PP[13].y := vRect.Bottom;
  PP[14].x := vRect.Left;
  PP[14].y := PP[7].y;
  PP[15].x := vRect.Left;
  PP[15].y := PP[5].y;
...
begin
  ...
  p21.x := p2.x + round(d * CosC);
  p21.y := p2.y - round(d * SinC);
  p22.x := p2.x + round(d * CosB);
  p22.y := p2.y - round(d * SinB);
  yC.x := p2.x + round(f * CosAB);
  yC.y := p2.y - round(f * SinAB);
...
var
  ...
  PP: TArrayOfFixedPoint;
begin
  xRect.Left := xP.x - xSize;
  xRect.Top := xP.y - xSize;
  xRect.Right := xP.x + xSize;
  xRect.Bottom := xP.y + xSize;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PolygonTS(Bitmap, Points, Color, FillMode);
        // eng: fill without antialising
        PolylineXS(Bitmap, Points, Color, true); 
  ...
  begin
    ...
    begin
      if ByteBool(Options and pdoFilling) then
        ...
        PolylineAS(Bitmap, Points, Color, Closed)
    end
    else
    begin
      if ByteBool(Options and pdoFilling) then
        ...
        PolylineTS(Bitmap, Points, Color, Closed);
    end
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PolyPolylineXS(Bitmap, Points, Color, true);
      end
      else
      begin
  ...
  begin
    ...
    begin
      if ByteBool(Options and pdoFilling) then
        ...
        PolyPolylineAS(Bitmap, Points, Color, Closed)
    end
    else
    begin
      if ByteBool(Options and pdoFilling) then
...
begin
  ...
  if Length(xPoints) = 1 then
    exit; { OldRgn <> rrInside }
...
{  eng:   CrvePoints - is a storage of linear approximation of qubic Bezier curve
}
...
procedure BuildApproxCurve(const xPoints: TArrayOfFixedPoint;
  const xMinSegmentLength: DCM32.tFixed;
...
var
  ...
  begin
    ...
    if (CurvePointsCount >= Length(CurvePoints) div 2 - 4) then
    begin
      try
    ...
    p11.x := (p1.x + p0.x) div 2;
    p11.y := (p1.y + p0.y) div 2;
    p21.x := (p2.x + p1.x) div 2;
    p21.y := (p2.y + p1.y) div 2;

    p31.x := (p3.x + p2.x) div 2;
    p31.y := (p3.y + p2.y) div 2;
    p22.x := (p11.x + p21.x) div 2;
    p22.y := (p11.y + p21.y) div 2;
    p32.x := (p21.x + p31.x) div 2;
    p32.y := (p21.y + p31.y) div 2;

    p33.x := (p22.x + p32.x) div 2;
    p33.y := (p22.y + p32.y) div 2;
    {$ELSE}
    p11 := MiddleLine(p1, p0);
    p21 := MiddleLine(p2, p1);

    p31 := MiddleLine(p3, p2);
    p22 := MiddleLine(p11, p21);
    p32 := MiddleLine(p21, p31);
    { new node }
    p33 := MiddleLine(p22, p32);
    {$ENDIF}
...
begin
  ...
  if (Length(Points) <= 1) or ((Length(Points) - 1) mod 3 <> 0) then
    exit;
  ...
  if ClipPolyOutside(Points, FixedRect(rect(0, 0, Bitmap.Width, Bitmap.Height))) then
    exit;
...
begin
  ...
  begin
    if (Length(Points[j]) <= 1) or ((Length(Points[j]) - 1) mod 3 <> 0) then
      continue;

    { eng: clipping... }
    if ClipPolyOutside(Points[j], FixedRect(rect(0, 0, Bitmap.Width, Bitmap.Height))) then
      exit;
...
begin
  ...
  begin
    ...
    for j := 0 to GPPointCount[i] - 1 do
      result[i, j] := GlyphPolygonPP[i, j];
  end;
end;
...
procedure BuildGlyphPolygon(xBufPtr: PTTPolygonHeader;
  const xBufSize: integer;
  const xMinSegmentLength: DCM32.tFixed;
  const xLeft, yTop: DCM32.tFixed;
  const xGM: TGLYPHMETRICS;
  const xCliptRect: DCM32.tFixedRect); 
...
var
  ...
  begin
    ...
    {$IFDEF OPTIMIZE_CALLFUNCTIONS_MDL}
    p21.x := (p1.x + p0.x) div 2;
    p21.y := (p1.y + p0.y) div 2;
    p22.x := (p2.x + p1.x) div 2;
    p22.y := (p2.y + p1.y) div 2;
    p3.x := (p21.x + p22.x) div 2;
    p3.y := (p21.y + p22.y) div 2;
    {$ELSE}
    ...
    begin
      ...
      {$ELSE}
      GlyphPolygon.Add(p22);
      GlyphPolygon.Add(p2);
      {$ENDIF}
    end
  end;

  function xWinFixToG32(const x: _FIXED): DCM32.TFixed;
  begin
    result := X.value * $10000 + x.fract + xLeft;
  end;

  function yWinFixToG32(const y: _FIXED): DCM32.TFixed;
  begin
...
begin
  ...
  begin
    ...
    pcSize := xBufPtr^.cb - SizeOf(TTTPOLYGONHEADER);
    pChar(pc) := pChar(xBufPtr) + SizeOf(TTTPOLYGONHEADER);
    ofs2 := 0;
...
constructor TCnsBitmap32Ex.Create;
begin
  inherited Create;
...
destructor TCnsBitmap32Ex.Destroy;
begin
  fCanvas.Free;
  inherited Destroy;
end;
...
procedure TCnsBitmap32Ex.DrawGlyph(const xCharCode: longword;
  const xLeft, yTop: DCM32.tFixed;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);
var
...
begin
  ...
  buf := bufPtr;
  if not FUNICODE then
    Res := GetGlyphOutline(dc, xCharCode, GGO_NATIVE, gm, bufSize, pchar(buf), fFontMat2)
  else
    Res := GetGlyphOutlineW(dc, xCharCode, GGO_NATIVE, gm, bufSize, pchar(buf), fFontMat2);
  ...
  BuildGlyphPolygon(bufPtr,
    ...
    xLeft, yTop,
    gm,
    FixedRect(fDrawOrign.x, fDrawOrign.y, DCM32.Fixed(Self.Width) + fDrawOrign.x, DCM32.Fixed(Self.Height) + fDrawOrign.y)); { указывае?прямоугольник отсечения }
...
procedure TCnsBitmap32Ex.RenderTextEx(const xLeft, yBottom: DCM32.tFixed;
  const xText: string;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);
var
  vShift: DCM32.tFixedPoint;

  Res, bufSize: LongInt;
  bufPtr, buf: PTTPolygonHeader;
  gm1, gm: TGLYPHMETRICS;
...
begin
  ...
  try
    ...
    begin
      ...
      if (bufSize = 0) then
      begin
        vShift.x := vShift.x + DCM32.Fixed(gm1.gmCellIncX);
        vShift.y := vShift.y - DCM32.Fixed(gm1.gmCellIncY);
        continue;
      end;
      ...
      buf := bufPtr;
      if not FUNICODE then
        Res := GetGlyphOutline(Handle, vCharCode, GGO_NATIVE, gm, bufSize, pchar(buf), fFontMat2 {VertFlip_mat2})
      else
        Res := GetGlyphOutlineW(Handle, vCharCode, GGO_NATIVE, gm, bufSize, pchar(buf), fFontMat2 {VertFlip_mat2});
      if (res = GDI_ERROR) or (buf^.dwType <> TT_POLYGON_TYPE) then
        continue;
      ...
      BuildGlyphPolygon(bufPtr,
        bufSize,
        vQuality,
        xLeft + vShift.x, yBottom + vShift.y,
        gm1,
        FixedRect(fDrawOrign.x,
        fDrawOrign.y,
        DCM32.Fixed(Self.Width) + fDrawOrign.x,
        DCM32.Fixed(Self.Height) + fDrawOrign.y));

      vShift.x := vShift.x + DCM32.Fixed(gm1.gmCellIncX * FittedText_SpacingFactor);
      vShift.y := vShift.y - DCM32.Fixed(gm1.gmCellIncY);
...
function TCnsBitmap32Ex.GetPointAtCurve(const xStartSegmentInd: integer;
  const xCurve: tArrayOfFixedPoint;
  const xLength: DCM32.tFixed;
  out xPathX, xPathY: DCM32.tFixed;
  out xAngle: double;
...
begin
  ...
  if xApproxNeed then
    {eng: 1.approx curve }
    BuildApproxCurve(xCurve, Bezier3SegmentMinLengthInPixel_2 shl 16,
      FixedRect(fDrawOrign.x, fDrawOrign.y, DCM32.Fixed(Self.Width) + fDrawOrign.x, DCM32.Fixed(Self.Height) + fDrawOrign.y));

  {conditional: Curve is placed in CurvePoints array }

  if xStartSegmentInd <> 0 then
    vSumLength := fLastSumLength
  else
  begin
    vSumLength := 0;
    fLastSumLength := 0;
...
procedure TCnsBitmap32Ex.RenderFittedText(const xText: string;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions;
  const xPath: tArrayOfFixedPoint);
var
  ...
  PathX, PathY: DCM32.tFixed;
  vTangent: double;

  vFontMat2: tMat2;

  vQuality: integer;
  vCurveLength: DCM32.tFixed;

  vLastSegment: integer;
  vLengthShift: DCM32.tFixed;

  vApproxedPath: tArrayOfFixedPoint;

  procedure GrowLengthShift(const dX, dY: DCM32.tFixed);
  begin
    { dL^2  = dx^2 + dy^2}
    vLengthShift := vLengthShift + DCM32.Fixed(sqrt(sqr(div65536 * dX) + sqr(div65536 * dy)) * FittedText_SpacingFactor);
  end;
begin
  ...
  if ClipPolyOutside(xPath, FixedRect(fDrawOrign.x, fDrawOrign.y, DCM32.Fixed(Self.Width) + fDrawOrign.x, DCM32.Fixed(Self.Height) + fDrawOrign.y)) then
    exit;
  ...
  for i := 0 to High(vApproxedPath) do
    vApproxedPath[i] := CurvePoints[i];
  ...
  begin
    ...
    if (bufSize = 0) then
    begin
      GrowLengthShift(DCM32.Fixed(gm1.gmCellIncX), DCM32.Fixed(gm1.gmCellIncY));
      continue;
    end;

    if (vLengthShift < 0) or (vCurveLength < vLengthShift) then
      break;
    ...
    if abs(DetMat2(vFontMat2)) > 0.01 then
    begin
      try
        bufPtr := AllocMem(bufSize);
        buf := bufPtr;
...
function TCnsBitmap32Ex.SelectFontMat2(const xValue: tMat2): tMat2;
begin
  result := fFontMat2;
  fFontMat2 := xValue;
end;

function TCnsBitmap32Ex.SelectFontTransform(const xValue: tFloatMatrix): tFloatMatrix; { столбе?свободны?членов игнориуется }
begin
...
function TCnsBitmap32Ex.GetCanvas: tCanvas;
begin
  fCanvas.Handle := Self.Handle;
  result := fCanvas;
end;

procedure TCnsBitmap32Ex.PolyBezier(const Points: TArrayOfFixedPoint;
...
procedure TCnsBitmap32Ex.Ellipse(const xRect: tFixedRect;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions);
begin
...
procedure TCnsBitmap32Ex.Arc(const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.ArcElliptic(const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.EllipseRotated(const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.Pie(const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.PieElliptic(const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.Segment(const xCenter: tFixedPoint;
  const xR: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.SegmentElliptic(const xCenter: tFixedPoint;
  const xA, xB: DCM32.tFixed;
  const xStartAngle, xEndAngle: double;
  const xColor: tColor32;
  const xOptions: tPolygonDrawOptions = pdoFloat);
...
procedure TCnsBitmap32Ex.RectangleHole(const xRect: tFixedRect;
  const xColor: TColor32;
  const xOptions: tPolygonDrawOptions);
begin
...
procedure TCnsBitmap32Ex.RectangleHole(const xRect: tRect;
  const xColor: TColor32;
  const xOptions: tPolygonDrawOptions);
begin
...
procedure TCnsBitmap32Ex.Polygon(const Points: TArrayOfFixedPoint;
  const Color: TColor32;
  const Options: tPolygonDrawOptions;
  const Closed: Boolean;
  const FillMode: TPolyFillMode);
...
procedure TCnsBitmap32Ex.PolyPolygon(
...
begin
  if (fDrawOrign.X = 0) and (fDrawOrign.Y = 0) then
  begin
    gPolyPolygon(Self, Points, Color, Options, Closed, FillMode);
  end
  else
    if fDrawOrign.X = 0 then
  begin
    for i := 0 to Length(Points) - 1 do
    begin
      xPoints := Points[i];
...
procedure TCnsBitmap32Ex.PolyPolyBezier(const Points: TArrayOfArrayOfFixedPoint; const Color: tColor32;
  const Options: tPolygonDrawOptions;
