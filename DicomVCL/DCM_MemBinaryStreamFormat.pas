unit DCM_MemBinaryStreamFormat;
...
begin
  ...
  begin
    ...
    if (j mod 100) = 0 then
      ADataSet.Progress(trunc((j / cnt) * 100), mtpcSave);
    ...
    if (ADataSet.OverrideActiveRecordBuffer = nil) then
      continue;
    ...
    Accept := true;
    if Assigned(ADataSet.OnSaveRecord) then
      ADataSet.OnSaveRecord(ADataset, Accept);
    if not Accept then
      continue;
    ...
    if (not (sfSaveData in sfData)) and (ARec.UpdateStatus = usUnmodified) then
      continue;
    ...
    begin
      ...
      if Rec.UpdateStatus = usInserted then
        continue;
    end;
    {$ELSE}
    if (not (sfSaveData in sfData)) and (ADataSet.OverrideActiveRecordBuffer^.UpdateStatus = usUnmodified) then
      continue;
    ...
    begin
      ...
      if pRec^.UpdateStatus = usInserted then
        continue;
    end;
    {$ENDIF}
    ...
    begin
      ...
      begin
        if SaveFields[i] >= 0 then
        begin
          if NewestVersion and Assigned(ADataSet.OnSaveField) then
      ...
      if not (sfSaveDeltas in sfDeltas) then
        break;
...
begin
  ...
  if InitIndexDef then
    ADataset.CreateIndexes;
  ADataset.OverrideActiveRecordBuffer := nil;
  inherited;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if FileVersion >= 250 then
        KName := Reader.ReadString
      else
        KName := FieldKindNames[0]; // fkData
      if FileVersion >= 251 then
        DExpr := Reader.ReadString
      else
        DExpr := '';

      // Find fieldtype from fieldtypename.
      for i := 0 to ord(High(FieldTypeNames)) do
        if FieldTypeNames[TFieldType(i)] = TName then
          break;
      FT := TFieldType(i);
    ...
    begin
      ...
      begin
        ...
        if Reader.ReadBoolean then
          ioptions := ioptions + [ixDescending];
        if Reader.ReadBoolean then
          ioptions := ioptions + [ixCaseInSensitive];

        {$IFNDEF LEVEL3}
        if Reader.ReadBoolean then
          ioptions := ioptions + [ixNonMaintained];
        {$ELSE}
        Reader.ReadBoolean; // Skip ixNonMaintained info since not supported for D3/BCB3.
        {$ENDIF}
        if Reader.ReadBoolean then
          ioptions := ioptions + [ixUnique];
    ...
    if InitTableDef then
      ADataSet.Open;
  end;
  if not (ld and ldidx) then
    InitIndexDef := false;
end;
...
begin
  ...
  try
    ...
    if ApproxRecs > 0 then
      ADataSet.Common.Records.Capacity := ApproxRecs; // For speed reason try to preallocate room for all records.
    ...
    begin
      ...
      begin
        ...
        ARec := TKbmRecord(Marshal.PtrToStructure(ADataSet.OverrideActiveRecordBuffer, TypeOf(TKbmRecord)));
        if FileVersion >= 230 then
          ARec.UpdateStatus := TUpdateStatus(Reader.ReadInteger);
        Marshal.StructureToPtr(ARec, ADataSet.OverrideActiveRecordBuffer, False);
        {$ELSE}
        if FileVersion >= 230 then
          ADataSet.OverrideActiveRecordBuffer^.UpdateStatus := TUpdateStatus(Reader.ReadInteger);
        {$ENDIF}
        {$ENDIF}
        {$ENDIF}
        ...
        begin
          ...
          begin
            ...
            case ADataSet.Fields[j].DataType of
              ...
              {$ELSE}
              ftWideString: ADataSet.Fields[j].Value :=
                {$IFDEF LEVEL6}
                {$IFDEF LEVEL13}
                UTF8ToWideString(RawByteString(Reader.ReadString))
                  {$ELSE}
                UTF8Decode(Reader.ReadString)
                  {$ENDIF}
                {$ELSE}
                Reader.ReadString
                  {$ENDIF};
              ...
              ftDate,
                ...
                begin
                  ...
                  if Date = 0 then
                    ...
                    ADataSet.Fields[j].AsFloat := Date;
                end;
            else
          ...
          if NewestVersion and Assigned(ADataSet.OnLoad) then
            ADataSet.OnLoadField(ADataset, i, ADataSet.Fields[i]);
          {$IFNDEF BINARY_FILE_1XX_COMPATIBILITY}
          {$IFNDEF BINARY_FILE_200_COMPATIBILITY} // New for v. 2.24.
        end;
      ...
      Accept := true;
      if Assigned(ADataSet.OnLoadRecord) then
        ADataSet.OnLoadRecord(ADataset, Accept);
      if Accept then
      begin
        {$IFDEF DOTNET}
        Rec := TKbmRecord(Marshal.PtrToStructure(pRec, TypeOf(TKbmRecord)));
