...
var
  ...
  AppName    : array[0..255] of {$IFDEF LEVEL12}WideChar{$ELSE}AnsiChar{$ENDIF};
begin
{$IFDEF LEVEL12}
  result := false;
{$ELSE}
  ...
  AppName[0] := #0;
  lstrcat(AppName, {$IFDEF LEVEL12}PWideChar{$ELSE}PChar{$ENDIF}(ParamStr(0)));  // ParamStr(0) = Application.ExeName
  CharUpperBuff(AppName, SizeOf(AppName)div 2);
