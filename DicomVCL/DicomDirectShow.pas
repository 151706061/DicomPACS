...
type
  ...
  //=====================================================================
type
  TVMRPresentationFlags = TOleEnum;
const
...
type
  ...
  TVMRSurfaceAllocationFlags = TOleEnum;
const
...
const
  ...
  RenderPrefs_RestrictToInitialMonitor = $00000010; // output only to initial monitor
...
type
  ...
  TAMPlayListEventFlags = TOleEnum;
const
...
const
  ...
  //
type
  TAM_Property_DVDCopyProt = TOLeEnum;
const
...
const
  ...
  ED_MODE_RECORD_FREEZE = ED_BASE + 808;
...
type
  TDXVA_EncryptProtocolHeader = record
  ...
  TDXVA_ConfigPictureDecode = record
    // Operation Indicated
  ...
  TDXVA_PictureParameters = record
...
type
  TDXVA_BufferDescription = record
...
type
  TDXVA_MBctrl_I_HostResidDiff_1 = record
    wMBaddress: WORD;
  ...
  TDXVA_MBctrl_I_OffHostIDCT_1 = record
    wMBaddress: WORD;
  ...
  // Host Residual Differences
  TDXVA_MBctrl_P_HostResidDiff_1 = record
    wMBaddress: WORD;
  ...
  // Without OBMC, without BinPB and 4MV together, without MV RPS
  // Off-Host IDCT, 4:2:0 sampling
  TDXVA_MBctrl_P_OffHostIDCT_1 = record
    wMBaddress: WORD;
...
type
  TDXVA_ConfigAlphaCombine = record
    // Operation Indicated
    dwFunction: TDXVA_ConfigQueryOrReplyFunc;
    // Alignment
    dwReservedBits: array[0..2] of DWORD;
...
type
  ...
  { removed from dx80
  const
  DXVA_MAX_URIsize = 1024;

  type
  TDXVA_EncryptProt1Data_Sign512_Accel512 = record
       header : TDXVA_EncryptProtocolHeader;
       dwSigningModulusSize : LongWord;           // equal to 64
       SigningModulusValue : array [0..63] of BYTE;
       dwSigningExponentSize : LongWord;          // equal to 64
       SigningExponentValue : array [0..63] of BYTE;
       dwAccelModulusSize : LongWord;             // equal to 64
       AccelModulusValue : array [0..63] of BYTE;
       dwAccelExponentSize : LongWord;            // equal to 64
       AccelExponentValue : array [0..63] of BYTE;
       dwEncryptionFrequency : LongWord;
       dwURIsize : LongWord;
       URIstring : array [0..DXVA_MAX_URIsize-1] of WideChar;
       Signature : array [0..63] of BYTE;
       end;
  PDXVA_EncryptProt1Data_Sign512_Accel512 = ^TDXVA_EncryptProt1Data_Sign512_Accel512;
  }
  //#pragma pack(push, 16)
...
type
  ...
  TBDA_TEMPLATE_CONNECTION = record
    FromNodeType: ULONG;
  ...
  TBDA_TEMPLATE_PIN_JOINT = record
  ...
  TBDA_IPv6_ADDRESS = record
    rgbAddress: array[0..5] of BYTE;
  end;

  PBDA_IPv6_ADDRESS_LIST = ^TBDA_IPv6_ADDRESS_LIST;
  TBDA_IPv6_ADDRESS_LIST = record
    ulcAddresses: ULONG;
...
type
  ...
  TMPEG2_TRANSPORT_STRIDE = record
    dwOffset: DWORD;
...
type
  TBinaryConvolutionCodeRate = TOleEnum;
const
...
type
  ...
  PBDA_CAT_DESCRIPTOR = ^TBDA_CAT_DESCRIPTOR;
  TBDA_CAT_DESCRIPTOR = record
    ulTag: ULONG;
    ulcbLength: ULONG;
    argbData: array[0..MIN_DIMENSION - 1] of ULONG;
  end;

  PBDA_ES_DESCRIPTOR = ^TBDA_ES_DESCRIPTOR;
  TBDA_ES_DESCRIPTOR = record
    ulProgramNum: ULONG;
    ulContentPID: ULONG;
    ulTag: ULONG;
    ulcbLength: ULONG;
  ...
  PBDA_PROGRAM_DESCRIPTOR = ^TBDA_PROGRAM_DESCRIPTOR;
  TBDA_PROGRAM_DESCRIPTOR = record
    ulProgramNum: ULONG;
    ulTag: ULONG;
    ulcbLength: ULONG;
    argbData: array[0..MIN_DIMENSION - 1] of ULONG;
  ...
  PBDA_ES_MAP_STATE = ^TBDA_ES_MAP_STATE;
  TBDA_ES_MAP_STATE = (
    ES_MAP_STATE_GOOD_FLOW,
    ES_MAP_STATE_NO_CONTENT,
    ES_MAP_STATE_NO_ECM,
    ES_MAP_STATE_BAD_MAP
    );

  PBDA_ES_MAP = ^TBDA_ES_MAP;
  TBDA_ES_MAP = record
    ulContentPID: ULONG;
    ulProgramNum: ULONG;
    ulStreamType: ULONG;
    mapState: TBDA_ES_MAP_STATE;
  end;
...
type
  ...
  // Tuner library CoCLASS
  CoAnalogRadioTuningSpace = class
    class function Create: IAnalogRadioTuningSpace;
    class function CreateRemote(const MachineName: string): IAnalogRadioTuningSpace;
  end;

  CoAnalogTVTuningSpace = class
    class function Create: IAnalogTVTuningSpace;
    class function CreateRemote(const MachineName: string): IAnalogTVTuningSpace;
  end;

  CoATSCChannelTuneRequest = class
    class function Create: IATSCChannelTuneRequest;
    class function CreateRemote(const MachineName: string): IATSCChannelTuneRequest;
  end;

  CoATSCComponentType = class
    class function Create: IATSCComponentType;
    class function CreateRemote(const MachineName: string): IATSCComponentType;
  end;

  CoATSCLocator = class
    class function Create: IATSCLocator;
    class function CreateRemote(const MachineName: string): IATSCLocator;
  end;

  CoATSCTuningSpace = class
    class function Create: IATSCTuningSpace;
    class function CreateRemote(const MachineName: string): IATSCTuningSpace;
  end;

  CoChannelTuneRequest = class
    class function Create: IChannelTuneRequest;
    class function CreateRemote(const MachineName: string): IChannelTuneRequest;
  end;
  ...
  CoComponents = class
    class function Create: IComponents;
    class function CreateRemote(const MachineName: string): IComponents;
  end;

  CoComponentType = class
    class function Create: IComponentType;
    class function CreateRemote(const MachineName: string): IComponentType;
  end;

  CoComponentTypes = class
    class function Create: IComponentTypes;
    class function CreateRemote(const MachineName: string): IComponentTypes;
  end;

  CoCreatePropBagOnRegKey = class
    class function Create: ICreatePropBagOnRegKey;
    class function CreateRemote(const MachineName: string): ICreatePropBagOnRegKey;
  end;

  CoDVBCLocator = class
    class function Create: ILocator;
    class function CreateRemote(const MachineName: string): ILocator;
  end;

  CoDVBSLocator = class
    class function Create: IDVBSLocator;
    class function CreateRemote(const MachineName: string): IDVBSLocator;
  end;

  CoDVBTLocator = class
    class function Create: IDVBTLocator;
    class function CreateRemote(const MachineName: string): IDVBTLocator;
  end;

  CoDVBTuneRequest = class
    class function Create: IDVBTuneRequest;
    class function CreateRemote(const MachineName: string): IDVBTuneRequest;
  end;

  CoLanguageComponentType = class
    class function Create: ILanguageComponentType;
    class function CreateRemote(const MachineName: string): ILanguageComponentType;
  end;
  ...
  CoMPEG2Component = class
    class function Create: IMPEG2Component;
    class function CreateRemote(const MachineName: string): IMPEG2Component;
  end;

  CoMPEG2ComponentType = class
    class function Create: IMPEG2ComponentType;
    class function CreateRemote(const MachineName: string): IMPEG2ComponentType;
  end;

  CoSystemTuningSpaces = class
    class function Create: ITuningSpaceContainer;
    class function CreateRemote(const MachineName: string): ITuningSpaceContainer;
  end;

  CoTuneRequest = class
    class function Create: ITuneRequest;
    class function CreateRemote(const MachineName: string): ITuneRequest;
  end;

  CoTuningSpace = class
    class function Create: ITuningSpace;
    class function CreateRemote(const MachineName: string): ITuningSpace;
  end;
...
type
  ...
  ITunerEvents = interface(IUnknown)
    ['{68481420-0280-11d3-9D8E-00C04F72D980}']
    function OnTune(Request: ITuneRequest; Strength: longint): HRESULT; stdcall;
  end;

  ISignalEvents = interface(IUnknown)
    ['{85E2439E-0E23-11d3-9D8E-00C04F72D980}']
    function OnSignal(out Strength: longint): HRESULT; stdcall;
  end;
...
type
  ...
  CoDXTransformFactory = class
    class function Create: IDXTransformFactory;
    class function CreateRemote(const MachineName: string): IDXTransformFactory;
  end;

  CoDXTaskManager = class
    class function Create: IDXTaskManager;
    class function CreateRemote(const MachineName: string): IDXTaskManager;
  end;

  CoDXTScale = class
    class function Create: IDXTScale;
    class function CreateRemote(const MachineName: string): IDXTScale;
  end;

  CoDXSurface = class
    class function Create: IDXSurface;
    class function CreateRemote(const MachineName: string): IDXSurface;
  end;

  CoDXSurfaceModifier = class
    class function Create: IDXSurfaceModifier;
    class function CreateRemote(const MachineName: string): IDXSurfaceModifier;
  end;

  CoDXGradient = class
    class function Create: IDXGradient;
    class function CreateRemote(const MachineName: string): IDXGradient;
  end;
...
type
  TDEXTER_AUDIO_VOLUMEENVELOPE = record
    rtEnd: TREFERENCE_TIME;
    dLevel: double;
    bMethod: BOOL;
  end;
...
type
  ...
  IPropertySetter = interface(IUnknown)
    ...
    function PrintXMLW(out pszXML: WideChar; cchXML: integer; out pcchPrinted: integer; indent: integer): HResult; stdcall;
  end;
...
type
  //CoClasses
  CoAMTimeline = class
    class function Create: IAMTimeline;
    class function CreateRemote(const MachineName: string): IAMTimeline;
  end;

  CoAMTimelineComp = class
    class function Create: IAMTimelineComp;
    class function CreateRemote(const MachineName: string): IAMTimelineComp;
  end;

  CoAMTimelineEffect = class
    class function Create: IAMTimelineEffect;
    class function CreateRemote(const MachineName: string): IAMTimelineEffect;
  end;

  CoAMTimelineGroup = class
    class function Create: IAMTimelineGroup;
    class function CreateRemote(const MachineName: string): IAMTimelineGroup;
  end;

  CoAMTimelineObj = class
    class function Create: IAMTimelineObj;
    class function CreateRemote(const MachineName: string): IAMTimelineObj;
  end;

  CoAMTimelineSrc = class
    class function Create: IAMTimelineSrc;
    class function CreateRemote(const MachineName: string): IAMTimelineSrc;
  end;

  CoAMTimelineTrack = class
    class function Create: IAMTimelineTrack;
    class function CreateRemote(const MachineName: string): IAMTimelineTrack;
  end;

  CoAMTimelineTrans = class
    class function Create: IAMTimelineTrans;
    class function CreateRemote(const MachineName: string): IAMTimelineTrans;
  end;
  ...
  CoDxtAlphaSetter = class
    class function Create: IDxtAlphaSetter;
    class function CreateRemote(const MachineName: string): IDxtAlphaSetter;
  end;

  CoDxtCompositor = class
    class function Create: IDxtCompositor;
    class function CreateRemote(const MachineName: string): IDxtCompositor;
  end;

  CoDxtJpeg = class
    class function Create: IDxtJpeg;
    class function CreateRemote(const MachineName: string): IDxtJpeg;
  end;

  CoDxtKey = class
    class function Create: IDxtKey;
    class function CreateRemote(const MachineName: string): IDxtKey;
  end;

  CoMediaDet = class
    class function Create: IMediaDet;
    class function CreateRemote(const MachineName: string): IMediaDet;
  end;

  CoMediaLocator = class
    class function Create: IMediaLocator;
    class function CreateRemote(const MachineName: string): IMediaLocator;
  end;
  ...
  CoPropertySetter = class
    class function Create: IPropertySetter;
    class function CreateRemote(const MachineName: string): IPropertySetter;
  end;

  CoRenderEngine = class
    class function Create: IRenderEngine;
    class function CreateRemote(const MachineName: string): IRenderEngine;
  end;

  CoSampleGrabber = class
    class function Create: ISampleGrabber;
    class function CreateRemote(const MachineName: string): ISampleGrabber;
  end;

  CoSmartRenderEngine = class
    class function Create: IRenderEngine;
    class function CreateRemote(const MachineName: string): IRenderEngine;
  end;

  CoXml2Dex = class
    class function Create: IXml2Dex;
    class function CreateRemote(const MachineName: string): IXml2Dex;
  end;
...
type
  ...
  //look at GetCurrentBuffer(pBuffer) pbuffer: pointer
  ISampleGrabber = interface(IUnknown)
    ['{6B652FFF-11FE-4FCE-92AD-0266B5D7C78F}']
    // set this to have the filter immediate stop after
    // garnishing a sample
...
type
  ...
  TDMO_INPUT_DATA_BUFFER_FLAGS = TOleEnum;
const
...
type
  TDMO_OUTPUT_DATA_BUFFER_FLAGS = TOleEnum;
const
...
type
  TDMO_INPUT_STATUS_FLAGS = TOleEnum;
const
...
type
  TDMO_INPUT_STREAM_INFO_FLAGS = TOleEnum;
const
...
type
  TDMO_OUTPUT_STREAM_INFO_FLAGS = TOleEnum;
const
...
type
  TDMO_PROCESS_OUTPUT_FLAGS = TOleEnum;
const
...
type
  ...
  TDMO_INPLACE_PROCESS_FLAGS = TOleEnum;
const
...
type
  ...
  TDMO_QUALITY_STATUS_FLAGS = TOleEnum;
const
...
type
  ...
  TDMO_VIDEO_OUTPUT_STREAM_FLAGS = TOleEnum;
const
...
type
  TKS_SEEKING_CAPABILITIES = TOleEnum;
const
...
type
  ...
  TALLOCATOR_PROPERTIES_EX = record
    cBuffers: longint;
    cbBuffer: longint;
    cbAlign: longint;
    cbPrefix: longint;
...
type
  ...
  TKSDATAFORMAT_WAVEFORMATEX = record
...
type
  TKSPROPERTY_BIBLIOGRAPHIC = TOleEnum;
const
...
type
  ...
  TKSAUDIO_STEREO_ENHANCE = record
...
type
  ...
  TKS_AnalogVideoStandard = TOleEnum;
const
...
type
  ...
  TKSPROPERTY_TUNER_MODES = TOleEnum;
const
...
type
  ...
  TKS_VIDEODECODER_FLAGS = TOleEnum;
const
...
type
  ...
  TKS_PhysicalConnectorType = TOleEnum;
const
...
type
  TKSPROPERTY_OVERLAYUPDATE = TOleEnum;
const
...
type
  ...
  TKS_VideoControlFlags = TOleEnum;
const
...
type
  ...
  TKS_DATARANGE_BDA_ANTENNA = record
    DataRange: TKSDATARANGE;
  ...
  TKS_DATARANGE_BDA_TRANSPORT = record
    DataRange: TKSDATARANGE;
...
type
  TKSPROPERTY_BDA_AUTODEMODULATE = (
  ...
  //
const
  KSPROPSETID_BdaPmtProcessor: TGUID = '{516B99C5-971C-4aaf-B3F3-D9FDA8A15E16}';

type
  TKSPROPERTY_BDA_PMTPROCESSOR = (
  ...
  //
const
  KSPROPSETID_BdaDescriptor: TGUID = '{DE6A1C9E-87DF-48aa-95AA-289B7AA4F49F}';

type
  TKSPROPERTY_BDA_DESCRIPTOR = (
  ...
  //
const
  KSPROPSETID_BdaEsMap: TGUID = '{FA14A8B3-6068-48ef-96DD-53010B985A7D}';

type
  TKSPROPERTY_BDA_ESMAP = (
...
const
  ...
  IID_IBDA_SignalStatistics: TGUID = '{1347D106-CF3A-428a-A5CB-AC0D9A2A4338}'; // XP
  ...
  IID_IBDA_LNBInfo: TGUID = '{992CF102-49F9-4719-A664-C4F23E2408F4}'; // XP
  ...
  IID_IBDA_IPSinkInfo: TGUID = '{A750108F-492E-4d51-95F7-649B23FF7AD7}'; // XP
...
const
  ...
  IID_IBDA_PinControl: TGUID = '{0DED49D5-A8B7-4d5d-97A1-12B0C195874D}';
  IID_IBDA_SignalProperties: TGUID = '{D2F1644B-B409-11d2-BC69-00A0C9EE9E16}';
  IID_IBDA_Topology: TGUID = '{A14EE835-0A23-11d3-9CC7-00C04F7971E0}';
  IID_IBDA_VoidTransform: TGUID = '{71985F46-1CA1-11d3-9CC8-00C04F7971E0}';
  IID_IBDA_NullTransform: TGUID = '{DDF15B0D-BD25-11d2-9CA0-00C04F7971E0}';
  IID_IBDA_FrequencyFilter: TGUID = '{71985F47-1CA1-11d3-9CC8-00C04F7971E0}';
  IID_IBDA_AutoDemodulate: TGUID = '{DDF15B12-BD25-11d2-9CA0-00C04F7971E0}';
...
type
  ...
  IBDA_Topology = interface(IUnknown)
    ...
    function GetNodeTypes(var pulcNodeTypes: ULONG; ulcNodeTypesMax: ULONG;
      var rgulNodeTypes: ULONG): HRESULT; stdcall;
    function GetNodeInterfaces(ulNodeType: ULONG; var pulcInterfaces: ULONG;
      ulcInterfacesMax: ULONG; var rgguidInterfaces: TGUID): HRESULT; stdcall;
    function GetPinTypes(var pulcPinTypes: ULONG; ulcPinTypesMax: ULONG;
      var rgulPinTypes: ULONG): HRESULT; stdcall;
    function GetTemplateConnections(var pulcConnections: ULONG; ulcConnectionsMax: ULONG;
      var rgConnections: TBDA_TEMPLATE_CONNECTION): HRESULT; stdcall;
  ...
  IBDA_FrequencyFilter = interface(IUnknown)
    ['{71985F47-1CA1-11d3-9CC8-00C04F7971E0}']
    function put_Autotune(var pulTransponder: ULONG): HRESULT; stdcall;
    function put_Frequency(var pulFrequency: ULONG): HRESULT; stdcall;
    function get_Frequency(out pulFrequency: ULONG): HRESULT; stdcall;
    function put_Polarity(var pulPolarity: ULONG): HRESULT; stdcall;
    function get_Polarity(out pulPolarity: ULONG): HRESULT; stdcall;
    function put_Range(var pulRange: ULONG): HRESULT; stdcall;
...
class function CoSystemTuningSpaces.Create: ITuningSpaceContainer;
begin
  Result := CreateComObject(CLSID_SystemTuningSpaces) as ITuningSpaceContainer;
end;

class function CoSystemTuningSpaces.CreateRemote(const MachineName: string): ITuningSpaceContainer;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_SystemTuningSpaces) as ITuningSpaceContainer;
end;

class function CoTuningSpace.Create: ITuningSpace;
begin
  Result := CreateComObject(CLSID_TuningSpace) as ITuningSpace;
end;

class function CoTuningSpace.CreateRemote(const MachineName: string): ITuningSpace;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_TuningSpace) as ITuningSpace;
end;

class function CoATSCTuningSpace.Create: IATSCTuningSpace;
begin
  Result := CreateComObject(CLSID_ATSCTuningSpace) as IATSCTuningSpace;
end;

class function CoATSCTuningSpace.CreateRemote(const MachineName: string): IATSCTuningSpace;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ATSCTuningSpace) as IATSCTuningSpace;
end;

class function CoAnalogRadioTuningSpace.Create: IAnalogRadioTuningSpace;
begin
  Result := CreateComObject(CLSID_AnalogRadioTuningSpace) as IAnalogRadioTuningSpace;
end;

class function CoAnalogRadioTuningSpace.CreateRemote(const MachineName: string): IAnalogRadioTuningSpace;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AnalogRadioTuningSpace) as IAnalogRadioTuningSpace;
end;

class function CoAnalogTVTuningSpace.Create: IAnalogTVTuningSpace;
begin
  Result := CreateComObject(CLSID_AnalogTVTuningSpace) as IAnalogTVTuningSpace;
end;

class function CoAnalogTVTuningSpace.CreateRemote(const MachineName: string): IAnalogTVTuningSpace;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AnalogTVTuningSpace) as IAnalogTVTuningSpace;
end;

class function CoComponentTypes.Create: IComponentTypes;
begin
  Result := CreateComObject(CLSID_ComponentTypes) as IComponentTypes;
end;

class function CoComponentTypes.CreateRemote(const MachineName: string): IComponentTypes;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ComponentTypes) as IComponentTypes;
end;

class function CoComponentType.Create: IComponentType;
begin
  Result := CreateComObject(CLSID_ComponentType) as IComponentType;
end;

class function CoComponentType.CreateRemote(const MachineName: string): IComponentType;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ComponentType) as IComponentType;
end;

class function CoLanguageComponentType.Create: ILanguageComponentType;
begin
  Result := CreateComObject(CLSID_LanguageComponentType) as ILanguageComponentType;
end;

class function CoLanguageComponentType.CreateRemote(const MachineName: string): ILanguageComponentType;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_LanguageComponentType) as ILanguageComponentType;
end;

class function CoMPEG2ComponentType.Create: IMPEG2ComponentType;
begin
  Result := CreateComObject(CLSID_MPEG2ComponentType) as IMPEG2ComponentType;
end;

class function CoMPEG2ComponentType.CreateRemote(const MachineName: string): IMPEG2ComponentType;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_MPEG2ComponentType) as IMPEG2ComponentType;
end;

class function CoATSCComponentType.Create: IATSCComponentType;
begin
  Result := CreateComObject(CLSID_ATSCComponentType) as IATSCComponentType;
end;

class function CoATSCComponentType.CreateRemote(const MachineName: string): IATSCComponentType;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ATSCComponentType) as IATSCComponentType;
end;

class function CoComponents.Create: IComponents;
begin
  Result := CreateComObject(CLSID_Components) as IComponents;
end;

class function CoComponents.CreateRemote(const MachineName: string): IComponents;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_Components) as IComponents;
end;

class function CoComponent.Create: IComponent;
begin
  Result := CreateComObject(CLSID_Component) as IComponent;
end;

class function CoComponent.CreateRemote(const MachineName: string): IComponent;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_Component) as IComponent;
end;

class function CoMPEG2Component.Create: IMPEG2Component;
begin
  Result := CreateComObject(CLSID_MPEG2Component) as IMPEG2Component;
end;

class function CoMPEG2Component.CreateRemote(const MachineName: string): IMPEG2Component;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_MPEG2Component) as IMPEG2Component;
end;

class function CoTuneRequest.Create: ITuneRequest;
begin
  Result := CreateComObject(CLSID_TuneRequest) as ITuneRequest;
end;

class function CoTuneRequest.CreateRemote(const MachineName: string): ITuneRequest;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_TuneRequest) as ITuneRequest;
end;

class function CoChannelTuneRequest.Create: IChannelTuneRequest;
begin
  Result := CreateComObject(CLSID_ChannelTuneRequest) as IChannelTuneRequest;
end;

class function CoChannelTuneRequest.CreateRemote(const MachineName: string): IChannelTuneRequest;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ChannelTuneRequest) as IChannelTuneRequest;
end;

class function CoATSCChannelTuneRequest.Create: IATSCChannelTuneRequest;
begin
  Result := CreateComObject(CLSID_ATSCChannelTuneRequest) as IATSCChannelTuneRequest;
end;

class function CoATSCChannelTuneRequest.CreateRemote(const MachineName: string): IATSCChannelTuneRequest;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ATSCChannelTuneRequest) as IATSCChannelTuneRequest;
end;

class function CoLocator.Create: ILocator;
begin
  Result := CreateComObject(CLSID_Locator) as ILocator;
end;

class function CoLocator.CreateRemote(const MachineName: string): ILocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_Locator) as ILocator;
end;

class function CoATSCLocator.Create: IATSCLocator;
begin
  Result := CreateComObject(CLSID_ATSCLocator) as IATSCLocator;
end;

class function CoATSCLocator.CreateRemote(const MachineName: string): IATSCLocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ATSCLocator) as IATSCLocator;
end;

class function CoDVBTLocator.Create: IDVBTLocator;
begin
  Result := CreateComObject(CLSID_DVBTLocator) as IDVBTLocator;
end;

class function CoDVBTLocator.CreateRemote(const MachineName: string): IDVBTLocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DVBTLocator) as IDVBTLocator;
end;

class function CoDVBSLocator.Create: IDVBSLocator;
begin
  Result := CreateComObject(CLSID_DVBSLocator) as IDVBSLocator;
end;

class function CoDVBSLocator.CreateRemote(const MachineName: string): IDVBSLocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DVBSLocator) as IDVBSLocator;
end;

class function CoDVBCLocator.Create: ILocator;
begin
  Result := CreateComObject(CLSID_DVBCLocator) as ILocator;
end;

class function CoDVBCLocator.CreateRemote(const MachineName: string): ILocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DVBCLocator) as ILocator;
end;

class function CoDVBTuneRequest.Create: IDVBTuneRequest;
begin
  Result := CreateComObject(CLSID_DVBTuneRequest) as IDVBTuneRequest;
end;

class function CoDVBTuneRequest.CreateRemote(const MachineName: string): IDVBTuneRequest;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DVBTuneRequest) as IDVBTuneRequest;
end;

class function CoCreatePropBagOnRegKey.Create: ICreatePropBagOnRegKey;
begin
  Result := CreateComObject(CLSID_CreatePropBagOnRegKey) as ICreatePropBagOnRegKey;
end;

class function CoCreatePropBagOnRegKey.CreateRemote(const MachineName: string): ICreatePropBagOnRegKey;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_CreatePropBagOnRegKey) as ICreatePropBagOnRegKey;
end;
// DirectX Transform

class function CoDXTransformFactory.Create: IDXTransformFactory;
begin
  Result := CreateComObject(CLSID_DXTransformFactory) as IDXTransformFactory;
end;

class function CoDXTransformFactory.CreateRemote(const MachineName: string): IDXTransformFactory;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXTransformFactory) as IDXTransformFactory;
end;

class function CoDXTaskManager.Create: IDXTaskManager;
begin
  Result := CreateComObject(CLSID_DXTaskManager) as IDXTaskManager;
end;

class function CoDXTaskManager.CreateRemote(const MachineName: string): IDXTaskManager;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXTaskManager) as IDXTaskManager;
end;

class function CoDXTScale.Create: IDXTScale;
begin
  Result := CreateComObject(CLSID_DXTScale) as IDXTScale;
end;

class function CoDXTScale.CreateRemote(const MachineName: string): IDXTScale;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXTScale) as IDXTScale;
end;

class function CoDXSurface.Create: IDXSurface;
begin
  Result := CreateComObject(CLSID_DXSurface) as IDXSurface;
end;

class function CoDXSurface.CreateRemote(const MachineName: string): IDXSurface;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXSurface) as IDXSurface;
end;

class function CoDXSurfaceModifier.Create: IDXSurfaceModifier;
begin
  Result := CreateComObject(CLSID_DXSurfaceModifier) as IDXSurfaceModifier;
end;

class function CoDXSurfaceModifier.CreateRemote(const MachineName: string): IDXSurfaceModifier;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXSurfaceModifier) as IDXSurfaceModifier;
end;

class function CoDXGradient.Create: IDXGradient;
begin
  Result := CreateComObject(CLSID_DXGradient) as IDXGradient;
end;

class function CoDXGradient.CreateRemote(const MachineName: string): IDXGradient;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DXGradient) as IDXGradient;
end;

// Dexter Library

class function CoAMTimeline.Create: IAMTimeline;
begin
  Result := CreateComObject(CLSID_AMTimeline) as IAMTimeline;
end;

class function CoAMTimeline.CreateRemote(const MachineName: string): IAMTimeline;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimeline) as IAMTimeline;
end;

class function CoAMTimelineObj.Create: IAMTimelineObj;
begin
  Result := CreateComObject(CLSID_AMTimelineObj) as IAMTimelineObj;
end;

class function CoAMTimelineObj.CreateRemote(const MachineName: string): IAMTimelineObj;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineObj) as IAMTimelineObj;
end;

class function CoAMTimelineSrc.Create: IAMTimelineSrc;
begin
  Result := CreateComObject(CLSID_AMTimelineSrc) as IAMTimelineSrc;
end;

class function CoAMTimelineSrc.CreateRemote(const MachineName: string): IAMTimelineSrc;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineSrc) as IAMTimelineSrc;
end;

class function CoAMTimelineTrack.Create: IAMTimelineTrack;
begin
  Result := CreateComObject(CLSID_AMTimelineTrack) as IAMTimelineTrack;
end;

class function CoAMTimelineTrack.CreateRemote(const MachineName: string): IAMTimelineTrack;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineTrack) as IAMTimelineTrack;
end;

class function CoAMTimelineComp.Create: IAMTimelineComp;
begin
  Result := CreateComObject(CLSID_AMTimelineComp) as IAMTimelineComp;
end;

class function CoAMTimelineComp.CreateRemote(const MachineName: string): IAMTimelineComp;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineComp) as IAMTimelineComp;
end;

class function CoAMTimelineGroup.Create: IAMTimelineGroup;
begin
  Result := CreateComObject(CLSID_AMTimelineGroup) as IAMTimelineGroup;
end;

class function CoAMTimelineGroup.CreateRemote(const MachineName: string): IAMTimelineGroup;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineGroup) as IAMTimelineGroup;
end;

class function CoAMTimelineTrans.Create: IAMTimelineTrans;
begin
  Result := CreateComObject(CLSID_AMTimelineTrans) as IAMTimelineTrans;
end;

class function CoAMTimelineTrans.CreateRemote(const MachineName: string): IAMTimelineTrans;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineTrans) as IAMTimelineTrans;
end;

class function CoAMTimelineEffect.Create: IAMTimelineEffect;
begin
  Result := CreateComObject(CLSID_AMTimelineEffect) as IAMTimelineEffect;
end;

class function CoAMTimelineEffect.CreateRemote(const MachineName: string): IAMTimelineEffect;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AMTimelineEffect) as IAMTimelineEffect;
end;

class function CoRenderEngine.Create: IRenderEngine;
begin
  Result := CreateComObject(CLSID_RenderEngine) as IRenderEngine;
end;

class function CoRenderEngine.CreateRemote(const MachineName: string): IRenderEngine;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_RenderEngine) as IRenderEngine;
end;

class function CoSmartRenderEngine.Create: IRenderEngine;
begin
  Result := CreateComObject(CLSID_SmartRenderEngine) as IRenderEngine;
end;

class function CoSmartRenderEngine.CreateRemote(const MachineName: string): IRenderEngine;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_SmartRenderEngine) as IRenderEngine;
end;

class function CoAudMixer.Create: IBaseFilter;
begin
  Result := CreateComObject(CLSID_AudMixer) as IBaseFilter;
end;

class function CoAudMixer.CreateRemote(const MachineName: string): IBaseFilter;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_AudMixer) as IBaseFilter;
end;

class function CoXml2Dex.Create: IXml2Dex;
begin
  Result := CreateComObject(CLSID_Xml2Dex) as IXml2Dex;
end;

class function CoXml2Dex.CreateRemote(const MachineName: string): IXml2Dex;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_Xml2Dex) as IXml2Dex;
end;

class function CoMediaLocator.Create: IMediaLocator;
begin
  Result := CreateComObject(CLSID_MediaLocator) as IMediaLocator;
end;

class function CoMediaLocator.CreateRemote(const MachineName: string): IMediaLocator;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_MediaLocator) as IMediaLocator;
end;

class function CoPropertySetter.Create: IPropertySetter;
begin
  Result := CreateComObject(CLSID_PropertySetter) as IPropertySetter;
end;

class function CoPropertySetter.CreateRemote(const MachineName: string): IPropertySetter;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_PropertySetter) as IPropertySetter;
end;

class function CoMediaDet.Create: IMediaDet;
begin
  Result := CreateComObject(CLSID_MediaDet) as IMediaDet;
end;

class function CoMediaDet.CreateRemote(const MachineName: string): IMediaDet;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_MediaDet) as IMediaDet;
end;

class function CoSampleGrabber.Create: ISampleGrabber;
begin
  Result := CreateComObject(CLSID_SampleGrabber) as ISampleGrabber;
end;

class function CoSampleGrabber.CreateRemote(const MachineName: string): ISampleGrabber;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_SampleGrabber) as ISampleGrabber;
end;
...
class function CoDxtCompositor.Create: IDxtCompositor;
begin
  Result := CreateComObject(CLSID_DxtCompositor) as IDxtCompositor;
end;

class function CoDxtCompositor.CreateRemote(const MachineName: string): IDxtCompositor;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DxtCompositor) as IDxtCompositor;
end;

class function CoDxtAlphaSetter.Create: IDxtAlphaSetter;
begin
  Result := CreateComObject(CLSID_DxtAlphaSetter) as IDxtAlphaSetter;
end;

class function CoDxtAlphaSetter.CreateRemote(const MachineName: string): IDxtAlphaSetter;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DxtAlphaSetter) as IDxtAlphaSetter;
end;

class function CoDxtJpeg.Create: IDxtJpeg;
begin
  Result := CreateComObject(CLSID_DxtJpeg) as IDxtJpeg;
end;

class function CoDxtJpeg.CreateRemote(const MachineName: string): IDxtJpeg;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DxtJpeg) as IDxtJpeg;
end;

class function CoColorSource.Create: IBaseFilter;
begin
  Result := CreateComObject(CLSID_ColorSource) as IBaseFilter;
end;

class function CoColorSource.CreateRemote(const MachineName: string): IBaseFilter;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_ColorSource) as IBaseFilter;
end;

class function CoDxtKey.Create: IDxtKey;
begin
  Result := CreateComObject(CLSID_DxtKey) as IDxtKey;
end;

class function CoDxtKey.CreateRemote(const MachineName: string): IDxtKey;
begin
  Result := CreateRemoteComObject(MachineName, CLSID_DxtKey) as IDxtKey;
end;
...
begin
  if (not InputTypeSet(ulInputStreamIndex)) then
    Result := nil
  else
    Result := @m_InputInfo[ulInputStreamIndex].CurrentMediaType;
end;

function TMediaObjectImpl.OutputType(ulOutputStreamIndex: DWORD): PDMO_MEDIA_TYPE;
begin
  if (not OutputTypeSet(ulOutputStreamIndex)) then
    Result := nil
  else
    Result := @m_OutputInfo[ulOutputStreamIndex].CurrentMediaType;
end;
...
begin
  ...
  if (InputTypeSet(ulStreamIndex))
    then
    Result := MoCopyMediaType(pmt, @m_InputInfo[ulStreamIndex].CurrentMediaType)
  else
    {$IFDEF VER120}
...
begin
  ...
  if (OutputTypeSet(ulStreamIndex))
    then
    Result := MoCopyMediaType(pmt, @m_OutputInfo[ulStreamIndex].CurrentMediaType)
      {$IFDEF VER120}
  else
    Result := DMO_E_TYPE_NOT_SET;
  {$ELSE}
...
begin
  ...
  if FAILED(Result) or (Result = S_FALSE) then
    Exit;
...
begin
  ...
  begin
    if ((pOutputBuffers[dw].dwStatus and DMO_OUTPUT_DATA_BUFFERF_INCOMPLETE) <> 0)
      then
      m_OutputInfo[dw].fIncomplete := Integer(True)
    else
      m_OutputInfo[dw].fIncomplete := Integer(False);
  end;
end;
...
finalization
  begin
    ...
    if KSUSERDLL <> 0 then
      FreeLibrary(KSUSERDLL);
    if KSPROXYAX <> 0 then
      FreeLibrary(KSPROXYAX);
  end;
