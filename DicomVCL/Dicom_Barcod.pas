...
type
  ...
  published
   ...
   { Height of Barcode (Pixel)}
    ...
    property CheckSumMethod: TCheckSumMethod read FCheckSumMethod write FCheckSumMethod default
      csmModulo10;
...
uses WinProcs, WinTypes, SysUtils, math, printers;
...
constructor TDicomBarcode.Create;
begin
  FAngle := 0.0;
  FRatio := 2.0;
  FModul := 1;
  FTyp := bcCode_2_5_matrix;
  FCheckSum := FALSE;
  ...
  FDescription := 'ÕÅÈý·á ÄÐ 33Y';
  FText := '';
end;

destructor TDicomBarcode.Destroy;
begin
  FShowTextFont.Free;
  inherited;
end;

function TDicomBarcode.GetTypText: string;
begin
...
procedure TDicomBarcode.SetModul(v: integer);
begin
...
procedure TDicomBarcode.OneBarProps(code: char; var Width: integer; var lt: TBarLineType);
begin
  case code of
    '0':
      begin width := modules[0];
        lt := white;
      end;
    '1':
      begin width := modules[1];
        lt := white;
      end;
    '2':
      begin width := modules[2];
        lt := white;
      end;
    '3':
      begin width := modules[3];
        lt := white;
      end;
...
function TDicomBarcode.MakeData: string;
var
  i: integer;
begin
...
function TDicomBarcode.GetWidth: integer;
var
  data: string;
  i: integer;
  w: integer;
...
procedure TDicomBarcode.SetWidth(Value: integer);
var
  data: string;
  i: integer;
  w, wtotal: integer;
...
function TDicomBarcode.DoCheckSumming(const data: string): string;
begin
...
function TDicomBarcode.Code_EAN8: string;
var
  i: integer;
  tmp: string;
begin
...
function TDicomBarcode.Code_EAN13: string;
var
  i, LK: integer;
  tmp: string;
begin
...
function TDicomBarcode.Code_2_5_interleaved: string;
var
  i, j: integer;
  c: char;
...
function TDicomBarcode.Code_2_5_industrial: string;
var
  i, j: integer;
begin
...
function TDicomBarcode.Code_2_5_matrix: string;
var
  i, j: integer;
  c: char;
begin
...
function TDicomBarcode.Code_39: string;
...
function TDicomBarcode.Code_39Extended: string;
...
function TDicomBarcode.Code_128: string;
type
  TCode128 =
    record
    a, b: char;
...
function TDicomBarcode.Code_93: string;
type
  TCode93 =
    record
    c: char;
...
function TDicomBarcode.Code_93Extended: string;
const
...
function TDicomBarcode.Code_MSI: string;
const
...
function TDicomBarcode.Code_PostNet: string;
const
...
function TDicomBarcode.Code_Codabar: string;
type
  TCodabar =
    record
    c: char;
...
function TDicomBarcode.SetLen(pI: byte): string;
begin
...
function TDicomBarcode.Code_UPC_A: string;
var
  i: integer;
  tmp: string;
begin
...
function TDicomBarcode.Code_UPC_E0: string;
var
  i, j: integer;
  tmp: string;
  c: char;
...
function TDicomBarcode.Code_UPC_E1: string;
var
  i, j: integer;
  tmp: string;
  c: char;
...
function TDicomBarcode.Code_Supp5: string;
var
  i, j: integer;
  tmp: string;
  c: char;
...
function TDicomBarcode.Code_Supp2: string;
var
  i, j: integer;
  tmp, mS: string;
begin
...
procedure TDicomBarcode.MakeModules;
begin
  case Typ of
    bcCode_2_5_interleaved,
      bcCode_2_5_industrial,
...
procedure TDicomBarcode.DoLines(data: string; Canvas: TCanvas);
...
procedure TDicomBarcode.DrawBarcode(Canvas: TCanvas);
var
  data: string;
  SaveFont: TFont;
  SavePen: TPen;
...
procedure TDicomBarcode.DrawText(Canvas: TCanvas);
var
  PosX, PosY: Integer;
  SaveFont: TFont;
begin
    ...
    try
      ...
      try
        ...
        if ShowTextPosition in [stpTopLeft, stpTopCenter, stpTopRight] then
          ...
          PosY := FTop + Height - TextHeight(Text) + 3;
        ...
        if FDescription <> '' then
          TextOut(FLeft, FTop + Round(Font.Height * 1.5), FDescription);
      finally
...
procedure TDicomBarcode.DoChange;
begin
  if Assigned(FOnChange) then
    FOnChange(Self);
end;

procedure TDicomBarcode.SetRatio(const Value: Double);
begin
...
procedure TDicomBarcode.SetTyp(const Value: TBarcodeType);
begin
  if Value <> FTyp then
  begin
    FTyp := Value;
...
procedure TDicomBarcode.SetAngle(const Value: Double);
begin
...
procedure TDicomBarcode.SetText(const Value: string);
begin
  if Value <> FText then
  begin
    FText := Value;
...
procedure TDicomBarcode.SetShowText(const Value: TBarcodeOption);
begin
...
procedure TDicomBarcode.SetTop(const Value: Integer);
begin
  if Value <> FTop then
  begin
    FTop := Value;
...
procedure TDicomBarcode.SetLeft(const Value: Integer);
begin
  if Value <> FLeft then
  begin
    FLeft := Value;
...
procedure TDicomBarcode.SetCheckSum(const Value: Boolean);
begin
...
procedure TDicomBarcode.SetHeight(const Value: integer);
begin
...
function TDicomBarcode.GetCanvasHeight: Integer;
var
  alpha: Extended;
begin
  alpha := FAngle / 180.0 * pi;
...
function TDicomBarcode.GetCanvasWidth: Integer;
var
  alpha: Extended;
begin
  alpha := FAngle / 180.0 * pi;
...
procedure TDicomBarcode.SetShowTextFont(const Value: TFont);
begin
  FShowTextFont.Assign(Value);
  DoChange;
end;

procedure TDicomBarcode.SetShowTextPosition(const Value: TShowTextPosition);
begin
...
procedure TDicomBarcode.SetDescription(const Value: string);
begin
  FDescription := Value;
end;
