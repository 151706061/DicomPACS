...
begin
  if fScrollData.AmScrolling then
    StopScrolling;
  Clear;
  FLines.Free;
  FTrapezes.Free;
...
begin
  if mc = fMouseActionData.fMouseControl then
    Exit;
...
begin
  if csDesigning in ComponentState then
    Exit;
  if fMouseActionData.fMouseControl = mcNone then
    Exit;
  with fMouseActionData do
  begin
...
var
  DeltaX, DeltaY: LongInt; //how far did the mouse move?
...
begin
  ...
  if fMouseActionData.fMouseControl = mcNone then
    Exit;
  if not fMouseActionData.ExecuteAction then
    Exit;
  ...
  if not fMouseActionData.DidClear then
  begin
    ClearCanvas(Canvas);
    fMouseActionData.DidClear := True;
  end
  else
    DrawAxesNow(False, Canvas); //delete them
...
var
  i: LongInt;
begin
  for i := 0 to FTrapezes.Count - 1 do
  begin
    Dispose(FTrapezes[i]);
    FTrapezes[i] := nil;
  end;
...
begin
  if b = fDrawHiddenLines then
    Exit;
  fDrawHiddenLines := b;
  fHiddenLinesChanged := True;
  if fDrawHiddenLines then
    DrawTrapezes(Canvas)
  else
    Draw(Canvas, True);
end;
...
begin
  if (p.X = fOffset.X) and (p.Y = fOffset.Y) then
    Exit;
...
begin
  ...
  with fIsodoses[Ind] do
  begin
    Percentage := Percent;
    if Col <> clNone then
      Color := col; //allow unchanged color
    Active := True;
  end;
  fColorsChanged := True;
end;
...
begin
  if (L.NoOfValues <> fLineDim) then
  begin
    if (NoOfLines = 0)
      then
      fLineDim := L.NoOfValues
    else
      raise Exception.Create('Given line does not match: wrong length');
  end;
  FLines.Add(L);
  fDataChanged := True;
  fDidInit := False;
...
begin
  ...
  fScrollData.ScrollPaused := False;
  while fScrollData.AmScrolling and not fScrollData.ScrollPaused do
  begin
    ...
    if not MouseIsInMyClientArea then
      StopScrolling
    else
    begin
      Application.ProcessMessages;
      Sleep(fScrollData.Speed);
      if fScrollData.Incremental then
      begin
      ...
      if fScrollData.Position = fLines.Count then
        fScrollData.Position := 0;
    end;
  end;
end;
...
begin
  ...
  if Assigned(fOnStopScrolling) then
    fOnStopScrolling(Self);
end;
...
//    then Raise Exception.Create('Graph3D.BuildTrapezes: Not enough data, cannot init trapezes');
  ...
  begin
    ...
    begin
      ...
      if CurrentX > XMax then
        Xmax := CurrentX;
      if CurrentX < XMin then
        Xmin := CurrentX;
      if CurrentY > YMax then
        Ymax := CurrentY;
      if CurrentY < YMin then
        Ymin := CurrentY;
      if CurrentZ > ZMax then
        Zmax := CurrentZ;
      if CurrentZ < ZMin then
        Zmin := CurrentZ;
  ...
  if (Xmax = Xmin) or (Ymax = Ymin) or (Zmax = Zmin)
    then
    raise Exception.Create('Graph3D.BuildTrapezes: Bad values: no range');
  ...
  for j := 0 to fLines.Count - 2 do
  begin //need next line, too
    CurrentLine := TGraphLine(fLines[j]);
    NextLine := TGraphLine(fLines[j + 1]);
    for i := 0 to fLineDim - 2 do
    begin //need next point, too
      T := New(PTrapezeInfo);
      with T^ do
      begin
...
const
  Far_Away = 10000; //viewpoint must be far to get parallel projection
begin
  ...
  begin
    ...
    begin
      case fWaterLevelType of
        ...
        wlAbove:
          begin //force points up if need be
            for j := 0 to 3 do
            begin
              ...
              if Vals[j].Z < fWaterlevelPercent
                then
                WaterPoint.Z := fWaterlevelPercent
              else
                WaterPoint.Z := Vals[j].Z;
              Pixs[j] := Map(WaterPoint);
            end;
          end;
        wlBelow:
          begin //force points down if need be
            for j := 0 to 3 do
            begin
              WaterPoint.X := Vals[j].X;
              WaterPoint.Y := Vals[j].Y;
...
begin
  ...
  begin
    ...
    begin
      ...
      case fWaterLevelType of
        wlAbove: if Value < fWaterlevelPercent then
            Value := fWaterLevelPercent;
        wlBelow: if Value > fWaterlevelPercent then
            Value := fWaterLevelPercent;
      end;
      //find which isdose-color must get used: j
      for j := 0 to MaxNoOfIsodoses - 1 do
        if Value <= fIsodoses[j].Percentage then
          Break;
      if j = MaxNoOfIsodoses then
        Dec(j); //For-loop: j is undefined at end
...
var
  Xfirst, Yfirst, Zfirst: Boolean; //draw axes before trapezes?
...
//  n := GetTimeCount;
  ClearCanvas(CV);
  if fShowFrame then
    DrawFrame(CV); //looks better if it's visible while drawing
  DrawCaption(CV);
  if csDesigning in ComponentState then
    Exit;
//  n1 := GetTimeCount;
...
//  n2 := GetTimeCount;
  ...
  if fDrawAxes and XFirst then
    DrawXaxis(True, psDash, CV);
  if fDrawAxes and YFirst then
    DrawYaxis(True, psDash, CV);
  if fDrawAxes and ZFirst then
    DrawZaxis(True, psDash, CV);
  if fDrawAxes and XFar then
    DrawXFarAxis(True, psDash, CV);
  if fDrawAxes and YFar then
    DrawYFarAxis(True, psDash, CV);
  //now draw the trapezes:
//  n3 := GetTimeCount;
  DrawTrapezes(CV);
//  n4 := GetTimeCount;
  if fDrawAxes and not XFirst then
    DrawXaxis(True, psSolid, CV);
  if fDrawAxes and not YFirst then
    DrawYaxis(True, psSolid, CV);
  if fDrawAxes and not ZFirst then
    DrawZaxis(True, psSolid, CV);
  if fDrawAxes and not XFar then
    DrawXFarAxis(True, psSolid, CV);
  if fDrawAxes and not YFar then
    DrawYFarAxis(True, psSolid, CV);
  if fShowFrame then
    DrawFrame(Canvas); //was probably ruined by trapezes
{  n5 := GetTimeCount;
  ...
  if CallEvent and Assigned(fAfterDraw) then
    fAfterDraw(Self);
end;
...
begin
  ...
  begin
    ...
    if fDrawHiddenLines
      ...
      Brush.Style := bsClear;
    for i := fTrapezes.Count - 1 downto Round(fTrapezes.Count * (100 - fViewPercentage) / 100) do
    begin //furthest ones first
      Trapeze := PTrapezeInfo(fTrapezes[i])^;
      if (Trapeze.LineNo > fLastLine) or (Trapeze.LineNo < fFirstLine) then
        Continue;
...
//  TWinControl(Self).PaintTo(BM.Canvas.Handle, 0, 0);
  ...
  if inWhite and (Color <> clWhite) then
  begin
