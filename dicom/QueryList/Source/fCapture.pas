...
uses
  ...
  cxGrid, LcxDBGrid, Variants, ExtDlgs;
...
type
  TfrmCapture = class(TForm)
    ...
    procedure aLoadFromDiocmFileExecute(Sender: TObject);
  private
    pPort : Integer;
    pIp, pAe : string;
    ...
    procedure ShowRejim;
    procedure SetAN;
    function ChkvImgSndList ( pInstanceNmb : string):Boolean;
  ...
  public
    ...
    vImgCheckList, vImgSndList: TStringList;
...
procedure TfrmCapture.CMDialogKey( var msg: TCMDialogKey );
begin  //
  if (not fIsTab(aCapture.ShortCut)) and
     (not fIsTab(aStartCapVideo.ShortCut)) and
     (not fIsTab(aStopCapVideo.ShortCut)) and
     (not fIsTab(aClose.ShortCut))
  then
    inherited
  else
  if (not fIsTab(msg.Charcode))
  then
    inherited
  else
    if fIsTab(aCapture.ShortCut)
    then
      aCapture.Execute
    else if fIsTab(aStartCapVideo.ShortCut)
    then
      aStartCapVideo.Execute
    else if fIsTab(aStopCapVideo.ShortCut)
    then
      aStopCapVideo.Execute
    else if fIsTab(aClose.ShortCut)
    then
      aClose.Execute;
end;
...
begin
  ...
  begin
    ...
    pAssignAttr( $0020, $000D, v_teStudyUID ); // STUDYUID: Add(425, $0020, $000D, tUI, 'StudyInstanceUID', '1');
    pAssignAttr( $0008, $0060, v_teModality ); // MODALITY: Add(81, $0008, $0060, tCS, 'Modality', '1');
    pAssignAttr( $0008, $0090, v_tePhysiciansName ); // Врач: Add(88, $0008, $0090, tPN, 'ReferringPhysiciansName', '1');
    pAssignAttr( $0008, $1010, v_teStationName ); // Наименование раб.станции: Add(94, $0008, $1010, tSH, 'StationName', '1');
...
begin
  ...
  begin
      vSOPInstanceUID := CaptureView.DicomDatasets.Item[i].Attributes.GetString(dSOPInstanceUID) ;
      if not ChkvImgSndList(vSOPInstanceUID) then
      begin
        vOK:=False;
        Break;
      end;
  ...
  if (vOK or v_cbCloseFormSnd or (CaptureView.DicomDatasets=nil) or (CaptureView.DicomDatasets.Count<=0) or
      (MessageDlg('Несохраненные данные будут потеряны. Выйти ?', mtConfirmation, [mbYes, mbNo], 0)=mrYes))
  then begin

    if v_cbSaveRazmer then
    begin
      vCapFrmHeight := frmCapture.Height;
      vCapFrmWidth  := frmCapture.Width;
      Reg := TRegIniFile.Create;
      try
        Reg.RootKey:=HKEY_CURRENT_USER;
        pRegCrtPart;
        Reg.WriteInteger(cRegIniPart, cCapFrmHeight, vCapFrmHeight ) ;
        Reg.WriteInteger(cRegIniPart, cCapFrmWidth, vCapFrmWidth ) ;
      finally
        Reg.Free;
      end;
    end;    
...
begin
  ...
  vImgSndList  := TStringList.Create;
  vRejim:=False;
  ShowRejim;
  vStartCaptureVideo:=False;
  aStopCapVideo.Enabled:=False;
  //
  teProcDescription.Text := v_teProcDescription;
  //
  FldClr;
  //
...
begin
  ...
  begin
    with TIniFile.Create(ExtractFilePath(paramstr(0))+'Video.ini') do
    try
...
function TfrmCapture.ChkvImgSndList ( pInstanceNmb : string):Boolean;
var i :Integer; vRes : Boolean;
begin
  vRes:=False;
  for I := 0 to vImgSndList.Count - 1 do
    if vImgSndList[i]=pInstanceNmb then
    begin
      vRes:=True;
      Break;
    end;
...
var i :Integer; vNmbStr : string; vRes : Boolean;
begin
  vRes:=False;
  vNmbStr:=IntToStr(pi);
...
begin
  ...
  ini1 := TIniFile.Create(ExtractFilePath(paramstr(0))+'Video.ini');
  try
...
begin
  v_now := Now;
  teAccessionNumber.Text := v_teModality+'-'+teP_PID.Text+'/'+
...
procedure TfrmCapture.aLoadFromDiocmFileExecute(Sender: TObject);
var vFileName : string;
begin  // Загрузить dcm
  OpenDialog1.InitialDir := ExtractFilePath(paramstr(0));
  if OpenDialog1.Execute then
  begin
    vFileName := OpenDialog1.Files[0];
    CnsDMTable1.LoadFromFile( vFileName );
    Inc(fImageIndex);
    CaptureView.RefreshNews;
    CaptureView.Last;
    CaptureView.UpdateScrollBars;

    CaptureViewClick(Self);
    CaptureViewDblClick(Self);
  end;
end;
...
begin //  Печать снимков (видео не печатается)
  ...
  try
    ...
    begin
      ...
      begin
        if (v_cbCheckedImages and (not ChkItemInChecked(i))) then Continue;
...
var CnsDicomConnection1 : TCnsDicomConnection;
  ...
  begin
    ...
    try
      ...
      //
      CnsDicomConnection1.v_is_log := False;
...
var vDicomDataSets : TDicomDatasets; vSOPInstanceUID : string;
begin
  if fchkFullDataInForm then Exit;
  // проверим есть ли видео
  if CaptureView.DicomDatasets.Count>0 then 
  begin
    ...
    try
        ...
        begin
            vSOPInstanceUID := CaptureView.DicomDatasets.Item[i].Attributes.GetString(dSOPInstanceUID) ;
//            if (v_cbCheckedImages and (not ChkItemInChecked(i)) and (vImgSndList[i]<>'1')) then Continue;
            if ( v_cbCheckedImages and (not ChkItemInChecked(i)) ) then Continue;
            if ChkvImgSndList(vSOPInstanceUID) then Continue;
            if CmnUnit.v_exit_progressbar=1 then Break;
            v_DCMAttr:=CaptureView.DicomDatasets.Item[i].Attributes;
            AFileName:=gt_avi_filename( v_DCMAttr.getInteger(dInstanceNumber) );
            //
            ...
            vImgSndList.Append( vSOPInstanceUID );
            if CmnUnit.v_exit_progressbar=1 then Break;
        end;
    finally
      if v_is_ok then
        ...
        begin
          ...
          if (vIsFromWeb or (v_cbCloseFormSnd)) then ModalResult:=mrOk;
//          if v_cbCleanData then
//            FldClr;
        end
      else
