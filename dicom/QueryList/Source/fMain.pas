...
type
  TfrmMain = class(TForm)
    ...
    aDownList: TAction;
    mdWorklistP_ENAME_RUS: TStringField;
    cxP_ENAME_RUS: TcxGridDBColumn;
    cbFindMod: TcxCheckBox;
    ...
    procedure aDownListExecute(Sender: TObject);
  private
    nDates : Integer;
  ...
  public
    Russian, Latin : HKL; // для переключения раскладки
    sUser : string;
    sCallingAETitle,
...
const v_is_ds_log = True;
      ...
      chkfrmFrmOpCl = 'Код открытия/закрытия формы видеозахвата';
      chkListDown   = 'Код перемещения вниз по списку назначений';

      cRegIniPart = '\Software\Softmaster\QueryList\guest';
      cModality       = 'Modality' ;
      ...
      cTAB = 'TAB'; cCode=VK_TAB;

function fIsTab(pShortCut:Word):Boolean;
...
function fIsTab(pShortCut:Word):Boolean;
begin
  Result:=(pShortCut=cCode);
end;

procedure TfrmMain.CMDialogKey( var msg: TCMDialogKey );
begin  //
  if (not fIsTab(aDoIssl.ShortCut)) and
     (not fIsTab(aDownList.ShortCut))
  then
    inherited
  else
  ...
  then
    ...
    if fIsTab(aDoIssl.ShortCut)
    then
      aDoIssl.Execute
    else if fIsTab(aDownList.ShortCut)
    then
      aDownList.Execute;
end;

procedure TfrmMain.pSetKeys;
begin
  aDoIssl.ShortCut := v_hkFrmOpCl;
  aDownList.ShortCut := v_hkListDown;
  //
  cbFindMod.Caption := 'отображать модальность '+v_teModality;
end;
...
begin
  ...
  try
    ...
    if not TryStrToInt(p_port,val) then
      val := 0;
    CnsDicomConnection1.Port := val;
...
begin  // video captute
  ...
  try
    frmCapture.aLoadFromDiocmFile.Visible := v_cbLoadDicom;
...
procedure TfrmMain.aDownListExecute(Sender: TObject);
begin
  if not mdWorklist.Eof then
    mdWorklist.Next;
  if mdWorklist.Eof then
    mdWorklist.First;  
end;
...
//    CnsDicomConnection1.v_log_filename := 'mwl_scu.log';
    ...
    if not TryStrToInt(v_teWLPort,val) then
      val:=0;
    CnsDicomConnection1.Port := val;
  ...
  //  CnsDicomConnection1.ClearPresentationContext;
    ...
    begin
      ...
      if cbFindMod.Checked then
        AddVariant(dModality, v_teModality) // Modality
      else
          ...
          begin
            ...
            ds_sav(das,nil,nil,'.records');
              ...
              //  vStrTmp := das.GetString( $0040, $0003 );
                ...
                mdWorklist.FieldByName('P_ENAME_RUS').AsString := TranslitEng(das.GetString($0010, $0010)); //(PatientName)PN=<0>NULL
                ...
                begin
                  ...
                  vIsOK:=( (not cbFindMod.Checked) or
                           (cbFindMod.Checked and (vModality=v_teModality)) or
                           (vModality='')
                         );
                  //
                  mdWorklist.FieldByName('Modality').AsString         := vModality;
...
begin
  ...
  try   //
    ...
    Ini.WriteInteger(c_rzd,chkfrmFrmOpCl,v_hkFrmOpCl);
    Ini.WriteInteger(c_rzd,chkListDown,v_hkListDown);
...
begin
  ...
  try   // чтение параметров соединения с БД
    ...
    v_hkFrmOpCl := Ini.ReadInteger(c_rzd,chkfrmFrmOpCl,0);
    v_hkListDown := Ini.ReadInteger(c_rzd,chkListDown,0);
  ...
  //  v_cbCleanData     := ( Reg.ReadInteger(cRegIniPart, cIsClear, 1 ) = 1 );
    ...
    v_cbLoadDicom := ( Reg.ReadInteger(cRegIniPart, cLoadDicom, 1 ) = 1 );
