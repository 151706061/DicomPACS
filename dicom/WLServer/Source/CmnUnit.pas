...
procedure f_upd_status( const p_BSTATE: ansistring; const P_PID: ansistring; const p_UIDFIELD: ansistring );
...
procedure EventLogIns( const sEvent: ansistring; const nType: Integer; const dDate: TDateTime; const sRemark: ansistring);
...
begin
  ...
  Ini := TIniFile.Create(ExtractFilePath(paramstr(0))+'SettingsWL.ini');
  try
    v_Lis_ds_log := Ini.ReadInteger(c_razd, c_dataset_log, 1);
    v_Lis_log    := Ini.ReadInteger(c_razd, c_db_log     , 1);
    v_Lis_db_log := Ini.ReadInteger(c_razd, c_txt_log    , 1);
    //
    Ini.WriteInteger(c_razd, c_dataset_log,v_Lis_ds_log);
    Ini.WriteInteger(c_razd, c_db_log     ,v_Lis_log);
    Ini.WriteInteger(c_razd, c_txt_log    ,v_Lis_db_log);
  finally
    Ini.Free;
  end;
...
begin
  If fMain.v_is_ds_log then begin
    ...
    try
      ...
      v_I := 0;
      Lg_ev('ds '+inttostr(CmnUnit.v_ds_i));
      //
      try
...
begin
  try
    ...
    //
    CmtTrans(DicomWLServer.trRead);
    //
    FIBQuery.Transaction := DicomWLServer.trRead;
    FIBQuery.SQL.Clear;
...
begin
  if fMain.v_is_db_log then begin
    ...
    try
      ...
      FIBQuery.Database := DB;
      //
      CmtTrans(DicomWLServer.trWrite);
      //
      FIBQuery.Transaction := DicomWLServer.trWrite;
