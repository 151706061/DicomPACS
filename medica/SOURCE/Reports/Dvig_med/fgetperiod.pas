...
begin
  Result:=
    ...
    '  <td colspan=10 class=xl4311056 width=705 style=''border-right:.5pt solid black; border-left:none;width:531pt''>'+AText+'</td>'#13#10+
    '</tr>'#13#10;
{
 Result:=
  '<tr height=17 style=''height:12.75pt''>'#13#10+
...
function TfrmGetPeriod.GetRow(A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11: String): String;
begin
  if A1='' then A1:=' ';
  if A2='' then A2:=' ';
  if A3='' then A3:=' ';
...
//    Open;
  ...
  if FileExists('Dvig_med_shablon.htm') then
  begin               
    sFileName1 := ChangeFileExt(fdmPrintSelect.PrepareFilePath(aFileName), 'htm');
    CopyFile(PAnsiChar('Dvig_med_shablon.htm'), PAnsiChar(sFileName1), false);
...
//    end;
  ...
  //  if not VarisNull(lcbType.KeyValue) then
    ...
    //if FileExists('Dvig_Med_frPrihDoc.frf') then            // если найден файл
    //  frPrihDoc.LoadFromFile('Dvig_Med_frPrihDoc.frf');  // загружаем его
    if FileExists('Dvig_Med_frPrihDoc.fr3') then            // если найден файл
      frPrihDoc.LoadFromFile('Dvig_Med_frPrihDoc.fr3');  // загружаем его
