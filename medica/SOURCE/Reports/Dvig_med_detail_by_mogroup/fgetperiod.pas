...
uses Variants, IniFiles, SMMainAPI, fdmPrintSelect;
{$R *.DFM}
...
begin
  ...
  if lcbMO_GROUP.Text = EmptyStr then
      lcbMO_GROUP.KeyValue := oqInitMO.GetVariable('nMO_ROOT_GROUP');
...
begin
  ...
  try
    ...
    odsRep.SetVariable('pMOGROUP_ID', lcbMO_GROUP.KeyValue);
    odsRep.Open;

    if FileExists('frxDvig_med_detail_by_mogroup.fr3') then
      frxRep.LoadFromFile('frxDvig_med_detail_by_mogroup.fr3');
...
begin
  ...
  if UpperCaseVarName = 'FC_OTD_NAME' then
