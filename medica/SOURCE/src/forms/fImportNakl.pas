...
uses
  ...
  Dialogs, xmldom, XMLIntf, StdCtrls, msxmldom, XMLDoc, Grids, Oracle, fdmMain,
...
implementation
uses
  fPostavLst, fMedic, ComObj, uTLINK_IMPNAKL, uGlobalConst, Excel2000;
{$R *.dfm}

function TfmImportNakl.OpenExcelFile(aFileName: string): Boolean;
begin
  try
...
procedure TfmImportNakl.CloseExcelFile;
begin
    ...
    //      DocImp.DocITEMSImp[i].FK_MEDIC_ID_OUR := DoShowMedic(n1, True, True, True, '', 'Медикамент:: '+DocImp.DocITEMSImp[i].ImpItem[IFI_MEDIC_TORG_NAME], nvl(dmMain.CurMO_SKLAD_ID,0));
        ...
        begin // не получилось найти
          ...
          begin
            ...
            if TempFloat = 0 then
            begin
              dmMain.pkgProdCert.Add(nvl(odsTIMP_DOC_ITEMS2.FieldByName('fc_src_party_proizvoditel').AsVariant,'Не указан'), TempFloat);
...
function TfmImportNakl.OpenExcel(aFileName: string): Boolean;
var
  i1, i2 : integer;
  ClearStr : boolean;
begin
  Result := True;
  if OpenExcelFile(aFileName) then
  begin
    oqAddDoc.Session.Savepoint('BeforeAdd');
    try
      try
        oqAddDoc.ClearVariables;
        oqAddDoc.SetVariable('pfc_src_doc_num', Trim(NVL( XlSheet.Cells[ 1, 7 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_doc_date', Trim(NVL(XlSheet.Cells[ 1, 3 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_postav', Trim(NVL(XlSheet.Cells[ 2, 3 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_kontrakt', Trim(NVL(XlSheet.Cells[ 3, 3 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_vid_oplati', Trim(NVL(XlSheet.Cells[ 3, 7 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_schet_faktura', Trim(NVL(XlSheet.Cells[ 3, 10 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_comment', Trim(NVL(XlSheet.Cells[ 4, 3 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_summa_s_nds', Trim(NVL(XlSheet.Cells[ 5, 3 ].Value,'')));
        oqAddDoc.SetVariable('pfc_src_kolvo_pos', Trim(NVL(XlSheet.Cells[ 5, 7 ].Value,'')));
        oqAddDoc.SetVariable('pfc_imp_filename', aFileName);
        oqAddDoc.Execute;
        DocID := oqAddDoc.GetVariable('pDocID');
        ...
        repeat
          oqAddDocItem.ClearVariables;
          oqAddDocItem.SetVariable('pfk_imp_doc_id', DocID);
          oqAddDocItem.SetVariable('pfc_src_num', Trim(NVL(XlSheet.Cells[ i1, 1 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_medic_torg_name', Trim(NVL(XlSheet.Cells[ i1, 2 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_medic_id_their', Trim(NVL(XlSheet.Cells[ i1, 3 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_medic_ed_izm', Trim(NVL(XlSheet.Cells[ i1, 4 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_medic_kolvo', Trim(NVL(XlSheet.Cells[ i1, 5 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_seria', Trim(NVL(XlSheet.Cells[ i1, 6 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_godnost', Trim(NVL(XlSheet.Cells[ i1, 7 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_price_bez_nds',  Trim(NVL(XlSheet.Cells[ i1, 8 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_summ_nds', Trim(NVL(XlSheet.Cells[ i1, 9 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_nds', Trim(NVL(XlSheet.Cells[ i1, 10 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_proizvoditel', Trim(NVL(XlSheet.Cells[ i1, 11 ].Value,'')));
          oqAddDocItem.SetVariable('pfc_src_party_sertifikat', Trim(NVL(XlSheet.Cells[ i1, 12 ].Value,'')));
          ClearStr := True;
  ...
  begin
    case cmbImportSource.ItemIndex of
      ...
      begin
        ...
        begin
          OpenExcel(OpenDialog2.FileName);
          ParseError := DoParse;
        end;
      end;
      1:
      begin
        ...
        begin
          OpenXML_OKB(XMLDocument1, OpenDialog1.FileName);
          ParseError := DoParse;
        end;
      end;
    end;
...
//  kontr := odsTIMP_DOC.FieldByName('fc_src_kontrakt').AsString;
  ...
  try
    ...
    MessageBox(Handle, 'MS Excel не установлен на компьютере', 'Внимание', MB_ICONWARNING or MB_OK);
    exit;
  end;
  Excel.Workbooks.Add;
...
//  Excel.ActiveSheet.Cells[ 1, 1 ].Value := 'Прейскурант цен на платные услуги';
  ...
  odsNaklByDogovor.Close;
  odsNaklByDogovor.SetVariable('PPOSTID', odsTIMP_DOC.FieldByName('fk_post_id').AsVariant);
  odsNaklByDogovor.SetVariable('PFC_DOG_NUM', kontr);
  odsNaklByDogovor.Open;
  odsNaklByDogovor.First;
  i := 1;
  while i <= odsNaklByDogovor.RecordCount do
  begin
    Excel.ActiveSheet.Cells[ i+HeaderRow, 1 ].Value := i;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 2 ].Value := odsNaklByDogovor.FieldByName('fc_medic').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 3 ].Value := odsNaklByDogovor.FieldByName('fc_ei').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 4 ].Value := odsNaklByDogovor.FieldByName('fn_dog_kol').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 5 ].Value := odsNaklByDogovor.FieldByName('fn_dog_price').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 6 ].Value := odsNaklByDogovor.FieldByName('fn_dog_sum').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 7 ].Value := odsNaklByDogovor.FieldByName('fn_nakl_kol').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 8 ].Value := odsNaklByDogovor.FieldByName('fn_nakl_sum').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 9 ].Value := odsNaklByDogovor.FieldByName('fn_nedopost_kol').AsVariant;
    Excel.ActiveSheet.Cells[ i+HeaderRow, 10].Value := odsNaklByDogovor.FieldByName('fn_nedopost_sum').AsVariant;

    fn_summ_dogovor := fn_summ_dogovor + odsNaklByDogovor.FieldByName('fn_dog_sum').AsFloat;
    fn_summ_nakl    := fn_summ_nakl    + odsNaklByDogovor.FieldByName('fn_nakl_sum').AsFloat;
    odsNaklByDogovor.Next;
    inc( i );
  end;
  odsNaklByDogovor.Close;
...
begin
  FIsNeedRefreshItems := True;
  odsTIMP_DOC.Open;
  odsMedicsList.SetVariable('PFK_SKLAD_ID', dmMain.CurMO_SKLAD_ID);
  odsMedicsList.Open;
  odsTProdCert_list.Open;
  odsTIMP_DOCAfterScroll( odsTIMP_DOC );

  buKontrNakl.Visible := (dmMain.ConfigParams.GetParamValue('FUNC_DOGOVOR_NAKL_ENABLE', 0));
  buImport.Visible    := (dmMain.ConfigParams.GetParamValue('FUNC_IMP_NAKL_ENABLE', 0));
end;
